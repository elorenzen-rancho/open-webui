const d = (n) => typeof n == 'string',
	...
	de = /###/g,
	_ = (n) => (n && n.indexOf('###') > -1 ? n.replace(de, '.') : n),
	ee = (n) => !n || d(n),
	D = (n, e, t) => {
	...
	ue = (n, e, t) => {
		for (const s in e)
			s !== '__proto__' &&
				s !== 'constructor' &&
				(s in n
					? d(n[s]) || n[s] instanceof String || d(e[s]) || e[s] instanceof String
						? t && (n[s] = e[s])
						: ue(n[s], e[s], t)
					: (n[s] = e[s]));
		return n;
...
class z {
	...
	forward(e, t, s, i) {
		return i && !this.debug
			? null
			: (d(e[0]) && (e[0] = `${s}${this.prefix} ${e[0]}`), this.logger[t](e));
	}
	create(e) {
		return new z(this.logger, { prefix: `${this.prefix}:${e}:`, ...this.options });
	}
	clone(e) {
...
class se extends B {
	...
	getResource(e, t, s) {
		...
		e.indexOf('.') > -1
			? (o = e.split('.'))
			: ((o = [e, t]),
				s && (Array.isArray(s) ? o.push(...s) : d(s) && r ? o.push(...s.split(r)) : o.push(s)));
		const l = M(this.data, o);
		return (
			!l && !t && !s && e.indexOf('.') > -1 && ((e = o[0]), (t = o[1]), (s = o.slice(2).join('.'))),
			l || !a || !d(s) ? l : Y(this.data && this.data[e] && this.data[e][t], s, r)
		);
	}
	addResource(e, t, s, i) {
	...
	addResources(e, t, s) {
		...
		for (const r in s)
			(d(s[r]) || Array.isArray(s[r])) && this.addResource(e, t, r, s[r], { silent: !0 });
...
class J extends B {
	...
	extractFromKey(e, t) {
		...
		if (a && !o) {
			...
			if (l && l.length > 0) return { key: e, namespaces: d(r) ? [r] : r };
			const f = e.split(s);
			((s !== i || (s === i && this.options.ns.indexOf(f[0]) > -1)) && (r = f.shift()),
				(e = f.join(i)));
		}
		return { key: e, namespaces: d(r) ? [r] : r };
	}
	translate(e, t, s) {
		if (
			(typeof t != 'object' &&
		...
		const c = (g && g.usedKey) || a,
			p = (g && g.exactUsedKey) || a,
			x = Object.prototype.toString.apply(u),
			...
			k = !d(u) && typeof u != 'boolean' && typeof u != 'number';
		if (y && u && k && m.indexOf(x) < 0 && !(d(O) && Array.isArray(u))) {
			...
			if (r) {
				...
				for (const S in u)
					if (Object.prototype.hasOwnProperty.call(u, S)) {
						const T = `${C}${r}${S}`;
						((v[S] = this.translate(T, { ...t, joinArrays: !1, ns: o })),
							v[S] === T && (v[S] = u[S]));
					}
				u = v;
			}
			...
			let b = !1,
				v = !1;
			const C = t.count !== void 0 && !d(t.count),
				S = J.hasDefaultValue(t),
				T = C ? this.pluralResolver.getSuffix(f, t.count, t) : '',
				he = t.ordinal && C ? this.pluralResolver.getSuffix(f, t.count, { ordinal: !1 }) : '',
				G = C && !t.ordinal && t.count === 0 && this.pluralResolver.shouldUseIntlApi(),
				E =
					(G && t[`defaultValue${this.options.pluralSeparator}zero`]) ||
					t[`defaultValue${T}`] ||
					t[`defaultValue${he}`] ||
					t.defaultValue;
			(!this.isValidLookup(u) && S && ((b = !0), (u = E)),
				this.isValidLookup(u) || ((v = !0), (u = a)));
			const ge =
					(t.missingKeyNoValueFallbackToKey || this.options.missingKeyNoValueFallbackToKey) && v
						? void 0
						: u,
				F = S && E !== u && this.options.updateMissing;
			if (v || b || F) {
				if ((this.logger.log(F ? 'updateKey' : 'missingKey', f, l, a, F ? E : u), r)) {
					const w = this.resolve(a, { ...t, keySeparator: !1 });
					w &&
						w.res &&
						this.logger.warn(
				...
				let I = [];
				const K = this.languageUtils.getFallbackCodes(
					this.options.fallbackLng,
					t.lng || this.language
				);
				if (this.options.saveMissingTo === 'fallback' && K && K[0])
					for (let w = 0; w < K.length; w++) I.push(K[w]);
				else
					this.options.saveMissingTo === 'all'
						? (I = this.languageUtils.toResolveHierarchy(t.lng || this.language))
						: I.push(t.lng || this.language);
				const Z = (w, R, j) => {
					const X = S && j !== u ? j : ge;
					(this.options.missingKeyHandler
						? this.options.missingKeyHandler(w, l, R, X, F, t)
						: this.backendConnector &&
							this.backendConnector.saveMissing &&
							this.backendConnector.saveMissing(w, l, R, X, F, t),
						this.emit('missingKey', w, l, R, u));
				};
	...
	extendTranslation(e, t, s, i, r) {
		...
		if (this.i18nFormat && this.i18nFormat.parse)
			...
			if (
				...
				(e = this.interpolator.interpolate(e, g, s.lng || this.language || i.usedLng, s)),
				f)
			) {
				const u = e.match(this.interpolator.nestingRegexp),
					c = u && u.length;
	...
	resolve(e) {
		...
		return (
			...
			e.forEach((l) => {
				...
				const u = t.count !== void 0 && !d(t.count),
					c = u && !t.ordinal && t.count === 0 && this.pluralResolver.shouldUseIntlApi(),
					p =
						t.context !== void 0 &&
				...
				g.forEach((m) => {
					this.isValidLookup(s) ||
						...
						!ie[`${x[0]}-${m}`] &&
							...
							((ie[`${x[0]}-${m}`] = !0),
							this.logger.warn(
								`key "${i}" for languages "${x.join(', ')}" won't get resolved as namespace "${o}" was not yet loaded`,
							...
							if (this.i18nFormat && this.i18nFormat.addLookupKeys)
								...
								if (
									(u &&
										...
										t.ordinal &&
											b.indexOf(C) === 0 &&
											y.push(h + b.replace(C, this.options.pluralSeparator)),
										c && y.push(h + v)),
									p)
								) {
									...
									(y.push(S),
										u &&
											...
											t.ordinal &&
												b.indexOf(C) === 0 &&
												y.push(S + b.replace(C, this.options.pluralSeparator)),
											c && y.push(S + v)));
								}
							}
							let k;
			...
			return (
				this.options.lowerCaseLng
					...
					: s.length === 2
						? ((s[0] = s[0].toLowerCase()),
							(s[1] = s[1].toUpperCase()),
							t.indexOf(s[1].toLowerCase()) > -1 && (s[1] = W(s[1].toLowerCase())))
						: s.length === 3 &&
							((s[0] = s[0].toLowerCase()),
							s[1].length === 2 && (s[1] = s[1].toUpperCase()),
							s[0] !== 'sgn' && s[2].length === 2 && (s[2] = s[2].toUpperCase()),
							t.indexOf(s[1].toLowerCase()) > -1 && (s[1] = W(s[1].toLowerCase())),
							t.indexOf(s[2].toLowerCase()) > -1 && (s[2] = W(s[2].toLowerCase()))),
				s.join('-')
			);
		}
				...
				e.forEach((s) => {
					...
					t = this.options.supportedLngs.find((r) => {
						...
						if (
							...
							((r.indexOf('-') > 0 && i.indexOf('-') < 0 && r.substring(0, r.indexOf('-')) === i) ||
	...
	toResolveHierarchy(e, t) {
		...
		return (
			d(e) && (e.indexOf('-') > -1 || e.indexOf('_') > -1)
				...
				: d(e) && r(this.formatLanguageCode(e)),
			s.forEach((a) => {
...
const Ne = ['v1', 'v2', 'v3'],
	...
	Pe = () => {
		...
		return (
			we.forEach((e) => {
				e.lngs.forEach((t) => {
					n[t] = { numbers: e.nr, plurals: Ce[e.fc] };
				});
			}),
			n
		);
...
class $e {
	constructor(e) {
		...
		((this.languageUtils = e),
			...
			(!this.options.compatibilityJSON || Re.includes(this.options.compatibilityJSON)) &&
	...
	getSuffixes(e) {
		...
		return s
			? this.shouldUseIntlApi()
				? s
						.resolvedOptions()
						.pluralCategories.sort((i, r) => re[i] - re[r])
						.map(
							(i) =>
...
class ke {
	...
	init() {
		...
		((this.escape = t !== void 0 ? t : be),
			...
			(this.nestingPrefix = u ? P(u) : c || P('$t(')),
			(this.nestingSuffix = p ? P(p) : x || P(')')),
	...
	interpolate(e, t, s, i) {
		...
		const l =
			...
			f = (c) => {
				...
				return this.format(
					ae(t, l, x, this.options.keySeparator, this.options.ignoreJSONStructure),
					m,
					s,
					{ ...i, ...t, interpolationkey: x }
				);
			};
		this.resetRegExp();
		...
		return (
			[
				{ regex: this.regexpUnescape, safeValue: (c) => Q(c) },
				{ regex: this.regexp, safeValue: (c) => (this.escapeValue ? Q(this.escape(c)) : Q(c)) }
			].forEach((c) => {
	...
	nest(e, t) {
		...
		for (; (i = this.nestingRegexp.exec(e)); ) {
			let l = [];
			((a = { ...s }),
				(a = a.replace && !d(a.replace) ? a.replace : a),
			...
			(d(r) || (r = q(r)),
				...
				f &&
					(r = l.reduce(
						(h, g) => this.format(h, g, s.lng, { ...s, interpolationkey: i[1].trim() }),
						r.trim()
					)),
				(e = e.replace(i[0], r)),
...
class Fe {
	...
	format(e, t, s) {
		...
		return r.reduce((o, l) => {
			const { formatName: f, formatOptions: h } = Ee(l);
			if (this.formats[f]) {
				let g = o;
				try {
...
class je extends B {
	...
	read(e, t, s) {
		...
		if (this.readingCalls >= this.maxParallelReads) {
			this.waitingReads.push({ lng: e, ns: t, fcName: s, tried: i, wait: r, callback: a });
			return;
		}
		this.readingCalls++;
		const o = (f, h) => {
				...
				if (f.length < 6)
					try {
						...
						(f.length === 5 ? (h = f(e, t, s, i, l)) : (h = f(e, t, s, i)),
							h && typeof h.then == 'function' ? h.then((g) => o(null, g)).catch(o) : o(null, h));
					} catch (h) {
						o(h);
					}
				else f(e, t, s, i, o, l);
...
const oe = () => ({
		...
		overloadTranslationOptionHandler: (n) => {
			...
			if (
				...
				d(n[1]) && (e.defaultValue = n[1]),
				d(n[2]) && (e.tDescription = n[2]),
	...
	le = (n) => (
		d(n.ns) && (n.ns = [n.ns]),
		d(n.fallbackLng) && (n.fallbackLng = [n.fallbackLng]),
		d(n.fallbackNS) && (n.fallbackNS = [n.fallbackNS]),
		n.supportedLngs &&
...
class V extends B {
	...
	init() {
		...
		const i = oe();
		((this.options = { ...i, ...this.options, ...le(t) }),
			this.options.compatibilityAPI !== 'v1' &&
		...
		if (!this.options.isClone) {
			...
			((u.logger = N),
				...
				(u.interpolator = new ke(this.options)),
				(u.utils = { hasLoadedNamespace: this.hasLoadedNamespace.bind(this) }),
				(u.backendConnector = new je(r(this.modules.backend), u.resourceStore, u, this.options)),
	...
	use(e) {
		...
		return (
			...
			e.type === 'postProcessor' && fe.addPostProcessor(e),
	...
	loadLanguages(e, t) {
		...
		const i = this.options.preload || [],
			r = e.filter((a) => i.indexOf(a) < 0 && this.services.languageUtils.isSupportedCode(a));
		return r.length
	...
	dir(e) {
		...
		const t = [
			...
			s = (this.services && this.services.languageUtils) || new ne(oe());
	...
	cloneInstance() {
		...
		return (
			...
			s && ((r.store = new se(this.store.data, i)), (r.services.resourceStore = r.store)),
			(r.translator = new J(r.services, i)),
...
L.loadLanguages;
export { L as i };
//# sourceMappingURL=B2185GOa.js.map
