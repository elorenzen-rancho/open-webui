...
import { html as bt, htmlCompletionSource as wt } from './DDyF20IL.js';
class _ {
	...
	addChild(e, r) {
		(e.prop(v.contextHash) != this.hash &&
			(e = new H(e.type, e.children, e.positions, e.length, this.hashProp)),
...
function de(t, e, r) {
	...
	return (
		n > 0 &&
		(t.type != u.BulletList || se(r, e, !1) < 0) &&
		r.text.charCodeAt(r.pos + n - 1) == t.value
	);
}
const He = {
	[u.Blockquote](t, e, r) {
...
function oe(t, e, r) {
	...
	return n == t.pos ||
		...
		(r &&
			!Ne(e, u.OrderedList) &&
			(t.skipSpace(n + 1) == t.text.length || n > t.pos + 1 || t.next != 49))
		? -1
		: n + 1 - t.pos;
}
function ye(t) {
...
const $ = {
	...
	IndentedCode(t, e) {
		...
		return (
			a.length &&
				((a = a.filter((l) => l.type != u.CodeText)),
	...
	FencedCode(t, e) {
		...
		return (
			t.addNode(t.buffer.writeElements(l, -n).finish(u.FencedCode, t.prevLineEnd() - n), n),
			!0
		);
	},
	Blockquote(t, e) {
	...
	OrderedList(t, e) {
		let r = oe(e, t, !1);
		if (r < 0) return !1;
		t.block.type != u.OrderedList &&
			t.startContext(u.OrderedList, e.basePos, e.text.charCodeAt(e.pos + r - 1));
		let n = ce(e, e.pos + r);
		return (
			t.startContext(u.ListItem, e.basePos, n - e.baseIndent),
			t.addNode(u.ListMark, t.lineStart + e.pos, t.lineStart + e.pos + r),
	...
	advance(e) {
		for (;;) {
			...
			if (this.stage == 0) {
				...
				(this.elts.push(k(u.LinkMark, this.pos + this.start, this.pos + this.start + 1)),
					this.pos++);
		...
		for (this.absoluteLineEnd = n, e.reset(r); e.depth < this.stack.length; e.depth++) {
			...
			if (!i(s, this, e)) {
				(this.line.markers.length > o &&
					(s.end = this.line.markers[this.line.markers.length - 1].to),
					e.forward());
				break;
			}
			e.forward();
...
`);
	...
	addNode(e, r, n) {
		(typeof e == 'number' &&
			(e = new H(this.parser.nodeSet.types[e], N, N, (n ?? this.prevLineEnd()) - r)),
	...
	addLeafElement(e, r) {
		this.addNode(
			this.buffer.writeElements(re(r.children, e.marks), -r.from).finish(r.type, r.to - r.from),
			r.from
		);
	}
	finishContext() {
	...
	addGaps(e) {
		return this.ranges.length > 1
			? Fe(this.ranges, 0, e.topNode, this.ranges[0].from, this.reusePlaceholders)
			: e;
	}
	finishLeaf(e) {
		...
		this.addNode(
			this.buffer.writeElements(r, -e.start).finish(u.Paragraph, e.content.length),
			e.start
		);
	}
	elt(e, r, n, s) {
		...
		if (R(r.defineNodes)) {
			...
			for (let L of r.defineNodes) {
				...
				(d.push(Q.define({ id: b, name: g, props: E && [[v.group, E]] })),
					S &&
						(c || (c = {}),
						Array.isArray(S) || S instanceof it ? (c[g] = S) : Object.assign(c, S)));
			}
...
for (let t = 1, e; (e = u[t]); t++)
	Xe[t] = Q.define({
		...
		props:
			t >= u.Escape
				? []
				: [[v.group, t in He ? ['Block', 'BlockContext'] : ['Block', 'LeafBlock']]],
		top: e == 'Document'
	});
const N = [];
class $e {
...
const V = {
	Escape(t, e, r) {
		...
		for (let s = 0; s < me.length; s++)
			if (me.charCodeAt(s) == n) return t.append(k(u.Escape, r, r + 2));
		return -1;
	},
	Entity(t, e, r) {
		if (e != 38) return -1;
		...
		for (; n < t.end; n++)
			if (t.char(n) == 96) {
				if ((i++, i == s && t.char(n + 1) != 96))
					return t.append(
						k(u.InlineCode, r, n + 1, [k(u.CodeMark, r, r + s), k(u.CodeMark, n + 1 - s, n + 1)])
					);
			} else i = 0;
		return -1;
	},
...
function Mt(t, e, r, n, s) {
	let { text: i } = t,
		...
		a = s;
	if (
		(e.unshift(k(u.LinkMark, n, n + (r == u.Image ? 2 : 1))),
		e.push(k(u.LinkMark, s - 1, s)),
		o == 40)
	) {
		...
		(h &&
			((l = t.skipSpace(h.to)),
			l != h.to && ((f = Qe(i, l - t.offset, t.offset)), f && (l = t.skipSpace(f.to)))),
			t.char(l) == 41 &&
				(e.push(k(u.LinkMark, s, s + 1)),
				(a = l + 1),
				h && e.push(h),
				f && e.push(f),
				e.push(k(u.LinkMark, l, a))));
	} else if (o == 91) {
...
`;
	...
	takeNodes(e) {
		...
		for (;;) {
			...
			if (
				(r.type.is('Block') &&
					(Ht.indexOf(r.type.id) < 0
						? ((o = r.to - n), (a = e.block.children.length))
						: ((o = l), (a = h)),
					(l = r.to - n),
...
class ke {
	...
	finish(e, r) {
		return this.rows
			? (e.addLeafElement(r, e.elt('Table', r.start, r.start + r.content.length, this.rows)), !0)
			: !1;
	}
}
const zt = {
		...
		parseBlock: [
			{
				name: 'TaskList',
				leaf(t, e) {
					return /^\[[ xX]\][ \t]/.test(e.content) && t.parentType().name == 'ListItem'
						? new Dt()
						: null;
				},
				after: 'SetextHeading'
					...
					if (p.item && d) {
						...
						if (
							...
							let b = f.length > 1 ? f[f.length - 2] : null,
								...
								I = '';
							b && b.item
								? ((E = h.from + b.from), (I = b.marker(s, 1)))
								: (E = h.from + (b ? b.to : 0));
					...
					if (!L || /^[\s\d.)\-+*>]*/.exec(h.text)[0].length >= p.to)
						for (let w = 0, S = f.length - 1; w <= S; w++)
							g +=
								w == S && !L
									? f[w].marker(s, 1)
									: f[w].blank(w < S ? z(h.text, 4, f[w + 1].from) - g.length : null);
					let B = l;
...
//# sourceMappingURL=BCUIkOtC.js.map
