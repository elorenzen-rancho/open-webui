import { L as R, a as X, i as h, d as E, f as S, b as x, s as C, t as i } from './chV95BsE.js';
import { L as V, E as a, C as _ } from './COV9u0MG.js';
const U = C({
		'for in if else endfor endif': i.controlKeyword,
		'Attribute/Identifier': i.definition(i.propertyName),
		'ForIntro/Identifier': i.definition(i.propertyName),
		'Block/Identifier': i.definitionKeyword,
		'FunctionCall/Identifier': i.function(i.variableName),
		'Attribute/Expression': i.attributeValue,
		'VariableExpr/Identifier': i.variableName,
		'GetAttr/Identifier': i.variableName,
		Identifier: i.name,
		'LiteralValue/...': i.literal,
		'StringLit/... TemplateLiteral': i.string,
		NumericLit: i.number,
		BoolLit: i.bool,
		NullLit: i.null,
		Comment: i.comment,
		...
		ControlOp: i.controlOperator,
		HeredocIdentifier: i.special(i.controlKeyword),
		HeredocStart: i.special(i.brace),
		'TemplateInterpolationStart TemplateInterpolationEnd': i.special(i.brace),
		'TemplateDirectiveStart TemplateDirectiveEnd': i.special(i.brace),
		StripMarker: i.special(i.brace),
	...
	f = 7,
	Q = 93,
	r = { QUOTED_TEMPLATE: 1, TEMPLATE_INTERPOLATION: 2, TEMPLATE_DIRECTIVE: 4, HEREDOC_TEMPLATE: 8 },
	j = function (O) {
		var e = 0,
			t,
			o;
		if (O.length === 0) return e;
		for (t = 0; t < O.length; t++) ((o = O.charCodeAt(t)), (e = (e << 5) - e + o), (e |= 0));
		return e;
	};
class l {
	constructor(e, t, o) {
		((this.parent = e),
			(this.type = t),
			(this.heredoc_identifier = o),
			(this.hash =
				(e ? (e.hash + e.hash) << 8 : 0) +
				(t << 4) +
				(this.heredoc_identifier === '' ? 0 : j(this.heredoc_identifier))));
	}
}
const g = new l(null, '', ''),
	u = new _({
		start: g,
		shift(O, e, t, o) {
			if (e === d) return new l(O, r.QUOTED_TEMPLATE, '');
			if (e === T) return new l(O, r.TEMPLATE_INTERPOLATION, '');
			if (e === s) return new l(O, r.TEMPLATE_DIRECTIVE, '');
			if (e === f && O.type !== r.HEREDOC_TEMPLATE)
				return new l(O, r.HEREDOC_TEMPLATE, o.read(o.pos, t.pos));
			if (e === f && O.type === r.HEREDOC_TEMPLATE) return O.parent;
			switch (e) {
				case P:
				case m:
				case k:
	...
	Y = new a(
		(O, e) => {
			if (
				e.canShift(d) &&
				!(e.context && e.context.type === r.QUOTED_TEMPLATE) &&
				String.fromCharCode(O.next) === '"'
			)
				return (O.advance(), O.acceptToken(d));
		},
	...
	G = new a(
		(O, e) => {
			if (
				e.canShift(P) &&
				e.context &&
				e.context.type === r.QUOTED_TEMPLATE &&
				String.fromCharCode(O.next) === '"'
			)
				return (O.advance(), O.acceptToken(P));
		},
		{ contextual: !0, fallback: !0 }
	),
	b = new a(
		(O, e) => {
			if (e.canShift(Q) && e.context && e.context.type === r.QUOTED_TEMPLATE)
				switch (String.fromCharCode(O.next)) {
					case '\\':
						switch ((O.advance(), String.fromCharCode(O.next))) {
							case '"':
							case 'n':
							case 'r':
							case 't':
							case '\\':
								return (O.advance(), O.acceptToken(Q));
							case 'u':
								for (let t = 0; t < 4; t++)
									if ((O.advance(), !/[0-9a-fA-F]/.test(String.fromCharCode(O.next)))) return;
								return (O.advance(), O.acceptToken(Q));
							case 'U':
								for (let t = 0; t < 8; t++)
									if ((O.advance(), !/[0-9a-fA-F]/.test(String.fromCharCode(O.next)))) return;
			...
			if (
				e.canShift(Q) &&
				e.context &&
				(e.context.type === r.QUOTED_TEMPLATE || e.context.type === r.HEREDOC_TEMPLATE) &&
				O.next !== -1
			)
				return (O.advance(), O.acceptToken(Q));
		},
	...
	D = new a(
		(O, e) => {
			if (
				e.canShift(T) &&
				e.canShift(Q) &&
				!(e.context && e.context.type === r.TEMPLATE_INTERPOLATION) &&
				String.fromCharCode(O.next) === '$'
			)
				return (
					O.advance(),
					String.fromCharCode(O.next) === '{'
						? (O.advance(), O.acceptToken(T))
						: (String.fromCharCode(O.next) === '$' &&
								(O.advance(), String.fromCharCode(O.next) === '{') &&
								O.advance(),
							O.acceptToken(Q))
				);
		},
	...
	w = new a(
		(O, e) => {
			if (
				e.canShift(m) &&
				e.context &&
				e.context.type === r.TEMPLATE_INTERPOLATION &&
				String.fromCharCode(O.next) === '}'
			)
				return (O.advance(), O.acceptToken(m));
		},
		{ contextual: !0, fallback: !0 }
	),
	v = new a(
		(O, e) => {
			if (
				e.canShift(s) &&
				e.canShift(Q) &&
				!(e.context && e.context.type === r.TEMPLATE_DIRECTIVE) &&
				String.fromCharCode(O.next) === '%'
			)
				return (
					O.advance(),
					String.fromCharCode(O.next) === '{'
						? (O.advance(), O.acceptToken(s))
						: (String.fromCharCode(O.next) === '%' &&
								(O.advance(), String.fromCharCode(O.next) === '{') &&
								O.advance(),
							O.acceptToken(Q))
	...
	W = new a(
		(O, e) => {
			if (
				e.canShift(k) &&
				e.context &&
				e.context.type === r.TEMPLATE_DIRECTIVE &&
				String.fromCharCode(O.next) === '}'
			)
				return (O.advance(), O.acceptToken(k));
		},
		{ contextual: !0, fallback: !0 }
	),
	L = new a(
		(O, e) => {
			if (e.canShift(f) && !(e.context && e.context.type === r.HEREDOC_TEMPLATE) && O.next !== -1) {
				let t = String.fromCharCode(O.next);
				for (; /[0-9a-zA-Z]/.test(t) || t === '_' || t === '-'; )
					(O.advance(), (t = String.fromCharCode(O.next)));
				return O.acceptToken(f);
			}
			if (
				e.canShift(f) &&
				e.canShift(Q) &&
				e.context &&
				e.context.type === r.HEREDOC_TEMPLATE &&
				O.next != -1
			) {
				const t = e.context.heredoc_identifier;
				let o = !1,
					c = !1,
					$ = String.fromCharCode(O.next);
				for (; /\s/.test($); )
...
` && (o = !0),
						O.advance(),
						($ = String.fromCharCode(O.next)));
				if ((c && O.acceptToken(Q), !o)) return;
				let p = 0;
				for (let n = 0; n < t.length; n++)
					if (String.fromCharCode(O.next) === t[n]) (O.advance(), p++);
					else {
						p && O.acceptToken(Q);
						return;
					}
...
`
						...
						return O.acceptToken(f);
					if (!/\s/.test(String.fromCharCode(n))) return O.acceptToken(Q);
					y++;
				}
			}
		},
...
export { H as hcl, q as hclLanguage };
//# sourceMappingURL=BJeWbfiJ.js.map
