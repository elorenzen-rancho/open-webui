import { g as te } from './BPRsW9hX.js';
import { s as ee } from './Dc-KqKSo.js';
import {
	_ as f,
	l as b,
	c as F,
	...
	var t = f(function (V, l, h, n) {
		...
		gt = {
			...
			parse: f(function (l) {
				...
				function Zt(O) {
					((n.length = n.length - 2 * O), (_.length = _.length - O), (r.length = r.length - O));
				}
				f(Zt, 'popStack');
				function Lt() {
					var O;
					return (
						(O = g.pop() || k.lex() || tt),
						typeof O != 'number' &&
							(O instanceof Array && ((g = O), (O = g.pop())), (O = h.symbols_[O] || O)),
						O
					);
				}
				f(Lt, 'lex');
				for (var x, j, N, mt, J = {}, dt, Y, Rt, ft; ; ) {
					if (
						((j = n[n.length - 1]),
...
Expecting ` +
					...
					if (N[0] instanceof Array && N.length > 1)
						throw new Error(
							'Parse Error: multiple actions possible at state: ' + j + ', token: ' + x
						);
					switch (N[0]) {
						case 1:
							(n.push(x),
						...
						case 2:
							if (
								...
								return mt;
							(Y &&
								((n = n.slice(0, -1 * Y * 2)), (_ = _.slice(0, -1 * Y)), (r = r.slice(0, -1 * Y))),
								n.push(this.productions_[N[1]][0]),
								_.push(J.$),
								r.push(J._$),
								(Rt = Z[n[n.length - 2]][n[n.length - 1]]),
								n.push(Rt));
							break;
						case 3:
							return !0;
...
` +
				...
				next: f(function () {
					...
					for (var _ = this._currentRules(), r = 0; r < _.length; r++)
						if (
							((n = this._input.match(this.rules[_[r]])), n && (!h || n[0].length > h[0].length))
						) {
	...
	Ne = f(async function (t, e, s, a) {
		...
		try {
			(typeof a.db.getLinks == 'function' ? a.db.getLinks() : new Map()).forEach((L, v) => {
				var B;
				const R =
					typeof v == 'string' ? v : typeof (v == null ? void 0 : v.id) == 'string' ? v.id : '';
				if (!R) {
				...
				if (
					(w == null ||
						w.forEach((I) => {
							var P;
							((P = I.textContent) == null ? void 0 : P.trim()) === R && (C = I);
						}),
					!C)
				) {
					b.warn('⚠️ Could not find node matching text:', R);
					return;
				}
				const d = C.parentNode;
				if (!d) {
					b.warn('⚠️ Node has no parent, cannot wrap:', R);
					return;
				}
				...
				(d.replaceChild(D, C),
					D.appendChild(C),
					b.info('🔗 Wrapped node in <a> tag for:', R, L.url));
			});
		} catch (m) {
...
var st = f((t, e, s, a, i, o, u, p) => {
		...
		if ((b.info('dataFetcher parsedItem', e, T, m), S !== 'root')) {
			...
			(e.start === !0 ? (C = ye) : e.start === !1 && (C = ge),
				...
				St.get(S) ||
					St.set(S, {
						id: S,
						shape: C,
						description: H.sanitizeText(S, L),
						cssClasses: `${E} ${me}`,
						cssStyles: m
					}));
			const d = St.get(S);
			(e.description &&
				...
				((R = d.description) == null ? void 0 : R.length) === 1 &&
					d.shape === Dt &&
			...
			if (
				...
				t &&
					t.id !== 'root' &&
					(b.trace('Setting node ', S, ' to be child of its parent ', t.id), (D.parentId = t.id)),
				(D.centerLabel = !0),
				e.note)
			) {
				const $ = {
		...
		((z = class {
			...
			extract(e) {
				...
				const s = this.getStates(),
					a = F();
				($e(),
					st(void 0, this.getRootDocV2(), s, this.nodes, this.edges, !0, a.look, this.classes));
			...
			startIdIfNeeded(e = '') {
				return e === A.START_NODE
					? (this.startEndCount++, `${A.START_TYPE}${this.startEndCount}`)
					: e;
			}
			...
			addRelationObjs(e, s, a = '') {
				...
				(this.addState(i, o, e.doc, e.description, e.note, e.classes, e.styles, e.textStyles),
					...
					this.currentDocument.relations.push({
						id1: i,
						id2: u,
						relationTitle: H.sanitizeText(a, F())
					}));
			}
			addRelation(e, s, a) {
				...
				s &&
					...
					s.split(A.STYLECLASS_SEP).forEach((i) => {
						...
						if (RegExp(A.COLOR_KEYWORD).exec(i)) {
							const p = o
								.replace(A.FILL_KEYWORD, A.BG_FILL)
								.replace(A.COLOR_KEYWORD, A.FILL_KEYWORD);
							a.textStyles.push(p);
						}
						a.styles.push(o);
					});
