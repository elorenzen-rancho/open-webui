const e = 'Sobre',
	...
	qi = {
		'-1 for no limit, or a positive integer for a specific limit':
			'-1 para nenhum limite ou um inteiro positivo para um limite específico',
		"'s', 'm', 'h', 'd', 'w' or '-1' for no expiration.":
			"'s', 'm', 'h', 'd', 'w' ou '-1' para sem expiração.",
		'(e.g. `sh webui.sh --api --api-auth username_password`)':
			'(por exemplo, `sh webui.sh --api --api-auth username_password`)',
		'(e.g. `sh webui.sh --api`)': '(por exemplo, `sh webui.sh --api`)',
		'(latest)': '(mais recente)',
		'(leave blank for to use commercial endpoint)':
			'(deixe em branco para usar o endpoint comercial)',
		...
		'*Prompt node ID(s) are required for image generation':
			'*Prompt node ID(s) são obrigatórios para gerar imagens',
		...
		'A collaboration channel where people join as members':
			'Um canal de colaboração onde as pessoas se juntam como membros.',
		'A discussion channel where access is controlled by groups and permissions':
			'Um canal de discussão onde o acesso é controlado por grupos e permissões.',
		'A new version (v{{LATEST_VERSION}}) is now available.':
			'Uma nova versão (v{{LATEST_VERSION}}) está disponível.',
		'A private conversation between you and selected users':
			'Uma conversa privada entre você e usuários selecionados.',
		'A task model is used when performing tasks such as generating titles for chats and web search queries':
			'Um modelo de tarefa é usado ao realizar tarefas como gerar títulos para chats e consultas de pesquisa na web',
		'a user': 'um usuário',
		About: e,
...
Ir para a variável de prompt`,
		...
		'Activate this command by typing "/{{COMMAND}}" to chat input.':
			'Ativar esse comando no chat digitando "/{{COMMAND}}"',
		Active: i,
		...
		'Add a short description about what this model does':
			'Adicione uma descrição curta sobre o que este modelo faz',
		...
		'Add your Open Terminal URL and API key in Settings → Integrations.':
			'Adicione a URL do Open Terminal e a chave de API em Configurações → Integrações.',
		...
		'Adds filenames, titles, sections, and snippets into the BM25 text to improve lexical recall.':
			'Adiciona nomes de arquivos, títulos, seções e trechos ao texto BM25 para melhorar a recuperação lexical.',
		'Adjusting these settings will apply changes universally to all users.':
			'Ajustar essas configurações aplicará mudanças para todos os usuários.',
		admin: l,
		Admin: c,
		...
		'Admins have access to all tools at all times; users need tools assigned per model in the workspace.':
			'Os administradores têm acesso a todas as ferramentas o tempo todo; os usuários precisam de ferramentas atribuídas, por modelo, no workspace.',
		Advanced: m,
		'Advanced Parameters': 'Parâmetros Avançados',
		'Advanced parameters for MinerU parsing (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)':
			'Parâmetros avançados para análise do MinerU (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)',
		...
		'After updating or changing the embedding model, you must reindex the knowledge base for the changes to take effect. You can do this using the "Reindex" button below.':
			'Após atualizar ou alterar o modelo de embedding, você deve reindexar a base de conhecimento para que as alterações entrem em vigor. Você pode fazer isso usando o botão "Reindexar" abaixo.',
		AI: u,
		All: p,
		...
		'API Base URL for Datalab Marker service. Defaults to: https://www.datalab.to/api/v1/marker':
			'URL base da API para o serviço Datalab Marker. O padrão é: https://www.datalab.to/api/v1/marker',
		'API Key': 'Chave API',
		...
		'Are you sure you want to archive all chats? This action cannot be undone.':
			'Tem certeza de que deseja arquivar todos os chats? Esta ação não pode ser desfeita.',
		...
		'Are you sure you want to delete **{{modelName}}**?':
			'Tem certeza de que deseja excluir **{{modelName}}**?',
		'Are you sure you want to delete all chats? This action cannot be undone.':
			'Tem certeza de que deseja excluir todas as conversas? Esta ação não pode ser desfeita.',
		...
		"Are you sure you want to delete this version? Child versions will be relinked to this version's parent.":
			'Tem certeza de que deseja excluir esta versão? As versões filhas serão vinculadas novamente à versão pai.',
		...
		'Are you sure you want to unarchive all archived chats?':
			'Você tem certeza que deseja desarquivar todos os chats arquivados?',
		...
		'Autocomplete Generation Input Max Length':
			'Comprimento máximo de entrada de geração de preenchimento automático',
		Automatic1111: k,
		...
		'Automatically inject system tools in native function calling mode (e.g., timestamps, memory, chat history, notes, etc.)':
			'Injetar automaticamente ferramentas do sistema no modo de chamada de função nativa (por exemplo, carimbos de data/hora, memória, histórico de chat, notas, etc.)',
		Automation: R,
		...
		'Base Model List Cache speeds up access by fetching base models only at startup or on settings save—faster, but may not show recent base model changes.':
			'O cache da lista de modelos base acelera o acesso buscando modelos base somente na inicialização ou ao salvar as configurações — mais rápido, mas pode não mostrar alterações recentes no modelo base.',
		Bearer: z,
		before: W,
		...
		'Boosting or penalizing specific tokens for constrained responses. Bias values will be clamped between -100 and 100 (inclusive). (Default: none)':
			'Aumentar ou penalizar tokens específicos para respostas restritas. Os valores de viés serão fixados entre -100 e 100 (inclusive). (Padrão: nenhum)',
		Brave: K,
		'Brave Search API Key': 'Chave API do Brave Search',
		'Break down complex requests into trackable steps':
			'Divida solicitações complexas em etapas rastreáveis.',
		...
		'Call feature is not supported when using Web STT engine':
			'O recurso de chamada não é suportado ao usar o mecanismo Web STT',
		Camera: H,
		Cancel: Q,
		...
		'Channel name must be less than 128 characters':
			'O nome do canal deve ter menos de 128 caracteres.',
		...
		'Character limit for autocomplete generation input':
			'Limite de caracteres para entrada de geração de preenchimento automático',
		...
		'Chunks smaller than this threshold will be merged with neighboring chunks when possible. Set to 0 to disable merging.':
			'Os blocos menores que esse limite serão mesclados com os blocos vizinhos sempre que possível. Defina como 0 para desativar a mesclagem.',
		Ciphers: te,
		Citation: ie,
		Citations: ne,
		...
		'Click here to download user import template file.':
			'Clique aqui para baixar o arquivo de modelo de importação de usuários.',
		'Click here to learn more about faster-whisper and see the available models.':
			'Clique aqui para aprender mais sobre Whisper e ver os modelos disponíveis.',
		...
		'Click here to upload a workflow.json file.':
			'Clique aqui para enviar um arquivo workflow.json.',
		'click here.': 'clique aqui.',
		"Click on the user role button to change a user's role.":
			'Clique no botão de perfil do usuário para alterar o perfil de um usuário.',
		...
		'Clipboard write permission denied. Please check your browser settings to grant the necessary access.':
			'Permissão de escrita na área de transferência negada. Verifique as configurações do seu navegador para conceder o acesso necessário.',
		Clone: de,
		...
		'Collaboration channel where people join as members':
			'Canal de colaboração onde as pessoas se juntam como membros.',
		Collapse: me,
		Collection: ue,
		Collections: pe,
		Color: he,
		...
		'Comma separated Node Ids (e.g. 1 or 1,2)':
			'IDs de Nodes separados por vírgula (por exemplo, 1 ou 1,2)',
		command: ve,
		Command: fe,
		Comment: be,
		...
		'Connect to an AI provider to start chatting':
			'Conecte-se a um provedor de IA para começar a conversar',
		'Connect to Open Terminal instances to browse files and use them as always-on tools. Only one can be active at a time.':
			'Conecte-se a instâncias do Open Terminal para navegar em arquivos e usá-las como ferramentas sempre ativas. Apenas uma pode estar ativa por vez.',
		'Connect to Open Terminal instances. All users will have access to file browsing and terminal tools through these servers.':
			'Conecte-se a instâncias do Open Terminal. Todos os usuários terão acesso à navegação de arquivos e ferramentas de terminal por meio desses servidores.',
		'Connect to your own OpenAI compatible API endpoints.':
			'Conecte-se aos seus próprios endpoints de API compatíveis com OpenAI.',
		'Connect to your own OpenAPI compatible external tool servers.':
			'Conecte-se aos seus próprios servidores de ferramentas externas compatíveis com OpenAPI.',
		...
		'Constrains effort on reasoning for reasoning models. Only applicable to reasoning models from specific providers that support reasoning effort.':
			'Restringe o esforço de raciocínio para modelos de raciocínio. Aplicável somente a modelos de raciocínio de provedores específicos que suportam o esforço de raciocínio.',
		'Contact Admin for WebUI Access': 'Contate o Administrador para verificar seu acesso',
		Content: Ie,
		...
		"Control how message text is split for TTS requests. 'Punctuation' splits into sentences, 'paragraphs' splits into paragraphs, and 'none' keeps the message as a single string.":
			"Controla como o texto da mensagem é dividido para solicitações TTS. 'Pontuação' divide em frases, 'Parágrafos' divide em parágrafos e 'Nenhum' mantém a mensagem como um texto único.",
		'Control the repetition of token sequences in the generated text. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 1.1) will be more lenient. At 1, it is disabled.':
			'Controla a repetição de sequências de tokens no texto gerado. Um valor mais alto (por exemplo, 1,5) penalizará as repetições com mais rigor, enquanto um valor mais baixo (por exemplo, 1,1) será mais tolerante. Em 1, ele está desabilitado.',
		Controls: Ee,
		'Controls the balance between coherence and diversity of the output. A lower value will result in more focused and coherent text.':
			'Controla o equilíbrio entre coerência e diversidade da saída. Um valor menor resultará em um texto mais focado e coerente.',
		...
		'Copied shared chat URL to clipboard!':
			'URL do chat compartilhado copiada para a área de transferência!',
		...
		'CORS must be properly configured by the provider to allow requests from Open WebUI.':
			'O CORS deve ser configurado corretamente pelo provedor para permitir solicitações do Open WebUI.',
		...
		'Create scheduled prompts that run automatically on a recurring basis.':
			'Crie prompts agendados que sejam executados automaticamente de forma recorrente.',
		'Create your first note by clicking on the plus button below.':
			'Crie sua primeira nota clicando no botão de adição abaixo.',
		...
		"Default mode works with a wider range of models by calling tools once before execution. Native mode leverages the model's built-in tool-calling capabilities, but requires the model to inherently support this feature.":
			'O modo padrão funciona com uma gama mais ampla de modelos, chamando as ferramentas uma vez antes da execução. O modo nativo aproveita os recursos integrados de chamada de ferramentas do modelo, mas exige que o modelo suporte esse recurso inerentemente.',
		...
		'Default to segmented retrieval for focused and relevant content extraction, this is recommended for most cases.':
			'Use a recuperação segmentada como padrão para extração de conteúdo focado e relevante; isso é recomendado para a maioria dos casos.',
		...
		'Deployment names are required for Azure OpenAI':
			'Nomes de implantação são necessários para o Azure OpenAI',
		Desc: _e,
		...
		'Direct Connections allow users to connect to their own OpenAI compatible API endpoints.':
			'As conexões diretas permitem que os usuários se conectem aos seus próprios endpoints de API compatíveis com OpenAI.',
		...
		'Disable image extraction from the PDF. If Use LLM is enabled, images will be automatically captioned. Defaults to False.':
			'Desabilite a extração de imagens do PDF. Se a opção Usar LLM estiver habilitada, as imagens serão legendadas automaticamente. O padrão é Falso.',
		Disabled: Ye,
		'Disconnect OAuth': '',
		...
		'Discover how to use Open WebUI and seek support from the community.':
			'Descubra como usar o Open WebUI e busque suporte da comunidade.',
		'Discover wonders': 'Descobrir maravilhas',
		'Discover, download, and explore custom functions':
			'Descubra, baixe e explore funções personalizadas',
		...
		'Discover, download, and explore custom tools':
			'Descubra, baixe e explore ferramentas personalizadas',
		'Discover, download, and explore model presets':
			'Descubra, baixe e explore predefinições de modelos',
		'Discussion channel where access is based on groups and permissions':
			'Canal de discussão onde o acesso é baseado em grupos e permissões.',
		Display: Je,
		...
		'Display the username instead of You in the Chat':
			'Exibir o nome de usuário em vez de Você no Chat',
		...
		'Displays status updates (e.g., web search progress) in the response':
			'Exibe atualizações de status (por exemplo, progresso da pesquisa na web) na resposta',
		'Dive into knowledge': 'Explorar base de conhecimento',
		'Do not install functions from sources you do not fully trust.':
			'Não instale funções de origens nas quais você não confia totalmente.',
		'Do not install tools from sources you do not fully trust.':
			'Não instale ferramentas de origens nas quais você não confia totalmente.',
		'Do you want to sync your usage stats with Open WebUI Community?':
			'Deseja sincronizar suas estatísticas de uso com a Comunidade Open WebUI?',
		Docling: He,
		...
		'does not make any external connections, and your data stays securely on your locally hosted server.':
			'não faz nenhuma conexão externa, e seus dados permanecem seguros no seu servidor local.',
		'Domain Filter List': 'Lista de Filtros de Domínio',
		"don't fetch random pipelines from sources you don't trust.":
			'Não busque pipelines aleatórios de fontes não confiáveis.',
		"Don't have an account?": 'Não tem uma conta?',
		"don't install random functions from sources you don't trust.":
			'não instale funções aleatórias de origens nas quais você não confia.',
		"don't install random tools from sources you don't trust.":
			'não instale ferramentas aleatórias de origens nas quais você não confia.',
		...
		"e.g. '30s','10m'. Valid time units are 's', 'm', 'h'.":
			"por exemplo, '30s', '10m'. Unidades de tempo válidas são 's', 'm', 'h'.",
		...
		'e.g. A filter to remove profanity from text':
			'Exemplo: Um filtro para remover palavrões do texto',
		...
		'e.g. Step-by-step instructions for code reviews':
			'Por exemplo, instruções passo a passo para revisões de código.',
		'e.g. Tell me a fun fact': 'Por exemplo: Conte-me uma curiosidade.',
		'e.g. Tell me a fun fact about the Roman Empire':
			'Por exemplo: Conte-me uma curiosidade sobre o Império Romano.',
		'e.g. Tools for performing various operations':
			'Exemplo: Ferramentas para executar operações diversas',
		'e.g., 3, 4, 5 (leave blank for default)':
			'por exemplo, 3, 4, 5 (deixe em branco para o padrão)',
		'e.g., audio/wav,audio/mpeg,video/* (leave blank for defaults)':
			'por exemplo, audio/wav, audio/mpeg, video/* (deixe em branco para os padrões)',
		'e.g., en-US,ja-JP (leave blank for auto-detect)':
			'por exemplo, en-US, ja-JP (deixe em branco para detecção automática)',
		...
		'Enable autocomplete generation for chat messages':
			'Habilitar geração de preenchimento automático para mensagens do chat',
		...
		'Enable, disable, or customize the reasoning tags used by the model. "Enabled" uses default tags, "Disabled" turns off reasoning tags, and "Custom" lets you specify your own start and end tags.':
			'Habilite, desabilite ou personalize as tags de raciocínio usadas pelo modelo. "Ativado" usa tags padrão, "Desativado" desativa as tags de raciocínio e "Personalizado" permite que você especifique suas próprias tags de início e fim.',
		Enabled: po,
		'End Tag': 'Tag final',
		...
		'Ensure your CSV file includes 4 columns in this order: Name, Email, Password, Role.':
			'Certifique-se de que seu arquivo CSV inclua 4 colunas nesta ordem: Nome, Email, Senha, Função.',
		'Enter {{role}} message here': 'Digite a mensagem de {{role}} aqui',
		'Enter a detail about yourself for your LLMs to recall':
			'Digite um detalhe sobre você para seus LLMs lembrarem',
		'Enter a title for the pending user info overlay. Leave empty for default.':
			'Insira um título para a sobreposição de informações pendentes do usuário. Deixe em branco como padrão.',
		'Enter a watermark for the response. Leave empty for none.':
			"Insira uma marca d'água para a resposta. Deixe em branco se não houver nenhuma.",
		...
		'Enter api auth string (e.g. username:password)':
			'Digite a string de autenticação da API (por exemplo, username:password)',
		...
		'Enter content for the pending user info overlay. Leave empty for default.':
			'Insira o conteúdo para a sobreposição de informações pendentes do usuário. Deixe em branco para o padrão.',
		...
		'Enter domains separated by commas (e.g., example.com,site.org,!excludedsite.com)':
			'Insira os domínios separados por vírgulas (ex.: example.com,site.org,!excludedsite.com)',
		'Enter Exa API Key': 'Insira a chave da API Exa',
		'Enter External Document Loader API Key':
			'Insira a chave da API do carregador de documentos externo',
		...
		'Enter function name filter list (e.g. func1, !func2)':
			'Insira a lista de filtros de nomes de funções (por exemplo, func1, !func2)',
		...
		'Enter JSON config (e.g., {"disable_links": true})':
			'Insira a configuração JSON (por exemplo, {"disable_links": true})',
		...
		'Enter proxy URL (e.g. https://user:password@host:port)':
			'Insira a URL do proxy (por exemplo, https://usuário:senha@host:porta)',
		...
		'Enter the prompt instructions for this automation...':
			'Insira as instruções para esta automação...',
		'Enter the public URL of your WebUI. This URL will be used to generate links in the notifications.':
			'Insira a URL pública da sua WebUI. Esta URL será usada para gerar links nas notificações.',
		...
		'Enter Yacy URL (e.g. http://yacy.example.com:8090)':
			'Digite a URL do Yacy (por exemplo, http://yacy.example.com:8090)',
		...
		"Error: A model with the ID '{{modelId}}' already exists. Please select a different ID to proceed.":
			"Erro: Já existe um modelo com o ID '{{modelId}}'. Selecione um ID diferente para prosseguir.",
		'Error: Model ID cannot be empty. Please enter a valid ID to proceed.':
			'Erro: O ID do modelo não pode estar vazio. Insira um ID válido para prosseguir.',
		Evaluations: fo,
		...
		'Example: (&(objectClass=inetOrgPerson)(uid=%s))':
			'Exemplo: (&(objectClass=inetOrgPerson)(uid=%s))',
		...
		'Example: sAMAccountName or uid or userPrincipalName':
			'Exemplo: sAMAccountName ou uid ou userPrincipalName',
		'Exceeded the number of seats in your license. Please contact support to increase the number of seats.':
			'Excedeu o número de licenças disponíveis. Entre em contato com o suporte para aumentar o número de licenças.',
		Exclude: bo,
		...
		'Failed to connect to {{URL}} OpenAPI tool server':
			'Falha ao conectar ao servidor da ferramenta OpenAPI {{URL}}',
		'Failed to connect to {{URL}} terminal server':
			'Falha ao conectar ao servidor de terminal {{URL}}',
		...
		'Failed to extract content from the file: {{error}}':
			'Falha ao extrair conteúdo do arquivo: {{error}}',
		...
		'Failed to load DOCX file. Please try downloading it instead.':
			'Não foi possível carregar o arquivo DOCX. Tente baixá-lo em vez disso.',
		'Failed to load Excel/CSV file. Please try downloading it instead.':
			'Não foi possível carregar o arquivo Excel/CSV. Tente baixá-lo em vez disso.',
		...
		'Failed to load PPTX file. Please try downloading it instead.':
			'Não foi possível carregar o arquivo PPTX. Tente baixá-lo em vez disso.',
		...
		'Fingerprint spoofing detected: Unable to use initials as avatar. Defaulting to default profile image.':
			'Falsificação de impressão digital detectada: Não foi possível usar as iniciais como avatar. Usando a imagem de perfil padrão.',
		...
		'Force OCR on all pages of the PDF. This can lead to worse results if you have good text in your PDFs. Defaults to False.':
			'Forçar OCR em todas as páginas do PDF. Isso pode levar a resultados piores se você tiver texto de boa qualidade nos seus PDFs. O padrão é Falso.',
		...
		'Format the lines in the output. Defaults to False. If set to True, the lines will be formatted to detect inline math and styles.':
			'Formata as linhas na saída. O padrão é Falso. Se definido como Verdadeiro, as linhas serão formatadas para detectar matemática e estilos embutidos.',
		'Formatting may be inconsistent from source.':
			'A formatação pode ser inconsistente em relação à fonte.',
		Forward: Fo,
		'Forwards system user OAuth access token to authenticate':
			'Encaminha o token de acesso OAuth do usuário do sistema para autenticação',
		'Forwards system user session credentials to authenticate':
			'Encaminha as credenciais da sessão do usuário do sistema para autenticação',
		Fr_day_of_week: zo,
		...
		'Functions allow arbitrary code execution.':
			'Funções permitem a execução arbitrária de código.',
		...
		'Get current time and perform date/time calculations':
			'Obtenha a hora atual e realize cálculos de data/hora.',
		...
		'Hex Color - Leave empty for default color':
			'Cor Hexadecimal - Deixe em branco para a cor padrão',
		Hidden: ra,
		Hide: ta,
		...
		'I acknowledge that I have read and I understand the implications of my action. I am aware of the risks associated with executing arbitrary code and I have verified the trustworthiness of the source.':
			'Eu reconheço que li e entendi as implicações da minha ação. Estou ciente dos riscos associados à execução de código arbitrário e verifiquei a confiabilidade da fonte.',
		ID: ua,
		...
		'Include `--api-auth` flag when running stable-diffusion-webui':
			'Incluir a flag `--api-auth` ao executar stable-diffusion-webui',
		'Include `--api` flag when running stable-diffusion-webui':
			'Incluir a flag `--api` ao executar stable-diffusion-webui',
		...
		'Influences how quickly the algorithm responds to feedback from the generated text. A lower learning rate will result in slower adjustments, while a higher learning rate will make the algorithm more responsive.':
			'Influencia a rapidez com que o algoritmo responde ao feedback do texto gerado. Uma taxa de aprendizado menor resultará em ajustes mais lentos, enquanto uma taxa de aprendizado maior tornará o algoritmo mais responsivo.',
		Info: ba,
		Initials: Aa,
		'Inject file content into conversation context':
			'Inserir conteúdo de arquivo no contexto da conversa',
		'Inject the entire content as context for comprehensive processing, this is recommended for complex queries.':
			'Injete todo o conteúdo como contexto para processamento abrangente; isso é recomendado para consultas complexas.',
		Input: Ca,
		'Input Key (e.g. text, unet_name, steps)':
			'Chave de entrada (por exemplo, texto, unet_name, etapas)',
		...
		'Instant Auto-Send After Voice Transcription':
			'Envio Automático Instantâneo Após Transcrição de Voz',
		Instructions: Sa,
		Integration: Pa,
		Integrations: xa,
		Interface: Ia,
		...
		'Invalid JSON format for ComfyUI Edit Workflow.':
			'Formato JSON inválido para o fluxo de trabalho de edição do ComfyUI.',
		'Invalid JSON format for ComfyUI Workflow.':
			'Formato JSON inválido para o fluxo de trabalho do ComfyUI.',
		...
		'Leave empty to include all models from "{{url}}" endpoint':
			'Deixe em branco para incluir todos os modelos do endpoint "{{url}}"',
		'Leave empty to include all models from "{{url}}/api/tags" endpoint':
			'Deixe em branco para incluir todos os modelos do endpoint "{{url}}/api/tags"',
		'Leave empty to include all models from "{{url}}/models" endpoint':
			'Deixe em branco para incluir todos os modelos do endpoint "{{url}}/models"',
		'Leave empty to include all models or select specific models':
			'Deixe vazio para incluir todos os modelos ou selecione modelos específicos',
		'Leave empty to use first admin user':
			'Deixe em branco para usar o primeiro usuário administrador.',
		'Leave empty to use the default config, or enter a valid json (see https://yandex.cloud/en/docs/search-api/api-ref/WebSearch/search#yandex.cloud.searchapi.v2.WebSearchRequest)':
			'Deixe em branco para usar a configuração padrão ou insira um JSON válido (consulte https://yandex.cloud/en/docs/search-api/api-ref/WebSearch/search#yandex.cloud.searchapi.v2.WebSearchRequest)',
		'Leave empty to use the default model (voxtral-mini-latest).':
			'Deixe em branco para usar o modelo padrão (voxtral-mini-latest).',
		'Leave empty to use the default prompt, or enter a custom prompt':
			'Deixe vazio para usar o prompt padrão, ou insira um prompt personalizado',
		'Leave model field empty to use the default model.':
			'Deixe o campo do modelo vazio para usar o modelo padrão.',
		Legacy: qa,
		lexical: La,
		License: Fa,
		...
		'Limit concurrent search queries. 0 = unlimited (default). Set to 1 for sequential execution (recommended for APIs with strict rate limits like Brave free tier).':
			'Limitar consultas de pesquisa simultâneas. 0 = ilimitado (padrão). Defina como 1 para execução sequencial (recomendado para APIs com limites de taxa rígidos, como o nível gratuito do Brave).',
		'Limits the number of concurrent embedding requests. Set to 0 for unlimited.':
			'Limita o número de solicitações simultâneas de embedding. Defina como 0 para ilimitado.',
		List: Wa,
		'List calendars, search, create, update, and delete calendar events':
			'Listar calendários, pesquisar, criar, atualizar e excluir eventos do calendário',
		'Listening...': 'Escutando...',
		Live: Ga,
		'Llama.cpp': 'Llama.cpp',
		'LLMs can make mistakes. Verify important information.':
			'LLMs podem cometer erros. Verifique informações importantes.',
		Loader: _a,
		...
		'Make sure to export a workflow.json file as API format from ComfyUI.':
			'Certifique-se de exportar um arquivo workflow.json como o formato API do ComfyUI.',
		Male: Ha,
		Manage: Qa,
		...
		'Maximum characters to return from fetched URLs. Leave empty for no limit.':
			'Número máximo de caracteres a retornar de URLs buscadas. Deixe em branco para sem limite.',
		'Maximum number of files allowed per folder.':
			'Número máximo de arquivos permitidos por pasta.',
		'Maximum number of files per folder is {{max}}.':
			'O número máximo de arquivos por pasta é {{max}}.',
		'Maximum of 3 models can be downloaded simultaneously. Please try again later.':
			'Máximo de 3 modelos podem ser baixados simultaneamente. Por favor, tente novamente mais tarde.',
		May: $a,
		MBR: es,
		MCP: os,
		'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the Open WebUI team, making it the more reliable option for compatibility.':
			'O suporte ao MCP é experimental e suas especificações mudam com frequência, o que pode levar a incompatibilidades. O suporte à especificação OpenAPI é mantido diretamente pela equipe do Open WebUI, tornando-o a opção mais confiável para compatibilidade.',
		Medium: as,
		...
		'Memories accessible by LLMs will be shown here.':
			'Memórias acessíveis por LLMs serão mostradas aqui.',
		Memory: is,
		...
		'Message counts and response timestamps':
			'Contagem de mensagens e registros de data e hora de resposta',
		'Message counts are based on assistant responses.':
			'A contagem de mensagens é baseada nas respostas do assistente.',
		'Message rating should be enabled to use this feature':
			'A avaliação de mensagens deve estar habilitada para usar este recurso',
		messages: ds,
		Messages: ls,
		"Messages you send after creating your link won't be shared. Users with the URL will be able to view the shared chat.":
			'Mensagens enviadas após criar seu link não serão compartilhadas. Usuários com o URL poderão visualizar o chat compartilhado.',
		...
		'MinerU API Key required for Cloud API mode.':
			'Chave de API MinerU necessária para o modo Cloud API.',
		...
		"Model '{{modelName}}' has been successfully downloaded.":
			"Modelo '{{modelName}}' foi baixado com sucesso.",
		"Model '{{modelTag}}' is already in queue for downloading.":
			"Modelo '{{modelTag}}' já está na fila para download.",
		...
		'Model can access Open Terminal for command execution and file management':
			'O modelo pode acessar o Open Terminal para execução de comandos e gerenciamento de arquivos',
		'Model can execute code and perform calculations':
			'O modelo pode executar código e realizar cálculos',
		'Model can generate images based on text prompts':
			'O modelo pode gerar imagens com base em prompts de texto',
		...
		'Model filesystem path detected. Model shortname is required for update, cannot continue.':
			'Caminho do sistema de arquivos do modelo detectado. Nome curto do modelo é necessário para atualização, não é possível continuar.',
		...
		'Model name already exists, please choose a different one':
			'O nome do modelo já existe, escolha um diferente',
		...
		'Name and ID are required, please fill them out':
			'Nome e ID são obrigatórios, por favor preencha-os',
		...
		'No access grants. Private to you.':
			'Sem permissões de acesso. Privacidade exclusiva para você.',
		...
		'No expiration can pose security risks.':
			'A ausência de expiração pode representar riscos de segurança.',
		...
		'No files yet. Upload files or run Python code to create them.':
			'Nenhum arquivo ainda. Envie arquivos ou execute código Python para criá-los.',
		...
		'No HTML, CSS, or JavaScript content found.':
			'Nenhum conteúdo HTML, CSS ou JavaScript encontrado.',
		'No inference engine with management support found':
			'Nenhum mecanismo de inferência com suporte de gerenciamento encontrado',
		...
		'No tool server connections configured.':
			'Nenhuma conexão de servidor de ferramentas configurada.',
		...
		'Note: If you set a minimum score, the search will only return documents with a score greater than or equal to the minimum score.':
			'Nota: Se você definir uma pontuação mínima, a pesquisa retornará apenas documentos com pontuação igual ou superior à pontuação mínima.',
		Notes: ws,
		...
		'Only active when "Paste Large Text as File" setting is toggled on.':
			'Ativo somente quando a configuração "Colar texto grande como arquivo" estiver ativada.',
		'Only active when the chat input is in focus and an LLM is generating a response.':
			'Ativo somente quando o campo de entrada do chat está em foco e um LLM está gerando uma resposta.',
		'Only active when the chat input is in focus.':
			'Ativo somente quando o campo de entrada do chat estiver em foco.',
		...
		'Only alphanumeric characters and hyphens are allowed in the command string.':
			'Apenas caracteres alfanuméricos e hífens são permitidos na string de comando.',
		'Only can be triggered when the chat input is in focus.':
			'Só pode ser acionado quando o campo de entrada do chat estiver em foco.',
		'Only collections can be edited, create a new knowledge base to edit/add documents.':
			'Somente coleções podem ser editadas. Crie uma nova base de conhecimento para editar/adicionar documentos.',
		...
		'Only select users and groups with permission can access':
			'Somente usuários e grupos selecionados com permissão podem acessar.',
		'Only sync new/updated chats': 'Sincronizar apenas conversas novas/atualizadas',
		'Oops! Looks like the URL is invalid. Please double-check and try again.':
			'Ops! Parece que a URL é inválida. Por favor, verifique novamente e tente de novo.',
		'Oops! There are files still uploading. Please wait for the upload to complete.':
			'Ops! Ainda há arquivos sendo enviados. Por favor, aguarde a conclusão do envio.',
		'Oops! There was an error in the previous response.':
			'Ops! Houve um erro na resposta anterior.',
		...
		'Open Modal To Manage Floating Quick Actions':
			'Abra o Modal para gerenciar ações rápidas flutuantes',
		'Open Modal To Manage Image Compression':
			'Abrir o Modal para gerenciar a compressão de imagens',
		...
		'Open WebUI can use tools provided by any OpenAPI server.':
			'O Open WebUI pode usar ferramentas fornecidas por qualquer servidor OpenAPI.',
		...
		'Open WebUI uses SpeechT5 and CMU Arctic speaker embeddings.':
			'A Open WebUI usa os embeddings de voz do SpeechT5 e do CMU Arctic.',
		'Open WebUI version': 'Versão do Open WebUI',
		'Open WebUI version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})':
			'A versão do Open WebUI (v{{OPEN_WEBUI_VERSION}}) é inferior à versão necessária (v{{REQUIRED_VERSION}})',
		OpenAI: Ls,
		...
		'Page mode creates one document per page. Single mode combines all pages into one document for better chunking across page boundaries.':
			'O modo de página cria um documento por página. O modo único combina todas as páginas em um único documento para melhor divisão entre páginas.',
		Paginate: Js,
		Parameters: Hs,
		...
		'Participate in community leaderboards and evaluations! Syncing aggregated usage stats helps drive research and improvements to Open WebUI. Your privacy is paramount: no message content is ever shared.':
			'Participe dos rankings e avaliações da comunidade! A sincronização de estatísticas de uso agregadas ajuda a impulsionar pesquisas e melhorias no Open WebUI. Sua privacidade é fundamental: nenhum conteúdo de mensagem é compartilhado.',
		Password: Qs,
		...
		'Permission denied when accessing media devices':
			'Permissão negada ao acessar dispositivos de mídia',
		'Permission denied when accessing microphone': 'Permissão negada ao acessar o microfone',
		'Permission denied when accessing microphone: {{error}}':
			'Permissão negada ao acessar o microfone: {{error}}',
		Permissions: er,
		...
		'Please carefully review the following warnings:':
			'Por favor, revise cuidadosamente os seguintes avisos:',
		'Please connect all required integrations before sending a message':
			'Conecte todas as integrações necessárias antes de enviar uma mensagem',
		'Please do not close the settings page while loading the model.':
			'Não feche a página de configurações enquanto estiver carregando o modelo.',
		'Please enter a message or attach a file.':
			'Por favor, insira uma mensagem ou anexe um arquivo.',
		...
		'Please save the connection to persist the OAuth client information and do not change the ID':
			'Salve a conexão para persistir as informações do cliente OAuth e não altere o ID',
		...
		'Please select at least one user for Direct Message channel.':
			'Por favor, selecione pelo menos um usuário para o canal de Mensagens Diretas.',
		'Please wait until all files are uploaded.':
			'Aguarde até que todos os arquivos sejam enviados.',
		...
		'Prefix ID is used to avoid conflicts with other connections by adding a prefix to the model IDs - leave empty to disable':
			'O ID de prefixo é utilizado para evitar conflitos com outras conexões, adicionando um prefixo aos IDs dos modelos - deixe em branco para desativar.',
		...
		'Reduces the probability of generating nonsense. A higher value (e.g. 100) will give more diverse answers, while a lower value (e.g. 10) will be more conservative.':
			'Reduz a probabilidade de gerar respostas sem sentido. Um valor mais alto (por exemplo, 100) resultará em respostas mais diversas, enquanto um valor mais baixo (por exemplo, 10) será mais conservador.',
		'Refer to yourself as "User" (e.g., "User is learning Spanish")':
			'Refira-se como "Usuário" (por exemplo, "Usuário está aprendendo espanhol")',
		...
		'Response notifications cannot be activated as the website permissions have been denied. Please visit your browser settings to grant the necessary access.':
			'Notificações de resposta não podem ser ativadas pois as permissões do site foram negadas. Por favor, visite as configurações do seu navegador para conceder o acesso necessário.',
		...
		'Runs embedding tasks concurrently to speed up processing. Turn off if rate limits become an issue.':
			'Executa tarefas de incorporação simultaneamente para acelerar o processamento. Desative se os limites de taxa se tornarem um problema.',
		Sa_day_of_week: Kr,
		Save: jr,
		...
		"Saving chat logs directly to your browser's storage is no longer supported. Please take a moment to download and delete your chat logs by clicking the button below. Don't worry, you can easily re-import your chat logs to the backend through":
			'Salvar registros de chat diretamente no armazenamento do seu navegador não é mais suportado. Por favor, reserve um momento para baixar e excluir seus registros de chat clicando no botão abaixo. Não se preocupe, você pode facilmente reimportar seus registros de chat para o backend através de',
		Schedule: Jr,
		...
		'Searxng search language (all, en, es, de, fr, etc.)':
			'Idioma de pesquisa do Searxng (all, en, es, de, fr, etc.)',
		...
		'Select a base model (e.g. llama3, gpt-4o)':
			'Selecione um modelo base (por exemplo, llama3, gpt-4o)',
		...
		'Select how to split message text for TTS requests':
			'Selecione como dividir o texto da mensagem para solicitações TTS',
		...
		'Selected model(s) do not support image inputs':
			'Modelo(s) selecionado(s) não suportam entradas de imagem',
		...
		'Sends `stream_options: { include_usage: true }` in the request.\nSupported providers will return token usage information in the response when set.':
			'Envia `stream_options: { include_usage: true }` na solicitação. Provedores compatíveis retornarão informações sobre o uso de tokens na resposta quando configurado.',
		September: ot,
		...
		'Set reranking model (e.g. {{model}})':
			'Definir modelo de reclassificação (por exemplo, {{model}})',
		'Set the default models that are automatically selected for all users when a new chat is created.':
			'Defina os modelos padrão que são automaticamente selecionados para todos os usuários quando um novo chat é criado.',
		'Set the models that are automatically pinned to the sidebar for all users.':
			'Defina os modelos que são automaticamente fixados na barra lateral para todos os usuários.',
		'Set the number of layers, which will be off-loaded to GPU. Increasing this value can significantly improve performance for models that are optimized for GPU acceleration but may also consume more power and GPU resources.':
			'Defina o número de camadas que serão transferidas para a GPU. Aumentar esse valor pode melhorar significativamente o desempenho de modelos otimizados para aceleração de GPU, mas também pode consumir mais energia e recursos da GPU.',
		...
		'Sets a flat bias against tokens that have appeared at least once. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'Define um viés fixo contra tokens que apareceram pelo menos uma vez. Um valor mais alto (por exemplo, 1,5) penalizará as repetições com mais força, enquanto um valor mais baixo (por exemplo, 0,9) será mais tolerante. Em 0, está desabilitado.',
		'Sets a scaling bias against tokens to penalize repetitions, based on how many times they have appeared. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'Define um viés de escala contra tokens para penalizar repetições, com base em quantas vezes elas apareceram. Um valor mais alto (por exemplo, 1,5) penalizará as repetições com mais rigor, enquanto um valor mais baixo (por exemplo, 0,9) será mais brando. Em 0, está desabilitado.',
		'Sets how far back for the model to look back to prevent repetition.':
			'Define até que ponto o modelo deve olhar para trás para evitar repetições.',
		'Sets the random number seed to use for generation. Setting this to a specific number will make the model generate the same text for the same prompt.':
			'Define a semente numérica aleatória a ser usada para geração. Definir um número específico fará com que o modelo gere o mesmo texto para o mesmo prompt.',
		'Sets the size of the context window used to generate the next token.':
			'Define o tamanho da janela de contexto usada para gerar o próximo token.',
		'Sets the stop sequences to use. When this pattern is encountered, the LLM will stop generating text and return. Multiple stop patterns may be set by specifying multiple separate stop parameters in a modelfile.':
			'Define as sequências de parada a serem usadas. Quando esse padrão for encontrado, o modelo de linguagem (LLM) parará de gerar texto e retornará. Vários padrões de parada podem ser definidos especificando parâmetros de parada separados em um arquivo de modelo.',
		Setting: st,
		Settings: rt,
		...
		'Show Admin Details in Account Pending Overlay':
			'Mostrar Detalhes do Administrador na Sobreposição de Conta Pendentes',
		...
		'Showing all messages (user + assistant) per user.':
			'Exibindo todas as mensagens (usuário + assistente) por usuário.',
		'Sign in': 'Entrar',
		...
		'Significantly improves accuracy by using an LLM to enhance tables, forms, inline math, and layout detection. Will increase latency. Defaults to False.':
			'Melhora significativamente a precisão usando um LLM para aprimorar tabelas, formulários, cálculos em linha e detecção de layout. Aumenta a latência. O padrão é Falso.',
		...
		'Skip the cache and re-run the inference. Defaults to False.':
			'Ignore o cache e execute a inferência novamente. O padrão é Falso.',
		...
		'Split documents by markdown headers before applying character/token splitting.':
			'Divida os documentos por cabeçalhos Markdown antes de aplicar a divisão de caracteres/tokens.',
		...
		'Strip existing OCR text from the PDF and re-run OCR. Ignored if Force OCR is enabled. Defaults to False.':
			'Remove o texto OCR existente do PDF e executa o OCR novamente. Ignorado se a opção Forçar OCR estiver habilitada. O padrão é Falso.',
		...
		'Syncs only chats with updates after your last sync timestamp. Disable to re-sync all chats.':
			'Sincroniza apenas as conversas com atualizações posteriores à sua última sincronização. Desative para sincronizar todas as conversas novamente.',
		System: xt,
		...
		'Tail free sampling is used to reduce the impact of less probable tokens from the output. A higher value (e.g., 2.0) will reduce the impact more, while a value of 1.0 disables this setting.':
			'A amostragem livre de cauda é usada para reduzir o impacto de tokens menos prováveis na saída. Um valor mais alto (por exemplo, 2,0) reduzirá ainda mais o impacto, enquanto um valor de 1,0 desabilita essa configuração.',
		...
		'The Application Account DN you bind with for search':
			'O DN (Distinguished Name) da Conta de Aplicação com a qual você se conecta para pesquisa.',
		...
		'The batch size determines how many text requests are processed together at once. A higher batch size can increase the performance and speed of the model, but it also requires more memory.':
			'O tamanho do lote determina quantas solicitações de texto são processadas simultaneamente. Um tamanho de lote maior pode aumentar o desempenho e a velocidade do modelo, mas também requer mais memória.',
		'The developers behind this plugin are passionate volunteers from the community. If you find this plugin helpful, please consider contributing to its development.':
			'Os desenvolvedores por trás deste plugin são voluntários apaixonados da comunidade. Se você achar este plugin útil, considere contribuir para o seu desenvolvimento.',
		'The evaluation leaderboard is based on the Elo rating system and is updated in real-time.':
			'A tabela de classificação de avaliação é baseada no sistema Elo e é atualizada em tempo real.',
		'The format to return a response in. Format can be json or a JSON schema.':
			'O formato para retornar uma resposta. O formato pode ser json ou um esquema JSON.',
		'The height in pixels to compress images to. Leave empty for no compression.':
			'Altura em pixels para compactar as imagens. Deixe em branco para não compactar.',
		'The language of the input audio. Supplying the input language in ISO-639-1 (e.g. en) format will improve accuracy and latency. Leave blank to automatically detect the language.':
			'O idioma do áudio de entrada. Fornecer o idioma de entrada no formato ISO-639-1 (por exemplo, en) aumentará a precisão e a latência. Deixe em branco para detectar o idioma automaticamente.',
		'The LDAP attribute that maps to the mail that users use to sign in.':
			'O atributo LDAP que mapeia o e-mail que os usuários usam para fazer login.',
		'The LDAP attribute that maps to the username that users use to sign in.':
			'O atributo LDAP que mapeia para o nome de usuário que os usuários usam para fazer login.',
		'The leaderboard is currently in beta, and we may adjust the rating calculations as we refine the algorithm.':
			'A tabela de classificação está atualmente em beta, e podemos ajustar os cálculos de avaliação conforme refinamos o algoritmo.',
		'The maximum file size in MB. If the file size exceeds this limit, the file will not be uploaded.':
			'Tamanho máximo do arquivo em MB. Se o tamanho do arquivo exceder este limite, o arquivo não será enviado.',
		'The maximum number of files that can be used at once in chat. If the number of files exceeds this limit, the files will not be uploaded.':
			'O número máximo de arquivos que podem ser utilizados de cada vez no chat. Se o número de arquivos exceder este limite, os arquivos não serão enviados.',
		"The output format for the text. Can be 'json', 'markdown', or 'html'. Defaults to 'markdown'.":
			"Formato de saída para o texto. Pode ser 'json', 'markdown' ou 'html'. O padrão é 'markdown'.",
		"The passwords you entered don't quite match. Please double-check and try again.":
			'As senhas digitadas não coincidem. Por favor, verifique e tente novamente.',
		'The score should be a value between 0.0 (0%) and 1.0 (100%).':
			'A pontuação deve ser um valor entre 0.0 (0%) e 1.0 (100%).',
		'The stream delta chunk size for the model. Increasing the chunk size will make the model respond with larger pieces of text at once.':
			'O tamanho do bloco delta do fluxo para o modelo. Aumentar o tamanho do bloco fará com que o modelo responda com trechos maiores de texto de uma só vez.',
		'The temperature of the model. Increasing the temperature will make the model answer more creatively.':
			'A temperatura do modelo. Aumentar a temperatura fará com que o modelo responda de forma mais criativa.',
		'The Weight of BM25 Hybrid Search. 0 more semantic, 1 more lexical. Default 0.5':
			'O Peso da Busca Híbrida BM25. 0 = mais semântico, 1 = mais lexical. Padrão 0,5',
		'The width in pixels to compress images to. Leave empty for no compression.':
			'A largura em pixels para compactar as imagens. Deixe em branco para não compactar.',
		Theme: Ut,
		'There was an error syncing your stats. Please try again.':
			'Ocorreu um erro ao sincronizar suas estatísticas. Tente novamente.',
		'Thinking...': 'Pensando...',
		'This action cannot be undone. Do you wish to continue?':
			'Esta ação não pode ser desfeita. Você deseja continuar?',
		'This channel was created on {{createdAt}}. This is the very beginning of the {{channelName}} channel.':
			'Este canal foi criado em {{createdAt}}. Este é o início do canal {{channelName}}.',
		"This chat won't appear in history and your messages will not be saved.":
			'Este chat não aparecerá no histórico e suas mensagens não serão salvas.',
		'This ensures that your valuable conversations are securely saved to your backend database. Thank you!':
			'Isso garante que suas conversas valiosas sejam salvas com segurança no banco de dados do backend. Obrigado!',
		'This feature is currently experimental and may not work as expected.':
			'Esta funcionalidade encontra-se atualmente em fase experimental e poderá não funcionar conforme o esperado.',
		'This feature is experimental and may be modified or discontinued without notice.':
			'Este recurso é experimental e pode ser modificado ou descontinuado sem aviso prévio.',
		'This folder is empty': 'Esta pasta está vazia',
		'This is a default user permission and will remain enabled.':
			'Esta é uma permissão de usuário padrão e permanecerá ativada.',
		'This is an experimental feature, it may not function as expected and is subject to change at any time.':
			'Esta é uma funcionalidade experimental, pode não funcionar como esperado e está sujeita a alterações a qualquer momento.',
		'This model is not publicly available. Please select another model.':
			'Este modelo não está disponível publicamente. Selecione outro modelo.',
		'This option controls how long the model will stay loaded into memory following the request (default: 5m)':
			'Esta opção controla por quanto tempo o modelo permanecerá carregado na memória após a solicitação (padrão: 5m)',
		'This option controls how many tokens are preserved when refreshing the context. For example, if set to 2, the last 2 tokens of the conversation context will be retained. Preserving context can help maintain the continuity of a conversation, but it may reduce the ability to respond to new topics.':
			'Esta opção controla quantos tokens são preservados ao atualizar o contexto. Por exemplo, se definido como 2, os últimos 2 tokens do contexto da conversa serão mantidos. Preservar o contexto pode ajudar a manter a continuidade de uma conversa, mas pode reduzir a capacidade de responder a novos tópicos.',
		'This option enables or disables the use of the reasoning feature in Ollama, which allows the model to think before generating a response. When enabled, the model can take a moment to process the conversation context and generate a more thoughtful response.':
			'Esta opção habilita ou desabilita o uso do recurso de raciocínio no Ollama, que permite que o modelo pense antes de gerar uma resposta. Quando habilitada, o modelo pode levar um momento para processar o contexto da conversa e gerar uma resposta mais ponderada.',
		'This option sets the maximum number of tokens the model can generate in its response. Increasing this limit allows the model to provide longer answers, but it may also increase the likelihood of unhelpful or irrelevant content being generated.':
			'Esta opção define o número máximo de tokens que o modelo pode gerar em sua resposta. Aumentar esse limite permite que o modelo forneça respostas mais longas, mas também pode aumentar a probabilidade de geração de conteúdo inútil ou irrelevante.',
		'This option will delete all existing files in the collection and replace them with newly uploaded files.':
			'Essa opção deletará todos os arquivos existentes na coleção e todos eles serão substituídos.',
		...
		'This will delete <strong>{{NAME}}</strong> and <strong>all its contents</strong>.':
			'Esta ação excluirá <strong>{{NAME}}</strong> e <strong>todos seus conteúdos</strong>.',
		'This will delete all models including custom models':
			'Isto vai excluir todos os modelos, incluindo personalizados',
		'This will delete all models including custom models and cannot be undone.':
			'Isto vai excluir todos os modelos, incluindo personalizados e não pode ser desfeito.',
		'This will permanently delete the calendar "{{name}}" and all its events. This action cannot be undone.':
			'Esta ação excluirá permanentemente o calendário "{{name}}" e todos os seus eventos. Esta ação não pode ser desfeita.',
		'This will reset the knowledge base and sync all files. Do you wish to continue?':
			'Esta ação resetará a base de conhecimento e sincronizará todos os arquivos. Deseja continuar?',
		...
		'Thumbs up/down ratings from users on model responses':
			'Avaliações positivas/negativas dos usuários sobre as respostas do modelo',
		Tika: Ot,
		...
		'To access the available model names for downloading,':
			'Para acessar os nomes de modelos disponíveis para download,',
		'To access the GGUF models available for downloading,':
			'Para acessar os modelos GGUF disponíveis para download,',
		'To access the WebUI, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.':
			'Para acessar a WebUI, entre em contato com o administrador. Os administradores podem gerenciar os status dos usuários no Painel de Administração.',
		'To attach knowledge base here, add them to the "Knowledge" workspace first.':
			'Para anexar a base de conhecimento aqui, adicione-os ao espaço de trabalho "Conhecimento" primeiro.',
		'To learn more about available endpoints, visit our documentation.':
			'Para saber mais sobre os endpoints disponíveis, visite nossa documentação.',
		'To select skills here, add them to the "Skills" workspace first.':
			'Para selecionar skills aqui, adicione-as primeiro ao espaço de trabalho "Skills".',
		'To select toolkits here, add them to the "Tools" workspace first.':
			'Para selecionar kits de ferramentas aqui, adicione-os ao espaço de trabalho "Ferramentas" primeiro.',
		...
		'Token counts are estimates and may not reflect actual API usage':
			'A contagem de tokens é uma estimativa e pode não refletir o uso real da API.',
		tokens: _t,
		Tokens: Vt,
		...
		'Tools are a function calling system with arbitrary code execution':
			'Ferramentas são um sistema de chamada de funções com execução de código arbitrário',
		'Tools Function Calling Prompt': 'Prompt de chamada de função de ferramentas',
		'Tools have a function calling system that allows arbitrary code execution.':
			'Ferramentas possuem um sistema de chamada de funções que permite a execução de código arbitrário.',
		...
		'Update for the latest features and improvements.':
			'Atualizar para as novas funcionalidades e melhorias.',
		...
		'Upgrade to a licensed plan for enhanced capabilities, including custom theming and branding, and dedicated support.':
			'Faça upgrade para um plano licenciado para obter recursos aprimorados, incluindo temas e marcas personalizados e suporte dedicado.',
		Upload: si,
		...
		'Upload Progress: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)':
			'Progresso do upload: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)',
		...
		"Use '#' in the prompt input to load and include your knowledge.":
			"Usar '#' no prompt para carregar e incluir seus conhecimentos.",
		'Use /v1/chat/completions endpoint instead of /v1/audio/transcriptions for potentially better accuracy.':
			'Use o endpoint /v1/chat/completions em vez de /v1/audio/transcriptions para obter uma precisão potencialmente melhor.',
		'Use Chat Completions API': 'Usar API de Chat Completions',
		'Use groups to organize your users and assign permissions.':
			'Use grupos para organizar seus usuários e atribuir permissões.',
		'Use LLM': 'Usar LLM',
		'Use no proxy to fetch page contents.': 'Não utilize proxy para buscar o conteúdo da página.',
		'Use proxy designated by http_proxy and https_proxy environment variables to fetch page contents.':
			'Use o proxy designado pelas variáveis de ambiente http_proxy e https_proxy para buscar o conteúdo da página.',
		user: ni,
		User: di,
		...
		'User ratings (thumbs up/down)':
			'Avaliações dos usuários (polegar para cima/polegar para baixo)',
		...
		'Uses OAuth 2.1 Dynamic Client Registration':
			'Utiliza o registro dinâmico de cliente OAuth 2.1',
		'Using Entire Document': 'Usando o documento inteiro',
		'Using Focused Retrieval': 'Usando Recuperação Focada',
		'Using the default arena model with all models. Click the plus button to add custom models.':
			'Usando a arena de modelos padrão para todos os modelos. Clique no botão mais para adicionar modelos personalizados.',
		...
		'Version {{selectedVersion}} of {{totalVersions}}':
			'Versão {{selectedVersion}} de {{totalVersions}}',
		...
		'Voice Mode Custom Prompt': 'Modo de voz com prompt personalizado',
		...
		'Warning: Enabling this will allow users to run scheduled prompts automatically.':
			'Aviso: Habilitar esta opção permitirá que os usuários executem solicitações agendadas automaticamente.',
		'Warning: Enabling this will allow users to upload arbitrary code on the server.':
			'Aviso: Habilitar isso permitirá que os usuários façam upload de código arbitrário no servidor.',
		'Warning: Jupyter execution enables arbitrary code execution, posing severe security risks—proceed with extreme caution.':
			'Aviso: a execução do Jupyter permite a execução de código arbitrário, o que representa sérios riscos de segurança. Prossiga com extremo cuidado.',
		We_day_of_week: Ci,
		Web: yi,
		'Web API': 'API Web',
		...
		'When enabled, the model will respond to each chat message in real-time, generating a response as soon as the user sends a message. This mode is useful for live chat applications, but may impact performance on slower hardware.':
			'Quando habilitado, o modelo responderá a cada mensagem de chat em tempo real, gerando uma resposta assim que o usuário enviar uma mensagem. Este modo é útil para aplicativos de chat ao vivo, mas pode impactar o desempenho em hardware mais lento.',
		...
		'Whether to paginate the output. Each page will be separated by a horizontal rule and page number. Defaults to False.':
			'Se a saída deve ser paginada. Cada página será separada por uma régua horizontal e um número de página. O padrão é Falso.',
		...
		'Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text.':
			'Funciona em conjunto com o top-k. Um valor mais alto (por exemplo, 0,95) resultará em um texto mais diverso, enquanto um valor mais baixo (por exemplo, 0,5) gerará um texto mais focado e conservador.',
		Workspace: Ti,
		'Workspace Permissions': 'Permissões do espaço de trabalho',
		Write: Mi,
		'Write a summary in 50 words that summarizes {{topic}}.':
			'Escreva um resumo em 50 palavras que resuma {{topic}}.',
...
ex.: Você é o Mario do Super Mario Bros e atua como assistente.`,
		...
		'You are currently using a trial license. Please contact support to upgrade your license.':
			'Você está usando uma licença de teste. Entre em contato com o suporte para atualizar sua licença.',
		'You can only chat with a maximum of {{maxCount}} file(s) at a time.':
			'Você só pode conversar com no máximo {{maxCount}} arquivo(s) de cada vez.',
		"You can personalize your interactions with LLMs by adding memories through the 'Manage' button below, making them more helpful and tailored to you.":
			"Você pode personalizar suas interações com LLMs adicionando memórias através do botão 'Gerenciar' abaixo, tornando-as mais úteis e adaptadas a você.",
		...
		'You do not have permission to edit this skill.':
			'Você não tem permissão para editar esta skill.',
		...
		'You do not have permission to make this public':
			'Você não tem permissão para tornar isso público.',
		...
		'You do not have permission to send messages in this thread.':
			'Você não tem permissão para enviar mensagens neste tópico.',
		'You do not have permission to upload files to this knowledge base.':
			'Você não tem permissão para enviar arquivos para esta base de conhecimento.',
		'You do not have permission to upload files.':
			'Você não tem permissão para fazer upload de arquivos.',
		'You do not have permission to upload web content.':
			'Você não tem permissão para enviar conteúdo da web.',
		...
		'Your account status is currently pending activation.':
			'O status da sua conta está atualmente aguardando ativação.',
		...
		'Your entire contribution will go directly to the plugin developer; Open WebUI does not take any percentage. However, the chosen funding platform might have its own fees.':
			'Toda a sua contribuição irá diretamente para o desenvolvedor do plugin; o Open WebUI não retém nenhuma porcentagem. No entanto, a plataforma de financiamento escolhida pode ter suas próprias taxas.',
		'Your message text or inputs': 'Seu texto de mensagem ou entradas',
		'Your usage stats have been successfully synced.':
			'Suas estatísticas de uso foram sincronizadas com sucesso.',
		YouTube: Ni,
...
//# sourceMappingURL=BY_lkbXN.js.map
