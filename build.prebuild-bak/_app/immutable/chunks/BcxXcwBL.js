...
let B;
function he(n, e) {
	if (Q && n.data instanceof Blob) return n.data.arrayBuffer().then(Y).then(e);
	if (j && (n.data instanceof ArrayBuffer || G(n.data))) return e(Y(n.data));
	V(n, !1, (t) => {
...
for (let n = 0; n < z.length; n++) E[z.charCodeAt(n)] = n;
const ue = (n) => {
		let e = n.length * 0.75,
			t = n.length,
			s,
	...
	M = (n, e) => {
		if (typeof n != 'string') return { type: 'message', data: Z(n, e) };
		const t = n.charAt(0);
		return t === 'b'
			? { type: 'message', data: le(n.substring(1), e) }
			: T[t]
				? n.length > 1
					? { type: T[t], data: n.substring(1) }
					: { type: T[t] }
				: P;
	},
	le = (n, e) => {
...
function A(n, e) {
	...
	let s = 0;
	for (let i = 0; i < e; i++) ((t[i] = n[0][s++]), s === n[0].length && (n.shift(), (s = 0)));
		...
		transform(o, a) {
			for (t.push(o); ; ) {
				if (s === 0) {
					...
					if (k(t) < i) break;
					const h = A(t, i);
					(a.enqueue(M(r ? h : x.decode(h), e)), (s = 0));
				}
				if (i === 0 || i > n) {
					a.enqueue(P);
					break;
...
const C =
		typeof Promise == 'function' && typeof Promise.resolve == 'function'
			...
			: (e, t) => t(e, 0),
	l = typeof self < 'u' ? self : typeof window < 'u' ? window : Function('return this')(),
	_e = 'arraybuffer';
function se(n, ...e) {
...
function S(n, e) {
	e.useNativeTimers
		? ((n.setTimeoutFn = be.bind(l)), (n.clearTimeoutFn = we.bind(l)))
		: ((n.setTimeoutFn = l.setTimeout.bind(l)), (n.clearTimeoutFn = l.clearTimeout.bind(l)));
}
const ve = 1.33;
function Ee(n) {
	return typeof n == 'string' ? ke(n) : Math.ceil((n.byteLength || n.size) * ve);
}
function ke(n) {
	let e = 0,
		t = 0;
...
class Oe extends H {
	...
	uri() {
		const e = this.opts.secure ? 'https' : 'http',
			t = this.query || {};
		return (
			this.opts.timestampRequests !== !1 && (t[this.opts.timestampParam] = ne()),
...
class Be extends Oe {
	constructor(e) {
		if ((super(e), typeof location < 'u')) {
			...
			let s = location.port;
			(s || (s = t ? '443' : '80'),
				(this.xd = (typeof location < 'u' && e.hostname !== location.hostname) || s !== e.port));
		}
	}
	doWrite(e, t) {
...
function J() {
	for (let n in d.requests) d.requests.hasOwnProperty(n) && d.requests[n].abort();
}
const xe = (function () {
	const n = re({ xdomain: !1 });
		...
		(this._transport.closed
			...
			this._transport.ready.then(() => {
				this._transport.createBidirectionalStream().then((e) => {
					const t = me(Number.MAX_SAFE_INTEGER, this.socket.binaryType),
...
class _ extends u {
	constructor(e, t) {
		...
		(S(this, t),
			...
			typeof this.opts.query == 'string' && (this.opts.query = Te(this.opts.query)),
			D &&
	...
	_getWritablePackets() {
		...
		for (let s = 0; s < this.writeBuffer.length; s++) {
			...
			if ((i && (t += Ee(i)), s > 0 && t > this._maxPayload)) return this.writeBuffer.slice(0, s);
			t += 2;
		}
		return this.writeBuffer;
	}
...
function K(n) {
	return (
		(We && (n instanceof ArrayBuffer || $e(n))) ||
		(Ye && n instanceof Blob) ||
...
function O(n, e) {
	...
	if (K(n)) return !0;
	if (n.toJSON && typeof n.toJSON == 'function' && arguments.length === 1) return O(n.toJSON(), !0);
	for (const t in n) if (Object.prototype.hasOwnProperty.call(n, t) && O(n[t])) return !0;
	return !1;
}
function Je(n) {
	const e = [],
...
function I(n, e) {
	...
	if (K(n)) {
		...
		const t = {};
		for (const s in n) Object.prototype.hasOwnProperty.call(n, s) && (t[s] = I(n[s], e));
		return t;
	}
	return n;
}
...
function U(n, e) {
	...
	} else if (Array.isArray(n)) for (let t = 0; t < n.length; t++) n[t] = U(n[t], e);
	else if (typeof n == 'object')
		for (const t in n) Object.prototype.hasOwnProperty.call(n, t) && (n[t] = U(n[t], e));
	return n;
}
const Qe = [
	'connect',
...
class je {
	...
	encode(e) {
		return (e.type === c.EVENT || e.type === c.ACK) && O(e)
			? this.encodeAsBinary({
...
class W extends u {
	...
	decodeString(e) {
		...
		if (c[s.type] === void 0) throw new Error('unknown packet type ' + s.type);
		if (s.type === c.BINARY_EVENT || s.type === c.BINARY_ACK) {
			...
			const a = Number(o);
			if (!Ze(a) || a < 0) throw new Error('Illegal attachments');
	...
	static isPayloadValid(e, t) {
		switch (e) {
			...
			case c.BINARY_EVENT:
				return (
					Array.isArray(t) &&
					(typeof t[0] == 'number' || (typeof t[0] == 'string' && Qe.indexOf(t[0]) === -1))
				);
			case c.ACK:
			case c.BINARY_ACK:
...
class ce extends u {
	...
	emit(e, ...t) {
		var s, i, r;
		if (tt.hasOwnProperty(e)) throw new Error('"' + e.toString() + '" is a reserved event name');
		if ((t.unshift(e), this._opts.retries && !this.flags.fromQueue && !this.flags.volatile))
			return (this._addToQueue(t), this);
		const o = { type: c.EVENT, data: t };
		if (
			((o.options = {}),
		...
		const a =
				(i = (s = this.io.engine) === null || s === void 0 ? void 0 : s.transport) === null ||
				i === void 0
					? void 0
					: i.writable,
			h =
				this.connected &&
				!(!((r = this.io.engine) === null || r === void 0) && r._hasPingExpired());
		return (
	...
	ack(e) {
		...
		let s = !1;
		return function (...i) {
			s || ((s = !0), t.packet({ type: c.ACK, id: e, data: i }));
		};
	}
	onack(e) {
...
function L(n, e) {
	...
	let h;
	return (
		a ? (h = new F(s, e)) : (v[i] || (v[i] = new F(s, e)), (h = v[i])),
...
export { L as l };
//# sourceMappingURL=BcxXcwBL.js.map
