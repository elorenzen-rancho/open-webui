import { g as ge } from './CjlqM2S6.js';
import {
	_ as d,
	D as lt,
	d as de,
...
import { c as Le } from './f9y8NCnX.js';
import { s as C } from './BX5WB7ra.js';
import { G as Se } from './Dzc2gp-m.js';
import { c as ve } from './B12XNpIA.js';
import { l as Ee, k as _e } from './Bb3-t4AS.js';
var bt = (function () {
			...
			performAction: d(function (y, u, p, S, E, o, k) {
				...
				switch (E) {
					...
					case 12:
						...
						break;
					case 13:
						(S.getLogger().debug('Rule: statement #2: ', o[g - 1]),
							(this.$ = [o[g - 1]].concat(o[g])));
						break;
					case 14:
						(S.getLogger().debug('Rule: link: ', o[g], y),
							(this.$ = { edgeTypeStr: o[g], label: '' }));
						break;
					case 15:
					...
					case 26:
						(S.getLogger().debug('APA123', this ? this : 'na'),
							...
							(this.$ = {
								type: 'column-setting',
								columns: o[g] === 'auto' ? -1 : parseInt(o[g])
							}));
						break;
					case 27:
					...
					case 28:
						...
						break;
					case 29:
						(S.getLogger().debug('Rule: node (NODE_ID separator): ', o[g]),
							(this.$ = { id: o[g] }));
						break;
					case 30:
						(S.getLogger().debug(
					...
					case 31:
						...
						break;
					case 32:
						(S.getLogger().debug('Rule: dirList: ', o[g - 1], o[g]),
							(this.$ = [o[g - 1]].concat(o[g])));
						break;
					case 33:
			...
			parse: d(function (y) {
				...
				function It() {
					...
					return (
						(X = S.pop() || R.lex() || P),
						typeof X != 'number' &&
							(X instanceof Array && ((S = X), (X = S.pop())), (X = u.symbols_[X] || X)),
						X
					);
				}
				d(It, 'lex');
...
Expecting ` +
					...
					if (V[0] instanceof Array && V.length > 1)
						throw new Error(
							'Parse Error: multiple actions possible at state: ' + $ + ', token: ' + H
						);
					switch (V[0]) {
						case 1:
							(p.push(H),
						...
						case 2:
							if (
								...
								return ft;
							(J &&
								((p = p.slice(0, -1 * J * 2)), (E = E.slice(0, -1 * J)), (o = o.slice(0, -1 * J))),
				...
				unput: d(function (y) {
					...
					return (
						(this.yylloc = {
							...
							last_column: p
								? (p.length === S.length ? this.yylloc.first_column : 0) +
									S[S.length - p.length].length -
									p[0].length
...
` +
				...
				next: d(function () {
					...
					for (var E = this._currentRules(), o = 0; o < E.length; o++)
						if (
							((p = this._input.match(this.rules[E[o]])), p && (!u || p[0].length > u[0].length))
						) {
				...
				performAction: d(function (u, p, S, E) {
					switch (S) {
						...
						case 86:
							return (
								(p.yytext = p.yytext.replace(/^,\s*/, '')),
								u.getLogger().debug('Lex (left):', p.yytext),
								'DIR'
							);
						case 87:
							return (
								(p.yytext = p.yytext.replace(/^,\s*/, '')),
								u.getLogger().debug('Lex (x):', p.yytext),
								'DIR'
							);
						case 88:
							return (
								(p.yytext = p.yytext.replace(/^,\s*/, '')),
								u.getLogger().debug('Lex (y):', p.yytext),
								'DIR'
							);
						case 89:
							return (
								(p.yytext = p.yytext.replace(/^,\s*/, '')),
								u.getLogger().debug('Lex (up):', p.yytext),
								'DIR'
							);
						case 90:
							return (
								(p.yytext = p.yytext.replace(/^,\s*/, '')),
								u.getLogger().debug('Lex (down):', p.yytext),
								'DIR'
							);
						case 91:
							return (
						...
						case 96:
							return (
								u.getLogger().debug('Lex: START_LINK', p.yytext),
						...
						case 100:
							return (
								u.getLogger().debug('Lex: Starting string'),
								this.pushState('string'),
								'LINK_LABEL'
							);
						case 101:
						...
						case 104:
							return (
								u.getLogger().debug('Lex: COLON', p.yytext),
								(p.yytext = p.yytext.slice(1)),
								27
							);
					}
				}, 'anonymous'),
...
var ke = bt,
	...
	gt = new Map(),
	_t = '',
	Ne = d((e) => be.sanitizeText(e, Te), 'sanitizeText'),
	Ie = d(function (e, t = '') {
		let a = gt.get(e);
...
function dt(e, t, a = 0, i = 0) {
	...
	(v.debug(
		...
		((n = e == null ? void 0 : e.size) != null && n.width) ||
	...
	if (((c = e.children) == null ? void 0 : c.length) > 0) {
		...
		((l = B.width),
			(s = B.height),
			v.debug('setBlockSizes abc95 maxWidth of', e.id, ':s children is ', l, s));
		...
		if (p < a) {
			...
			for (const k of e.children)
				k.size && ((k.size.width = E), (k.size.height = o), (k.size.x = 0), (k.size.y = 0));
		}
		if (
			(v.debug(
...
function Dt(e, t) {
	...
	const a = e.columns ?? -1;
	if (
		(v.debug('layoutBlocks columns abc95', e.id, '=>', a, e), e.children && e.children.length > 0)
	) {
		const y =
				((n = (r = e == null ? void 0 : e.children[0]) == null ? void 0 : r.size) == null
					? void 0
					: n.width) ?? 0,
	...
	dr = d(async (e, t) => {
		...
		if (i) {
			...
			((c = f.getBoundingClientRect()),
				(x = c),
				w.attr('width', c.width),
				w.attr('height', c.height));
		} else {
  ...
  node        : x:${e.x} y:${e.y} w:${e.width} h:${e.height}`);
	...
	xr = d(function (e, t, a, i, l, s, r) {
		...
		(h != null &&
			...
			a.toCluster &&
				(v.debug('to cluster abc88', i[a.toCluster]),
				(n = Ft(a.points, i[a.toCluster].node)),
				(c = !0)),
			a.fromCluster &&
		...
		const B = e
			...
			.attr('style', a.style);
		let I = '';
		((M().flowchart.arrowMarkerAbsolute || M().state.arrowMarkerAbsolute) && (I = xe(!0)),
			hr(B, a, I, r, l));
		let N = {};
	...
	F = d(async (e, t, a, i) => {
		...
		if (Y(l)) {
			...
			(await me(b, x),
				...
				m.attr('width', f.width),
				m.attr('height', f.height));
		}
		return (
			r
			...
			(t.intersect = function (h) {
				...
				if (
					...
					(Math.abs(w) < t.width / 2 ||
						(Math.abs(w) == t.width / 2 && Math.abs(f.y - t.y) > t.height / 2 - r))
				) {
					let b = r * r * (1 - (w * w) / (s * s));
...
var Wr = d(async (e, t) => {
		...
		(typeof n == 'object' ? (c = n[0]) : (c = n),
			v.info('Label text abc79', c, n, typeof n == 'object'));
		...
		return (
			C(b).attr(
				'transform',
				'translate( ' +
					(h.width > w.width ? 0 : (w.width - h.width) / 2) +
					', ' +
					(w.height + m + 5) +
					')'
			),
			C(x).attr(
				'transform',
				'translate( ' + (h.width < w.width ? 0 : -(w.width - h.width) / 2) + ', 0)'
			),
			(h = r.node().getBBox()),
	...
	Kr = d(async (e, t) => {
		...
		return (
			...
			(t.intersect = function (x) {
				return (
					v.info('DoubleCircle intersect', t, i.width / 2 + l + s, x),
					O.circle(t, i.width / 2 + l + s, x)
				);
			}),
			a
		);
	...
	Vr = d(async (e, t) => {
		...
		if (
			...
			let o = (h - B.width) / 2;
			(C(L).attr('transform', 'translate( ' + ((-1 * h) / 2 + o) + ', ' + (-1 * f) / 2 + ')'),
				(b = B.height + i));
		}
		let S = (h - y.width) / 2;
		return (
...
function Nt(e, t, a = !1) {
	...
	const c = we((i == null ? void 0 : i.styles) ?? []),
		x = i.label,
...
//# sourceMappingURL=Bpctu5MG.js.map
