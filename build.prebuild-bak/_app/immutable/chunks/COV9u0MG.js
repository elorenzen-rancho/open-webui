import { P as L, N as $, c as E, D as j, e as y, T as S, I as G } from './chV95BsE.js';
var N = {};
class v {
	...
	reduce(t) {
		...
		let f = o ? this.stack[o - 1] : 0,
			p = this.bufferBase + this.buffer.length - f;
	...
	shift(t, e, s, i) {
		if (t & 131072) this.pushState(t & 65535, this.pos);
			...
			((this.pos = i),
				...
				e <= this.p.parser.maxNode && this.buffer.push(e, s, i, 4));
		else {
			let h = t,
				{ parser: r } = this.p;
			this.pos = i;
	...
	t: for (; n & l[r]; ) {
		...
		for (let c = r + 3; c < a; c += 2)
			if ((l[c + 1] & n) > 0) {
				...
				if (
					o.allows(d) &&
					(t.token.value == -1 || t.token.value == d || q(d, t.token.value, i, h))
				) {
					t.acceptToken(d);
					break;
				}
			...
			for (;;) {
				if ((e < 0 ? s.to < t : s.from > t) && !s.type.isError)
					return e < 0
						...
						: Math.min(l.length, Math.max(s.from + 1, t + 25));
	...
	nodeAt(t) {
		...
		for (;;) {
			...
			if (h instanceof S) {
				...
				(this.index[e]++,
					r + h.length >= Math.max(this.safeFrom, t) &&
						(this.trees.push(h), this.start.push(r), this.index.push(0)));
...
class Q {
	constructor(t, e, s, i) {
		...
		((this.stacks = [v.start(this, t.top[0], h)]),
			(this.fragments =
				s.length && this.stream.end - h > t.bufferLength * 4 ? new J(s, t.nodeSet) : null));
	}
	get parsedPos() {
	...
	advance() {
		...
		if (this.bigReductionCount > 300 && t.length == 1) {
			...
			for (
				...
				r.forceReduce() &&
				r.stack.length &&
				r.stack[r.stack.length - 2] >= this.lastBigReductionStart;
			);
		...
		if (!s.length) {
			let r = i && X(i);
			if (r) return (g && '' + this.stackID(r), this.stackToTree(r));
		...
		if (this.recovering && i) {
			let r =
				this.stoppedAt != null && i[0].pos > this.stoppedAt ? i[0] : this.runRecovery(i, h, s);
			if (r) return (g && '' + this.stackID(r), this.stackToTree(r.forceAll()));
		}
		if (this.recovering) {
	...
	advanceStack(t, e, s) {
		...
		if (this.fragments) {
			...
			for (let f = this.fragments.nodeAt(i); f; ) {
				...
				if (p > -1 && f.length && (!a || (f.prop(y.contextHash) || 0) == u))
					return (
						t.useNode(f, p),
						g && r + this.stackID(t) + ` (via reuse of ${h.getName(f.type.id)})`,
						!0
					);
		...
		if (n > 0)
			return (
				t.reduce(n),
				g && r + this.stackID(t) + ` (via always-reduce ${h.getName(n & 65535)})`,
				!0
			);
		...
		for (let a = 0; a < o.length; ) {
			...
			if (
				(d.apply(u, f, b ? b.start : d.pos, p),
				g &&
					r +
						this.stackID(d) +
						` (via ${u & 65536 ? `reduce of ${h.getName(u & 65535)}` : 'shift'} for ${h.getName(f)} @ ${i}${d == t ? '' : ', split'})`,
				c)
			)
				return !0;
	...
	runRecovery(t, e, s) {
		...
		for (let r = 0; r < t.length; r++) {
			...
			if (
				n.deadEnd &&
				(h || ((h = !0), n.restart(), g && u + this.stackID(n) + '', this.advanceFully(n, s)))
			)
				continue;
			let f = n.split(),
				p = u;
			for (
				let c = 0;
				c < 10 && f.forceReduce() && (g && p + this.stackID(f) + '', !this.advanceFully(f, s));
				c++
			)
				g && (p = this.stackID(f) + ' -> ');
			for (let c of n.recoverByInsert(o)) (g && u + this.stackID(c) + '', this.advanceFully(c, s));
			this.stream.end > n.pos
				? (a == n.pos && (a++, (o = 0)),
					n.recoverByDelete(o, a),
					g && u + this.stackID(n) + ` (via recover-delete ${this.parser.getName(o)})`,
					B(n, s))
	...
	allActions(t, e) {
		...
		for (let h = this.stateSlot(t, 1); i == null; h += 3) {
			if (this.data[h] == 65535)
				if (this.data[h + 1] == 1) h = k(this.data, h + 2);
				else break;
			i = e(k(this.data, h + 1));
		}
		return i;
	...
	nextStates(t) {
		...
		for (let s = this.stateSlot(t, 1); ; s += 3) {
			if (this.data[s] == 65535)
				if (this.data[s + 1] == 1) s = k(this.data, s + 2);
				else break;
			if (!(this.data[s + 2] & 1)) {
				let i = this.data[s + 1];
...
//# sourceMappingURL=COV9u0MG.js.map
