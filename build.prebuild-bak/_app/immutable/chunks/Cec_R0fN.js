const e = 'درباره',
	...
	Fr = {
		'-1 for no limit, or a positive integer for a specific limit':
			'-1 برای بدون محدودیت، یا یک عدد مثبت برای محدودیت مشخص',
		"'s', 'm', 'h', 'd', 'w' or '-1' for no expiration.":
			"'s', 'm', 'h', 'd', 'w' یا '-1' برای غیر فعال کردن انقضا.",
		'(e.g. `sh webui.sh --api --api-auth username_password`)':
			'(مثال: `sh webui.sh --api --api-auth username_password`)',
		'(e.g. `sh webui.sh --api`)': '(e.g. `sh webui.sh --api`)',
		'(latest)': '(آخرین)',
		'(leave blank for to use commercial endpoint)':
			'(برای استفاده از نقطه پایانی تجاری خالی بگذارید)',
		...
		'*Prompt node ID(s) are required for image generation':
			'*شناسه(های) گره پرامپت برای تولید تصویر مورد نیاز است',
		'1 hour before': '',
		'1 Source': '۱ منبع',
		'10 minutes before': '',
		'15 minutes before': '',
		...
		'A new version (v{{LATEST_VERSION}}) is now available.':
			'نسخه جدید (v{{LATEST_VERSION}}) در دسترس است.',
		'A private conversation between you and selected users': '',
		'A task model is used when performing tasks such as generating titles for chats and web search queries':
			'یک مدل وظیفه هنگام انجام وظایف مانند تولید عناوین برای چت ها و نمایش های جستجوی وب استفاده می شود.',
		'a user': 'یک کاربر',
		About: e,
...
پرش به متغیر پرامپت`,
		...
		'Activate this command by typing "/{{COMMAND}}" to chat input.':
			'این دستور را با تایپ "/{{COMMAND}}" در ورودی چت فعال کنید.',
		Active: r,
		...
		'Add a short description about what this model does':
			'افزودن توضیحات کوتاه در مورد انچه که این مدل انجام می دهد',
		...
		'Adjusting these settings will apply changes universally to all users.':
			'با تنظیم این تنظیمات، تغییرات به طور کلی برای همه کاربران اعمال می‌شود.',
		admin: c,
		Admin: d,
		'Admin Contact Email': '',
		...
		'Admins have access to all tools at all times; users need tools assigned per model in the workspace.':
			'مدیران همیشه به تمام ابزارها دسترسی دارند؛ کاربران نیاز به ابزارهای اختصاص داده شده برای هر مدل در فضای کاری دارند.',
		Advanced: u,
		...
		'Advanced parameters for MinerU parsing (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)':
			'پارامترهای پیشرفته برای تجزیه MinerU (فعال‌سازی OCR، فعال‌سازی فرمول، فعال‌سازی جدول، زبان، نسخه مدل، محدوده‌های صفحه)',
		...
		'After updating or changing the embedding model, you must reindex the knowledge base for the changes to take effect. You can do this using the "Reindex" button below.':
			'پس از به‌روزرسانی یا تغییر مدل جاسازی، برای اعمال تغییرات باید پایگاه دانش را مجدداً نمایه‌سازی کنید. می‌توانید این کار را با استفاده از دکمه "نمایه‌سازی مجدد" در زیر انجام دهید.',
		AI: m,
		All: p,
		...
		'Allowed file extensions for upload. Separate multiple extensions with commas. Leave empty for all file types.':
			'پسوندهای فایل مجاز برای آپلود. چندین پسوند را با کاما از هم جدا کنید. برای همه انواع فایل‌ها خالی بگذارید.',
		'Already have an account?': 'از قبل حساب کاربری دارید؟',
		'Alternative to the top_p, and aims to ensure a balance of quality and variety. The parameter p represents the minimum probability for a token to be considered, relative to the probability of the most likely token. For example, with p=0.05 and the most likely token having a probability of 0.9, logits with a value less than 0.045 are filtered out.':
			'جایگزینی برای top_p و هدف آن اطمینان از تعادل کیفیت و تنوع است. پارامتر p نشان‌دهنده حداقل احتمال برای در نظر گرفتن یک توکن نسبت به احتمال محتمل‌ترین توکن است. به عنوان مثال، با p=0.05 و محتمل‌ترین توکن با احتمال 0.9، لاگیت‌های با مقدار کمتر از 0.045 فیلتر می‌شوند.',
		Always: h,
		...
		'API Base URL for Datalab Marker service. Defaults to: https://www.datalab.to/api/v1/marker':
			'آدرس پایه API برای سرویس مارکر دیتا‌لب. پیش‌فرض: https://www.datalab.to/api/v1/marker',
		'API Key': 'کلید API',
		...
		'Are you sure you want to clear all memories? This action cannot be undone.':
			'آیا مطمئن هستید که می‌خواهید تمام حافظه‌ها را پاک کنید؟ این عمل قابل بازگشت نیست.',
		...
		'Are you sure you want to unarchive all archived chats?':
			'آیا مطمئن هستید که می‌خواهید همه گفتگوهای بایگانی شده را از بایگانی خارج کنید؟',
		...
		'Base Model List Cache speeds up access by fetching base models only at startup or on settings save—faster, but may not show recent base model changes.':
			'کش لیست مدل پایه، با واکشی مدل‌های پایه فقط در هنگام راه‌اندازی یا ذخیره تنظیمات، دسترسی را سرعت می‌بخشد – سریع‌تر است، اما ممکن است تغییرات اخیر مدل پایه را نشان ندهد.',
		Bearer: W,
		before: K,
		...
		'Boosting or penalizing specific tokens for constrained responses. Bias values will be clamped between -100 and 100 (inclusive). (Default: none)':
			'تقویت یا جریمه توکن‌های خاص برای پاسخ‌های محدود. مقادیر بایاس بین -100 و 100 (شامل) محدود خواهند شد. (پیش‌فرض: هیچ)',
		Brave: q,
		...
		'Call feature is not supported when using Web STT engine':
			'ویژگی تماس هنگام استفاده از موتور Web STT پشتیبانی نمی‌شود',
		Camera: j,
		Cancel: Q,
		...
		'Character limit for autocomplete generation input':
			'محدودیت کاراکتر برای ورودی تولید تکمیل خودکار',
		...
		'Click here to download user import template file.':
			'برای دانلود فایل قالب واردات کاربر اینجا کلیک کنید.',
		'Click here to learn more about faster-whisper and see the available models.':
			'برای یادگیری بیشتر درباره faster-whisper و دیدن مدل‌های موجود اینجا کلیک کنید.',
		...
		"Click on the user role button to change a user's role.":
			'برای تغییر نقش کاربر، روی دکمه نقش کاربر کلیک کنید.',
		'Click to connect': '',
		'Click to copy ID': '',
		'Client ID': '',
		'Client Secret': '',
		'Clipboard write permission denied. Please check your browser settings to grant the necessary access.':
			'دسترسی نوشتن در کلیپ‌بورد رد شد. لطفاً تنظیمات مرورگر خود را برای اعطای دسترسی لازم بررسی کنید.',
		Clone: le,
		...
		'Comma separated Node Ids (e.g. 1 or 1,2)':
			'شناسه‌های گره که با کاما جدا شده‌اند (مثلاً ۱ یا ۱,۲)',
		command: ye,
		Command: fe,
		Comment: Ae,
		'Commit Message': '',
		...
		'Connect to your own OpenAI compatible API endpoints.':
			'به نقاط پایانی API سازگار با OpenAI خود متصل شوید.',
		'Connect to your own OpenAPI compatible external tool servers.':
			'به سرورهای ابزار خارجی سازگار با OpenAPI خود متصل شوید.',
		'Connected ({{type}})': '',
		...
		'Constrains effort on reasoning for reasoning models. Only applicable to reasoning models from specific providers that support reasoning effort.':
			'تلاش برای استدلال در مدل‌های استدلالی را محدود می‌کند. فقط برای مدل‌های استدلالی از ارائه‌دهندگان خاصی که از تلاش استدلالی پشتیبانی می‌کنند قابل اجراست.',
		...
		"Control how message text is split for TTS requests. 'Punctuation' splits into sentences, 'paragraphs' splits into paragraphs, and 'none' keeps the message as a single string.":
			"کنترل نحوه تقسیم متن پیام برای درخواست‌های TTS. 'علامت‌گذاری' به جملات تقسیم می‌کند، 'پاراگراف‌ها' به پاراگراف‌ها تقسیم می‌کند و 'هیچ‌کدام' پیام را به عنوان یک رشته واحد نگه می‌دارد.",
		'Control the repetition of token sequences in the generated text. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 1.1) will be more lenient. At 1, it is disabled.':
			'کنترل تکرار توالی‌های توکن در متن تولید شده. مقدار بالاتر (مثلاً 1.5) تکرارها را شدیدتر جریمه می‌کند، در حالی که مقدار پایین‌تر (مثلاً 1.1) ملایم‌تر خواهد بود. در 1، غیرفعال است.',
		Controls: Ee,
		'Controls the balance between coherence and diversity of the output. A lower value will result in more focused and coherent text.':
			'تعادل بین انسجام و تنوع خروجی را کنترل می‌کند. مقدار پایین‌تر منجر به متن متمرکزتر و منسجم‌تر می‌شود.',
		...
		'CORS must be properly configured by the provider to allow requests from Open WebUI.':
			'CORS باید توسط ارائه‌دهنده به درستی پیکربندی شود تا درخواست‌ها از Open WebUI مجاز باشند.',
		'Could not read file.': '',
		CPU: ke,
		Create: Me,
		...
		'Create your first note by clicking on the plus button below.':
			'با کلیک روی دکمه به‌علاوه در زیر، اولین یادداشت خود را ایجاد کنید.',
		...
		"Default mode works with a wider range of models by calling tools once before execution. Native mode leverages the model's built-in tool-calling capabilities, but requires the model to inherently support this feature.":
			'حالت پیش‌فرض با فراخوانی ابزارها یک بار قبل از اجرا، با طیف وسیع‌تری از مدل‌ها کار می‌کند. حالت بومی از قابلیت‌های داخلی فراخوانی ابزار مدل استفاده می‌کند، اما مدل باید به طور ذاتی این ویژگی را پشتیبانی کند.',
		...
		'Default to segmented retrieval for focused and relevant content extraction, this is recommended for most cases.':
			'پیش‌فرض به بازیابی قطعه‌ای برای استخراج محتوای متمرکز و مرتبط، این برای اکثر موارد توصیه می‌شود.',
		...
		'Deployment names are required for Azure OpenAI':
			'نام‌های استقرار برای Azure OpenAI مورد نیاز هستند',
		Desc: Ge,
		'Describe the edit...': '',
		'Describe the image...': '',
		...
		'Direct Connections allow users to connect to their own OpenAI compatible API endpoints.':
			'اتصالات مستقیم به کاربران اجازه می‌دهد به نقاط پایانی API سازگار با OpenAI خود متصل شوند.',
		'Direct Message': '',
		...
		'Disable image extraction from the PDF. If Use LLM is enabled, images will be automatically captioned. Defaults to False.':
			'غیرفعال کردن استخراج تصویر از PDF. اگر «استفاده از LLM» فعال باشد، تصاویر به‌طور خودکار زیرنویس خواهند شد. پیش‌فرض: False.',
		Disabled: He,
		'Disconnect OAuth': '',
		...
		'Discover how to use Open WebUI and seek support from the community.':
			'نحوه استفاده از Open WebUI را کشف کنید و از انجمن پشتیبانی بگیرید.',
		...
		'Discover, download, and explore custom prompts':
			'پرامپت‌های سفارشی را کشف، دانلود و کاوش کنید',
		...
		'Displays status updates (e.g., web search progress) in the response':
			'نمایش به‌روزرسانی‌های وضعیت (مثلاً پیشرفت جستجوی وب) در پاسخ',
		'Dive into knowledge': 'غوطه‌ور شدن در دانش',
		'Do not install functions from sources you do not fully trust.':
			'توابع را از منابعی که کاملاً به آنها اعتماد ندارید نصب نکنید.',
		'Do not install tools from sources you do not fully trust.':
			'ابزارها را از منابعی که کاملاً به آنها اعتماد ندارید نصب نکنید.',
		...
		'does not make any external connections, and your data stays securely on your locally hosted server.':
			'هیچ اتصال خارجی ایجاد نمی کند و داده های شما به طور ایمن در سرور میزبان محلی شما باقی می ماند.',
		'Domain Filter List': 'لیست فیلتر دامنه',
		"don't fetch random pipelines from sources you don't trust.":
			'pipelineهای تصادفی را از منابع غیرقابل اعتماد دریافت نکنید.',
		"Don't have an account?": 'حساب کاربری ندارید؟',
		"don't install random functions from sources you don't trust.":
			'توابع تصادفی را از منابعی که به آنها اعتماد ندارید نصب نکنید.',
		"don't install random tools from sources you don't trust.":
			'ابزارهای تصادفی را از منابعی که به آنها اعتماد ندارید نصب نکنید.',
		...
		"e.g. '30s','10m'. Valid time units are 's', 'm', 'h'.":
			"به طور مثال '30s','10m'. واحد‌های زمانی معتبر 's', 'm', 'h' هستند.",
		"e.g. 'low', 'medium', 'high'": '',
		...
		'e.g., audio/wav,audio/mpeg,video/* (leave blank for defaults)':
			'مثلاً audio/wav,audio/mpeg,video/* (برای پیش‌فرض‌ها خالی بگذارید)',
		'e.g., en-US,ja-JP (leave blank for auto-detect)':
			'مثلا en-US,ja-JP (برای تشخیص خودکار خالی بگذارید)',
		...
		'Enable autocomplete generation for chat messages':
			'فعال‌سازی تولید تکمیل خودکار برای پیام‌های چت',
		...
		"Enable Memory Locking (mlock) to prevent model data from being swapped out of RAM. This option locks the model's working set of pages into RAM, ensuring that they will not be swapped out to disk. This can help maintain performance by avoiding page faults and ensuring fast data access.":
			'فعال‌سازی قفل حافظه (mlock) برای جلوگیری از تعویض داده‌های مدل از RAM. این گزینه مجموعه صفحات کاری مدل را در RAM قفل می‌کند و اطمینان می‌دهد که به دیسک منتقل نمی‌شوند. این می‌تواند با جلوگیری از خطاهای صفحه و تضمین دسترسی سریع به داده‌ها، عملکرد را حفظ کند.',
		'Enable Memory Mapping (mmap) to load model data. This option allows the system to use disk storage as an extension of RAM by treating disk files as if they were in RAM. This can improve model performance by allowing for faster data access. However, it may not work correctly with all systems and can consume a significant amount of disk space.':
			'فعال‌سازی نگاشت حافظه (mmap) برای بارگیری داده‌های مدل. این گزینه به سیستم اجازه می‌دهد از فضای دیسک به عنوان گسترش RAM استفاده کند با در نظر گرفتن فایل‌های دیسک مانند اینکه در RAM هستند. این می‌تواند با اجازه دادن به دسترسی سریع‌تر به داده‌ها، عملکرد مدل را بهبود بخشد. با این حال، ممکن است با همه سیستم‌ها به درستی کار نکند و می‌تواند مقدار قابل توجهی از فضای دیسک را مصرف کند.',
		'Enable Message Queue': '',
		'Enable Message Rating': 'فعال‌سازی امتیازدهی پیام',
		'Enable Mirostat sampling for controlling perplexity.':
			'فعال‌سازی نمونه‌برداری میروستات برای کنترل سردرگمی',
		...
		'Enable, disable, or customize the reasoning tags used by the model. "Enabled" uses default tags, "Disabled" turns off reasoning tags, and "Custom" lets you specify your own start and end tags.':
			'تگ‌های استدلال مورد استفاده مدل را فعال، غیرفعال یا سفارشی کنید. «فعال» از تگ‌های پیش‌فرض استفاده می‌کند، «غیرفعال» تگ‌های استدلال را خاموش می‌کند، و «سفارشی» به شما امکان می‌دهد تگ‌های شروع و پایان خود را مشخص کنید.',
		Enabled: mt,
		'End Tag': 'تگ پایان',
		...
		'Ensure your CSV file includes 4 columns in this order: Name, Email, Password, Role.':
			'اطمینان حاصل کنید که فایل CSV شما شامل چهار ستون در این ترتیب است: نام، ایمیل، رمز عبور، نقش.',
		'Enter {{role}} message here': 'پیام {{role}} را اینجا وارد کنید',
		'Enter a detail about yourself for your LLMs to recall':
			'برای ذخیره سازی اطلاعات خود، یک توضیح کوتاه درباره خود را وارد کنید',
		'Enter a title for the pending user info overlay. Leave empty for default.':
			'یک عنوان برای پوشش اطلاعات کاربر در حال انتظار وارد کنید. برای پیش‌فرض خالی بگذارید.',
		'Enter a watermark for the response. Leave empty for none.':
			'یک واترمارک برای پاسخ وارد کنید. برای هیچ‌کدام خالی بگذارید.',
		...
		'Enter api auth string (e.g. username:password)':
			'رشته احراز هویت api را وارد کنید (مثلا username:password)',
		...
		'Enter content for the pending user info overlay. Leave empty for default.':
			'محتوا برای پوشش اطلاعات کاربر در حال انتظار وارد کنید. برای پیش‌فرض خالی بگذارید.',
		...
		'Enter JSON config (e.g., {"disable_links": true})':
			'پیکربندی JSON را وارد کنید (مثلاً، {"disable_links": true})',
		...
		'Enter proxy URL (e.g. https://user:password@host:port)':
			'آدرس پراکسی را وارد کنید (مثال: https://user:password@host:port)',
		...
		'Enter the public URL of your WebUI. This URL will be used to generate links in the notifications.':
			'آدرس عمومی رابط کاربری وب خود را وارد کنید. این آدرس برای تولید پیوندها در اعلان‌ها استفاده خواهد شد.',
		...
		'Enter URL (e.g. http://127.0.0.1:7860/)':
			'مقدار URL را وارد کنید (مثال http://127.0.0.1:7860/)',
		'Enter URL (e.g. http://localhost:11434)':
			'مقدار URL را وارد کنید (مثال http://localhost:11434)',
		...
		'Enter Yacy URL (e.g. http://yacy.example.com:8090)':
			'آدرس URL یاسی (Yacy) را وارد کنید (مثلاً http://yacy.example.com:8090)',
		...
		"Error: A model with the ID '{{modelId}}' already exists. Please select a different ID to proceed.":
			"خطا: مدلی با شناسه '{{modelId}}' قبلاً وجود دارد. لطفاً برای ادامه، یک شناسه متفاوت انتخاب کنید.",
		'Error: Model ID cannot be empty. Please enter a valid ID to proceed.':
			'خطا: شناسه مدل نمی‌تواند خالی باشد. لطفاً برای ادامه، یک شناسه معتبر وارد کنید.',
		Evaluations: yt,
		'Event created': '',
		'Event deleted': '',
		'Event title': '',
		...
		'Example: (&(objectClass=inetOrgPerson)(uid=%s))':
			'مثال: (&(objectClass=inetOrgPerson)(uid=%s))',
		'Example: ALL': 'مثال: ALL',
		'Example: mail': 'مثال: mail',
		'Example: ou=users,dc=foo,dc=example': 'مثال: ou=users,dc=foo,dc=example',
		'Example: sAMAccountName or uid or userPrincipalName':
			'مثال: sAMAccountName یا uid یا userPrincipalName',
		'Exceeded the number of seats in your license. Please contact support to increase the number of seats.':
			'تعداد جایگاه‌های مجاز در مجوز شما تمام شده است. لطفاً برای افزایش تعداد جایگاه‌ها با پشتیبانی تماس بگیرید.',
		Exclude: ft,
		'Execute code': '',
		...
		'Failed to connect to {{URL}} OpenAPI tool server':
			'خطا در اتصال به سرور ابزار OpenAPI {{URL}}',
		...
		'Failed to extract content from the file: {{error}}':
			'استخراج محتوا از فایل ناموفق بود: {{error}}',
		...
		'File size should not exceed {{maxSize}} MB.':
			'حجم پرونده نبایستی از {{maxSize}} MB بیشتر باشد.',
		...
		'Fingerprint spoofing detected: Unable to use initials as avatar. Defaulting to default profile image.':
			'فانگ سرفیس شناسایی شد: نمی توان از نمایه شما به عنوان آواتار استفاده کرد. پیش فرض به عکس پروفایل پیش فرض برگشت داده شد.',
		...
		'Force OCR on all pages of the PDF. This can lead to worse results if you have good text in your PDFs. Defaults to False.':
			'اجبار به OCR در تمام صفحات PDF. این ممکن است در صورت داشتن متن خوب در PDFها منجر به نتایج بدتری شود. پیش‌فرض: False.',
		...
		'Format the lines in the output. Defaults to False. If set to True, the lines will be formatted to detect inline math and styles.':
			'قالب‌بندی خطوط در خروجی. پیش‌فرض: False. اگر روی True تنظیم شود، خطوط برای تشخیص ریاضیات و استایل‌های درون‌خطی قالب‌بندی خواهند شد.',
		...
		'Forwards system user OAuth access token to authenticate':
			'ارسال توکن دسترسی OAuth کاربر سیستم برای احراز هویت',
		'Forwards system user session credentials to authenticate':
			'اعتبارنامه‌های نشست کاربر سیستم را برای احراز هویت ارسال می‌کند',
		Fr_day_of_week: _t,
		...
		'I acknowledge that I have read and I understand the implications of my action. I am aware of the risks associated with executing arbitrary code and I have verified the trustworthiness of the source.':
			'من تأیید می‌کنم که پیامدهای اقدام خود را خوانده و درک کرده‌ام. از خطرات مرتبط با اجرای کد دلخواه آگاه هستم و اعتبار منبع را تأیید کرده‌ام.',
		ID: mo,
		...
		'Include `--api-auth` flag when running stable-diffusion-webui':
			'هنگام اجرای stable-diffusion-webui پرچم `--api-auth` را اضافه کنید',
		...
		'Influences how quickly the algorithm responds to feedback from the generated text. A lower learning rate will result in slower adjustments, while a higher learning rate will make the algorithm more responsive.':
			'تأثیر می‌گذارد که الگوریتم چقدر سریع به بازخورد متن تولید شده پاسخ می‌دهد. نرخ یادگیری پایین‌تر منجر به تنظیمات کندتر می‌شود، در حالی که نرخ یادگیری بالاتر الگوریتم را پاسخگوتر می‌کند.',
		Info: Ao,
		Initials: bo,
		'Inject file content into conversation context': '',
		'Inject the entire content as context for comprehensive processing, this is recommended for complex queries.':
			'کل محتوا را به عنوان زمینه برای پردازش جامع تزریق کنید، این برای پرس‌وجوهای پیچیده توصیه می‌شود.',
		Input: wo,
		...
		'Invalid JSON format for ComfyUI Edit Workflow.':
			'قالب JSON برای گردش کار ویرایش ComfyUI نامعتبر است.',
		...
		'Leave empty to include all models from "{{url}}" endpoint':
			'برای گنجاندن همه مدل‌ها از نقطه پایانی "{{url}}" خالی بگذارید',
		'Leave empty to include all models from "{{url}}/api/tags" endpoint':
			'برای شامل شدن همه مدل‌ها از نقطه پایانی "{{url}}/api/tags" خالی بگذارید',
		'Leave empty to include all models from "{{url}}/models" endpoint':
			'برای شامل شدن همه مدل‌ها از نقطه پایانی "{{url}}/models" خالی بگذارید',
		'Leave empty to include all models or select specific models':
			'برای شامل شدن همه مدل‌ها خالی بگذارید یا مدل‌های خاص را انتخاب کنید',
		...
		'Leave empty to use the default model (voxtral-mini-latest).':
			'برای استفاده از مدل پیش‌فرض (voxtral-mini-latest) خالی بگذارید.',
		'Leave empty to use the default prompt, or enter a custom prompt':
			'برای استفاده از پرامپت پیش‌فرض خالی بگذارید، یا یک پرامپت سفارشی وارد کنید',
		'Leave model field empty to use the default model.':
			'برای استفاده از مدل پیش‌فرض، فیلد مدل را خالی بگذارید.',
		Legacy: xo,
		lexical: Fo,
		License: _o,
		...
		'LLMs can make mistakes. Verify important information.':
			'مدل‌های زبانی بزرگ می‌توانند اشتباه کنند. اطلاعات مهم را راستی‌آزمایی کنید.',
		Loader: Go,
		...
		'Make sure to export a workflow.json file as API format from ComfyUI.':
			'مطمئن شوید که یک فایل workflow.json را به عنوان قالب API از ComfyUI صادر کنید.',
		Male: jo,
		Manage: Qo,
		'Manage Connections': '',
		...
		'Maximum of 3 models can be downloaded simultaneously. Please try again later.':
			'حداکثر 3 مدل را می توان به طور همزمان دانلود کرد. لطفاً بعداً دوباره امتحان کنید.',
		May: $o,
		MBR: en,
		MCP: tn,
		'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the Open WebUI team, making it the more reliable option for compatibility.':
			'پشتیبانی MCP آزمایشی است و مشخصات آن اغلب تغییر می‌کند، که می‌تواند منجر به ناسازگاری شود. پشتیبانی از مشخصات OpenAPI مستقیماً توسط تیم Open WebUI نگهداری می‌شود و آن را به گزینه قابل اعتماد‌تری برای سازگاری تبدیل می‌کند.',
		Medium: on,
		...
		'Memories accessible by LLMs will be shown here.':
			'حافظه های دسترسی به LLMs در اینجا نمایش داده می شوند.',
		Memory: rn,
		...
		'Message rating should be enabled to use this feature':
			'برای استفاده از این ویژگی باید امتیازدهی پیام‌ها فعال باشد',
		messages: cn,
		Messages: dn,
		"Messages you send after creating your link won't be shared. Users with the URL will be able to view the shared chat.":
			'پیام های شما بعد از ایجاد لینک شما به اشتراک نمی گردد. کاربران با لینک URL می توانند چت اشتراک را مشاهده کنند.',
		...
		'MinerU API Key required for Cloud API mode.':
			'کلید API MinerU برای حالت Cloud API مورد نیاز است.',
		...
		"Model '{{modelName}}' has been successfully downloaded.":
			"مدل '{{modelName}}' با موفقیت دانلود شد.",
		"Model '{{modelTag}}' is already in queue for downloading.":
			"مدل '{{modelTag}}' در حال حاضر در صف برای دانلود است.",
		...
		'Model can execute code and perform calculations':
			'مدل می‌تواند کد را اجرا کرده و محاسبات را انجام دهد',
		'Model can generate images based on text prompts':
			'مدل می‌تواند تصاویر را بر اساس پرامپت‌های متنی تولید کند',
		...
		'Model filesystem path detected. Model shortname is required for update, cannot continue.':
			'مسیر فایل سیستم مدل یافت شد. برای بروزرسانی نیاز است نام کوتاه مدل وجود داشته باشد.',
		...
		'Model name already exists, please choose a different one':
			'نام مدل قبلاً وجود دارد، لطفاً یک نام دیگر انتخاب کنید',
		...
		'Name and ID are required, please fill them out':
			'نام و شناسه مورد نیاز هستند، لطفاً آنها را پر کنید',
		...
		'No inference engine with management support found':
			'موتور استنتاج با پشتیبانی مدیریت یافت نشد',
		'No kernel': '',
		...
		'Note: If you set a minimum score, the search will only return documents with a score greater than or equal to the minimum score.':
			'توجه: اگر حداقل نمره را تعیین کنید، جستجو تنها اسنادی را با نمره بیشتر یا برابر با حداقل نمره باز می گرداند.',
		Notes: Un,
		...
		'Only active when "Paste Large Text as File" setting is toggled on.':
			'فقط زمانی فعال است که تنظیم «چسباندن متن بزرگ به عنوان فایل» روشن باشد.',
		'Only active when the chat input is in focus and an LLM is generating a response.':
			'فقط زمانی فعال است که ورودی چت در فوکوس باشد و یک LLM در حال تولید پاسخ باشد.',
		'Only active when the chat input is in focus.': 'فقط زمانی فعال است که ورودی چت در فوکوس باشد.',
		'Only alphanumeric characters and hyphens are allowed':
			'فقط حروف الفبا، اعداد و خط تیره مجاز هستند',
		'Only alphanumeric characters and hyphens are allowed in the command string.':
			'فقط کاراکترهای الفبایی و خط فاصله در رشته فرمان مجاز هستند.',
		'Only can be triggered when the chat input is in focus.':
			'فقط زمانی قابل اجرا است که ورودی چت در فوکوس باشد.',
		'Only collections can be edited, create a new knowledge base to edit/add documents.':
			'فقط مجموعه‌ها قابل ویرایش هستند، برای ویرایش/افزودن اسناد یک پایگاه دانش جدید ایجاد کنید.',
		...
		'Only select users and groups with permission can access':
			'فقط کاربران و گروه‌های دارای مجوز می‌توانند دسترسی داشته باشند',
		'Only sync new/updated chats': '',
		'Oops! Looks like the URL is invalid. Please double-check and try again.':
			'اوه! به نظر می رسد URL نامعتبر است. لطفاً دوباره بررسی کنید و دوباره امتحان کنید.',
		'Oops! There are files still uploading. Please wait for the upload to complete.':
			'اوه! هنوز فایل‌هایی در حال آپلود هستند. لطفاً منتظر تکمیل آپلود بمانید.',
		'Oops! There was an error in the previous response.': 'اوه! در پاسخ قبلی خطایی رخ داد.',
		"Oops! You're using an unsupported method (frontend only). Please serve the WebUI from the backend.":
			'اوه! شما از یک روش پشتیبانی نشده (فقط frontend) استفاده می کنید. لطفاً WebUI را از بکند اجرا کنید.',
		...
		'Open WebUI can use tools provided by any OpenAPI server.':
			'Open WebUI می‌تواند از ابزارهای ارائه شده توسط هر سرور OpenAPI استفاده کند.',
		'Open WebUI uses faster-whisper internally.':
			'Open WebUI به صورت داخلی از faster-whisper استفاده می‌کند.',
		'Open WebUI uses SpeechT5 and CMU Arctic speaker embeddings.':
			'Open WebUI از SpeechT5 و جاسازی‌های گوینده CMU Arctic استفاده می‌کند.',
		'Open WebUI version': '',
		'Open WebUI version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})':
			'نسخه Open WebUI (v{{OPEN_WEBUI_VERSION}}) پایین‌تر از نسخه مورد نیاز (v{{REQUIRED_VERSION}}) است',
		OpenAI: _n,
		...
		'Pipelines are a plugin system with arbitrary code execution —':
			'Pipelines یک سیستم افزونه با امکان اجرای دلخواه کد است —',
		...
		'Please do not close the settings page while loading the model.':
			'لطفاً در حین بارگیری مدل، صفحه تنظیمات را نبندید.',
		...
		'Please save the connection to persist the OAuth client information and do not change the ID':
			'لطفاً اتصال را ذخیره کنید تا اطلاعات کلاینت OAuth ماندگار شود و شناسه را تغییر ندهید',
		...
		'Prefix ID is used to avoid conflicts with other connections by adding a prefix to the model IDs - leave empty to disable':
			'شناسه پیشوند برای جلوگیری از تداخل با سایر اتصالات با افزودن پیشوند به شناسه‌های مدل استفاده می‌شود - برای غیرفعال کردن خالی بگذارید',
		...
		'Reduces the probability of generating nonsense. A higher value (e.g. 100) will give more diverse answers, while a lower value (e.g. 10) will be more conservative.':
			'احتمال تولید محتوای بی‌معنی را کاهش می‌دهد. مقدار بالاتر (مثلاً 100) پاسخ‌های متنوع‌تری می‌دهد، در حالی که مقدار پایین‌تر (مثلاً 10) محافظه‌کارانه‌تر خواهد بود.',
		'Refer to yourself as "User" (e.g., "User is learning Spanish")':
			'به خود به عنوان "کاربر" اشاره کنید (مثلاً، "کاربر در حال یادگیری اسپانیایی است")',
		...
		'Response notifications cannot be activated as the website permissions have been denied. Please visit your browser settings to grant the necessary access.':
			'اعلان‌های پاسخ نمی‌توانند فعال شوند زیرا مجوزهای وب‌سایت رد شده‌اند. لطفاً تنظیمات مرورگر خود را برای اعطای دسترسی لازم بررسی کنید.',
		...
		"Saving chat logs directly to your browser's storage is no longer supported. Please take a moment to download and delete your chat logs by clicking the button below. Don't worry, you can easily re-import your chat logs to the backend through":
			'ذخیره گزارش‌های چت مستقیماً در حافظه مرورگر شما دیگر پشتیبانی نمی‌شود. لطفاً با کلیک بر روی دکمه زیر، چند لحظه برای دانلود و حذف گزارش های چت خود وقت بگذارید. نگران نباشید، شما به راحتی می توانید گزارش های چت خود را از طریق بکند دوباره وارد کنید',
		Schedule: ja,
		...
		'Select how to split message text for TTS requests':
			'نحوه تقسیم متن پیام برای درخواست‌های TTS را انتخاب کنید',
		...
		'Selected model(s) do not support image inputs':
			'مدل) های (انتخاب شده ورودیهای تصویر را پشتیبانی نمیکند',
		'Selected Models': '',
		semantic: es,
		Send: ts,
		...
		'Set the number of layers, which will be off-loaded to GPU. Increasing this value can significantly improve performance for models that are optimized for GPU acceleration but may also consume more power and GPU resources.':
			'تعداد لایه‌هایی را که به GPU منتقل می‌شوند تنظیم کنید. افزایش این مقدار می‌تواند عملکرد مدل‌هایی که برای شتاب‌دهی GPU بهینه‌سازی شده‌اند را به طور قابل توجهی بهبود بخشد اما ممکن است مصرف برق و منابع GPU را نیز افزایش دهد.',
		'Set the number of worker threads used for computation. This option controls how many threads are used to process incoming requests concurrently. Increasing this value can improve performance under high concurrency workloads but may also consume more CPU resources.':
			'تعداد نخ‌های کارگر مورد استفاده برای محاسبات را تنظیم کنید. این گزینه کنترل می‌کند که چند نخ برای پردازش همزمان درخواست‌های ورودی استفاده می‌شود. افزایش این مقدار می‌تواند عملکرد را در بارهای کاری با همزمانی بالا بهبود بخشد اما ممکن است منابع CPU بیشتری مصرف کند.',
		'Set Voice': 'تنظیم صدا',
		...
		'Sets a flat bias against tokens that have appeared at least once. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'یک بایاس ثابت در برابر توکن‌هایی که حداقل یک بار ظاهر شده‌اند تنظیم می‌کند. مقدار بالاتر (مثلاً 1.5) تکرارها را شدیدتر جریمه می‌کند، در حالی که مقدار پایین‌تر (مثلاً 0.9) آسان‌گیرتر خواهد بود. در 0، غیرفعال می‌شود.',
		'Sets a scaling bias against tokens to penalize repetitions, based on how many times they have appeared. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'یک بایاس مقیاس‌پذیر در برابر توکن‌ها برای جریمه کردن تکرارها، بر اساس تعداد دفعات ظاهر شدن آنها تنظیم می‌کند. مقدار بالاتر (مثلاً 1.5) تکرارها را شدیدتر جریمه می‌کند، در حالی که مقدار پایین‌تر (مثلاً 0.9) آسان‌گیرتر خواهد بود. در 0، غیرفعال می‌شود.',
		'Sets how far back for the model to look back to prevent repetition.':
			'تنظیم می‌کند که مدل چقدر به عقب نگاه کند تا از تکرار جلوگیری شود.',
		'Sets the random number seed to use for generation. Setting this to a specific number will make the model generate the same text for the same prompt.':
			'عدد تصادفی اولیه را برای تولید تنظیم می‌کند. تنظیم این به یک عدد خاص باعث می‌شود مدل برای پرامپت یکسان، متن یکسانی تولید کند.',
		'Sets the size of the context window used to generate the next token.':
			'اندازه پنجره متن مورد استفاده برای تولید توکن بعدی را تنظیم می‌کند.',
		'Sets the stop sequences to use. When this pattern is encountered, the LLM will stop generating text and return. Multiple stop patterns may be set by specifying multiple separate stop parameters in a modelfile.':
			'توالی‌های توقف مورد استفاده را تنظیم می‌کند. وقتی این الگو مشاهده شود، LLM تولید متن را متوقف کرده و برمی‌گردد. الگوهای توقف متعدد می‌توانند با مشخص کردن پارامترهای توقف جداگانه متعدد در فایل مدل تنظیم شوند.',
		Setting: as,
		Settings: ss,
		...
		'Significantly improves accuracy by using an LLM to enhance tables, forms, inline math, and layout detection. Will increase latency. Defaults to False.':
			'با استفاده از یک LLM برای بهبود جداول، فرم‌ها، ریاضیات درون‌خطی و تشخیص چیدمان، دقت را به طور قابل توجهی افزایش می‌دهد. باعث افزایش تأخیر خواهد شد. پیش‌فرض: False.',
		...
		'Skip the cache and re-run the inference. Defaults to False.':
			'کش را رد کرده و استنتاج را مجدداً اجرا کنید. پیش‌فرض: False.',
		...
		'Strip existing OCR text from the PDF and re-run OCR. Ignored if Force OCR is enabled. Defaults to False.':
			'حذف متن OCR موجود از PDF و اجرای مجدد OCR. اگر «اجبار به OCR» فعال باشد نادیده گرفته می‌شود. پیش‌فرض: False.',
		...
		'Tail free sampling is used to reduce the impact of less probable tokens from the output. A higher value (e.g., 2.0) will reduce the impact more, while a value of 1.0 disables this setting.':
			'نمونه‌برداری دنباله آزاد برای کاهش تأثیر توکن‌های کم احتمال‌تر از خروجی استفاده می‌شود. مقدار بالاتر (مثلاً 2.0) تأثیر را بیشتر کاهش می‌دهد، در حالی که مقدار 1.0 این تنظیم را غیرفعال می‌کند.',
		...
		'The Application Account DN you bind with for search':
			'DN حساب برنامه که برای جستجو به آن متصل می‌شوید',
		...
		'The batch size determines how many text requests are processed together at once. A higher batch size can increase the performance and speed of the model, but it also requires more memory.':
			'اندازه دسته تعیین می‌کند که چند درخواست متنی همزمان پردازش می‌شوند. اندازه دسته بزرگتر می‌تواند عملکرد و سرعت مدل را افزایش دهد، اما به حافظه بیشتری نیاز دارد.',
		'The developers behind this plugin are passionate volunteers from the community. If you find this plugin helpful, please consider contributing to its development.':
			'توسعه‌دهندگان این افزونه داوطلبان مشتاق از جامعه هستند. اگر این افزونه را مفید می‌دانید، لطفاً در توسعه آن مشارکت کنید.',
		'The evaluation leaderboard is based on the Elo rating system and is updated in real-time.':
			'تابلوی امتیازات ارزیابی بر اساس سیستم رتبه‌بندی Elo است و در زمان واقعی به‌روز می‌شود.',
		'The format to return a response in. Format can be json or a JSON schema.':
			'قالبی که باید پاسخ در آن برگردانده شود. قالب می‌تواند json یا یک شمای JSON باشد.',
		'The height in pixels to compress images to. Leave empty for no compression.':
			'ارتفاع بر حسب پیکسل برای فشرده‌سازی تصاویر. برای عدم فشرده‌سازی خالی بگذارید.',
		'The language of the input audio. Supplying the input language in ISO-639-1 (e.g. en) format will improve accuracy and latency. Leave blank to automatically detect the language.':
			'زبان صدای ورودی. ارائه زبان ورودی در قالب ISO-639-1 (مثلاً en) دقت و تأخیر را بهبود می‌بخشد. برای تشخیص خودکار زبان، خالی بگذارید.',
		'The LDAP attribute that maps to the mail that users use to sign in.':
			'ویژگی LDAP که به ایمیلی که کاربران برای ورود استفاده می‌کنند نگاشت می‌شود.',
		'The LDAP attribute that maps to the username that users use to sign in.':
			'ویژگی LDAP که به نام کاربری که کاربران برای ورود استفاده می‌کنند نگاشت می‌شود.',
		'The leaderboard is currently in beta, and we may adjust the rating calculations as we refine the algorithm.':
			'تابلوی امتیازات در حال حاضر در نسخه بتا است و ممکن است محاسبات رتبه‌بندی را با بهبود الگوریتم تنظیم کنیم.',
		'The maximum file size in MB. If the file size exceeds this limit, the file will not be uploaded.':
			'حداکثر اندازه فایل به مگابایت. اگر اندازه فایل از این حد بیشتر باشد، فایل آپلود نخواهد شد.',
		'The maximum number of files that can be used at once in chat. If the number of files exceeds this limit, the files will not be uploaded.':
			'حداکثر تعداد فایل‌هایی که می‌توانند همزمان در چت استفاده شوند. اگر تعداد فایل‌ها از این حد بیشتر باشد، فایل‌ها آپلود نخواهند شد.',
		"The output format for the text. Can be 'json', 'markdown', or 'html'. Defaults to 'markdown'.":
			"قالب خروجی برای متن. می‌تواند 'json'، 'markdown' یا 'html' باشد. پیش‌فرض: 'markdown'.",
		"The passwords you entered don't quite match. Please double-check and try again.":
			'رمزهای عبوری که وارد کردید کاملاً مطابقت ندارند. لطفاً دوباره بررسی کرده و تلاش کنید.',
		'The score should be a value between 0.0 (0%) and 1.0 (100%).':
			'امتیاز باید مقداری بین 0.0 (0%) و 1.0 (100%) باشد.',
		'The stream delta chunk size for the model. Increasing the chunk size will make the model respond with larger pieces of text at once.':
			'اندازه دلتا تکه جریانی برای مدل. افزایش اندازه تکه باعث می‌شود مدل با قطعات بزرگ‌تری از متن به یکباره پاسخ دهد.',
		'The temperature of the model. Increasing the temperature will make the model answer more creatively.':
			'دمای مدل. افزایش دما باعث می‌شود مدل خلاقانه‌تر پاسخ دهد.',
		'The Weight of BM25 Hybrid Search. 0 more semantic, 1 more lexical. Default 0.5':
			'وزن جستجوی ترکیبی BM25. ۰ معنایی‌تر، ۱ لغوی‌تر. پیش‌فرض ۰.۵',
		'The width in pixels to compress images to. Leave empty for no compression.':
			'عرض بر حسب پیکسل برای فشرده‌سازی تصاویر. برای عدم فشرده‌سازی خالی بگذارید.',
		Theme: Ds,
		'There was an error syncing your stats. Please try again.': '',
		'Thinking...': 'در حال فکر کردن...',
		'This action cannot be undone. Do you wish to continue?':
			'این عمل قابل بازگشت نیست. آیا می‌خواهید ادامه دهید؟',
		'This channel was created on {{createdAt}}. This is the very beginning of the {{channelName}} channel.':
			'این کانال در {{createdAt}} ایجاد شد. این آغاز کانال {{channelName}} است.',
		"This chat won't appear in history and your messages will not be saved.":
			'این چت در تاریخچه ظاهر نمی‌شود و پیام‌های شما ذخیره نخواهند شد.',
		'This ensures that your valuable conversations are securely saved to your backend database. Thank you!':
			'این اطمینان می‌دهد که مکالمات ارزشمند شما به طور امن در پایگاه داده پشتیبان ذخیره می‌شوند. متشکریم!',
		...
		'This feature is experimental and may be modified or discontinued without notice.':
			'این ویژگی آزمایشی است و ممکن است بدون اطلاع قبلی تغییر یا متوقف شود.',
		'This folder is empty': '',
		'This is a default user permission and will remain enabled.':
			'این یک مجوز کاربر پیش‌فرض است و فعال باقی خواهد ماند.',
		'This is an experimental feature, it may not function as expected and is subject to change at any time.':
			'این یک ویژگی آزمایشی است، ممکن است طبق انتظار کار نکند و در هر زمان ممکن است تغییر کند.',
		'This model is not publicly available. Please select another model.':
			'این مدل به صورت عمومی در دسترس نیست. لطفاً مدل دیگری انتخاب کنید.',
		'This option controls how long the model will stay loaded into memory following the request (default: 5m)':
			'این گزینه مدت زمانی را کنترل می‌کند که مدل پس از درخواست در حافظه بارگذاری شده باقی می‌ماند (پیش‌فرض: ۵ دقیقه)',
		'This option controls how many tokens are preserved when refreshing the context. For example, if set to 2, the last 2 tokens of the conversation context will be retained. Preserving context can help maintain the continuity of a conversation, but it may reduce the ability to respond to new topics.':
			'این گزینه کنترل می‌کند که هنگام تازه‌سازی متن، چند توکن حفظ شوند. برای مثال، اگر روی 2 تنظیم شود، 2 توکن آخر متن مکالمه حفظ خواهند شد. حفظ متن می‌تواند به حفظ پیوستگی مکالمه کمک کند، اما ممکن است توانایی پاسخ به موضوعات جدید را کاهش دهد.',
		'This option enables or disables the use of the reasoning feature in Ollama, which allows the model to think before generating a response. When enabled, the model can take a moment to process the conversation context and generate a more thoughtful response.':
			'این گزینه استفاده از ویژگی استدلال در اُلاما را فعال یا غیرفعال می‌کند، که به مدل اجازه می‌دهد قبل از تولید پاسخ فکر کند. هنگامی که فعال باشد، مدل می‌تواند لحظه‌ای را برای پردازش زمینه مکالمه صرف کند و یک پاسخ متفکرانه‌تر تولید کند.',
		'This option sets the maximum number of tokens the model can generate in its response. Increasing this limit allows the model to provide longer answers, but it may also increase the likelihood of unhelpful or irrelevant content being generated.':
			'این گزینه حداکثر تعداد توکن‌هایی را که مدل می‌تواند در پاسخ خود تولید کند تنظیم می‌کند. افزایش این محدودیت به مدل اجازه می‌دهد پاسخ‌های طولانی‌تری ارائه دهد، اما ممکن است احتمال تولید محتوای بی‌فایده یا نامربوط را نیز افزایش دهد.',
		'This option will delete all existing files in the collection and replace them with newly uploaded files.':
			'این گزینه تمام فایل‌های موجود در مجموعه را حذف کرده و با فایل‌های جدید آپلود شده جایگزین می‌کند.',
		...
		'This will delete <strong>{{NAME}}</strong> and <strong>all its contents</strong>.':
			'این <strong>{{NAME}}</strong> و <strong>تمام محتویات آن</strong> را حذف خواهد کرد.',
		'This will delete all models including custom models':
			'این همه مدل‌ها از جمله مدل‌های سفارشی را حذف خواهد کرد',
		'This will delete all models including custom models and cannot be undone.':
			'این همه مدل‌ها از جمله مدل‌های سفارشی را حذف خواهد کرد و قابل بازگشت نیست.',
		'This will permanently delete the calendar "{{name}}" and all its events. This action cannot be undone.':
			'',
		'This will reset the knowledge base and sync all files. Do you wish to continue?':
			'این پایگاه دانش را بازنشانی کرده و همه فایل‌ها را همگام‌سازی خواهد کرد. آیا می‌خواهید ادامه دهید؟',
		...
		'To access the available model names for downloading,':
			'برای دسترسی به نام مدل های موجود برای دانلود،',
		'To access the GGUF models available for downloading,':
			'برای دسترسی به مدل‌های GGUF موجود برای دانلود،',
		'To access the WebUI, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.':
			'برای دسترسی به رابط کاربری وب، لطفاً با مدیر تماس بگیرید. مدیران می‌توانند وضعیت کاربران را از پنل مدیریت مدیریت کنند.',
		'To attach knowledge base here, add them to the "Knowledge" workspace first.':
			'برای اتصال پایگاه دانش در اینجا، ابتدا آنها را به فضای کاری "دانش" اضافه کنید.',
		'To learn more about available endpoints, visit our documentation.':
			'برای کسب اطلاعات بیشتر در مورد نقاط پایانی موجود، به مستندات ما مراجعه کنید.',
		'To select skills here, add them to the "Skills" workspace first.': '',
		'To select toolkits here, add them to the "Tools" workspace first.':
			'برای انتخاب ابزارها در اینجا، ابتدا آنها را به فضای کاری "ابزارها" اضافه کنید.',
		...
		'Tools are a function calling system with arbitrary code execution':
			'ابزارها یک سیستم فراخوانی تابع با اجرای کد دلخواه هستند',
		'Tools Function Calling Prompt': 'پرامپت فراخوانی تابع ابزارها',
		'Tools have a function calling system that allows arbitrary code execution.':
			'ابزارها دارای سیستم فراخوانی تابع هستند که اجازه اجرای کد دلخواه را می‌دهد.',
		...
		'Try adjusting your search or filter to find what you are looking for.':
			'برای یافتن آنچه به دنبالش هستید، سعی کنید جستجو یا فیلتر خود را تنظیم کنید.',
		...
		'Type Hugging Face Resolve (Download) URL':
			'مقدار URL دانلود (Resolve) Hugging Face را وارد کنید',
		...
		'Update for the latest features and improvements.':
			'برای آخرین ویژگی‌ها و بهبودها به‌روزرسانی کنید.',
		...
		'Upgrade to a licensed plan for enhanced capabilities, including custom theming and branding, and dedicated support.':
			'برای قابلیت‌های پیشرفته، از جمله تم و برندسازی سفارشی و پشتیبانی اختصاصی، به طرح دارای مجوز ارتقا دهید.',
		Upload: ar,
		...
		'Upload Progress: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)':
			'پیشرفت آپلود: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}٪)',
		...
		"Use '#' in the prompt input to load and include your knowledge.":
			"از '#' در ورودی پرامپت برای بارگیری و شامل کردن دانش خود استفاده کنید.",
		'Use /v1/chat/completions endpoint instead of /v1/audio/transcriptions for potentially better accuracy.':
			'از نقطه پایانی /v1/chat/completions به جای /v1/audio/transcriptions برای دقت بالقوه بهتر استفاده کنید.',
		...
		'Use proxy designated by http_proxy and https_proxy environment variables to fetch page contents.':
			'از پراکسی تعیین شده توسط متغیرهای محیطی http_proxy و https_proxy برای دریافت محتوای صفحه استفاده کنید.',
		user: lr,
		User: cr,
		'User Activity': '',
		...
		'Uses DefaultAzureCredential to authenticate':
			'از DefaultAzureCredential برای احراز هویت استفاده می‌کند',
		...
		'Using the default arena model with all models. Click the plus button to add custom models.':
			'در حال استفاده از مدل آرنا با همهٔ مدل‌های دیگر به طور پیش‌فرض. برای افزودن مدل‌های سفارشی، روی دکمه به‌علاوه کلیک کنید.',
		...
		'Version {{selectedVersion}} of {{totalVersions}}':
			'نسخهٔ {{selectedVersion}} از {{totalVersions}}',
		'Version deleted': '',
		...
		'Warning: Enabling this will allow users to upload arbitrary code on the server.':
			'هشدار: فعال کردن این گزینه به کاربران اجازه می‌دهد کد دلخواه را روی سرور آپلود کنند.',
		'Warning: Jupyter execution enables arbitrary code execution, posing severe security risks—proceed with extreme caution.':
			'هشدار: اجرای ژوپیتر امکان اجرای کد دلخواه را فراهم می‌کند که خطرات امنیتی جدی به همراه دارد - با احتیاط زیاد ادامه دهید.',
		We_day_of_week: Sr,
		Web: Ir,
		'Web API': 'API وب',
		...
		'WebUI will make requests to "{{url}}/api/chat"':
			'WebUI به "{{url}}/api/chat" درخواست خواهد داد',
		'WebUI will make requests to "{{url}}/chat/completions"':
			'WebUI به "{{url}}/chat/completions" درخواست خواهد داد',
		Week: Cr,
		Weekly: Pr,
		...
		'When enabled, the model will respond to each chat message in real-time, generating a response as soon as the user sends a message. This mode is useful for live chat applications, but may impact performance on slower hardware.':
			'وقتی فعال باشد، مدل به هر پیام گفتگو در زمان واقعی پاسخ می‌دهد و به محض ارسال پیام توسط کاربر، پاسخی تولید می‌کند. این حالت برای برنامه‌های گفتگوی زنده مفید است، اما ممکن است در سخت‌افزارهای کندتر بر عملکرد تأثیر بگذارد.',
		...
		'Whether to paginate the output. Each page will be separated by a horizontal rule and page number. Defaults to False.':
			'آیا خروجی صفحه‌بندی شود یا خیر. هر صفحه با یک خط افقی و شماره صفحه از هم جدا می‌شود. پیش‌فرض: False.',
		...
		'Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text.':
			'با top-k همکاری می‌کند. مقدار بالاتر (مثلاً 0.95) منجر به متن متنوع‌تر می‌شود، در حالی که مقدار پایین‌تر (مثلاً 0.5) متن متمرکزتر و محافظه‌کارانه‌تری تولید می‌کند.',
		Workspace: Mr,
		'Workspace Permissions': 'مجوزهای محیط کار',
		Write: Rr,
		'Write a summary in 50 words that summarizes {{topic}}.':
			'خلاصه ای در 50 کلمه بنویسید که [موضوع یا کلمه کلیدی] را خلاصه کند.',
...
مثال: شما ماریو از بازی Super Mario Bros هستید و به عنوان دستیار عمل می‌کنید.`,
		...
		'You are currently using a trial license. Please contact support to upgrade your license.':
			'شما در حال حاضر از نسخه آزمایشی استفاده می‌کنید. لطفاً برای ارتقای مجوز خود با پشتیبانی تماس بگیرید.',
		'You can only chat with a maximum of {{maxCount}} file(s) at a time.':
			'شما در هر زمان نهایتا می‌توانید با {{maxCount}} پرونده گفتگو کنید.',
		"You can personalize your interactions with LLMs by adding memories through the 'Manage' button below, making them more helpful and tailored to you.":
			"شما می‌توانید تعاملات خود با LLM‌ها را با افزودن خاطرات از طریق دکمه 'مدیریت' در زیر شخصی‌سازی کنید تا آنها مفیدتر و متناسب‌تر با شما شوند.",
		...
		'You do not have permission to send messages in this thread.':
			'شما اجازه ارسال پیام در این رشته را ندارید.',
		...
		'Your account status is currently pending activation.':
			'وضعیت حساب شما در حال حاضر در انتظار فعال‌سازی است.',
...
//# sourceMappingURL=Cec_R0fN.js.map
