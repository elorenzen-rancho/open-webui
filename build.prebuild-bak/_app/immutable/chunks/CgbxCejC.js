...
import { E as me, L as ue } from './COV9u0MG.js';
const pe = 36,
	X = 1,
	fe = 2,
	b = 3,
...
function K(t) {
	return new me((e) => {
		...
		if ((e.advance(), _(r, S))) {
			...
			let n = P(e, '');
			e.next == 36 && (e.advance(), ze(e, n), e.acceptToken(b));
			...
			(e.advance(),
				t.treatBitsAsBytes
					? (g(e, n, t.backslashEscapes), e.acceptToken(Pe))
					: (D(e, n), e.acceptToken(R)));
		} else if (
			(r == 48 && (e.next == 120 || e.next == 88)) ||
			((r == 120 || r == 88) && e.next == 39)
		) {
			let n = e.next == 39;
			for (e.advance(); je(e.next); ) e.advance();
...
function Ze(t, e) {
	...
	for (let n = a.firstChild, s = !1, c = null; n; n = n.nextSibling) {
		...
		(o && (r || (r = Object.create(null)), (r[o] = Ie(t, c))),
			(c = /Identifier$/.test(n.name) ? n : null));
	}
	return r;
}
function Ne(t, e, a) {
	return a.map((r) => ({
		...r,
		label: r.label[0] == t ? r.label : t + r.label + e,
		apply: void 0
	}));
}
const Ve = /^\w*$/,
...
class z {
	...
	child(e) {
		...
		return (
			r ||
			(e &&
				!this.list.some((n) => n.label == e) &&
				this.list.push(A(e, 'type', this.idQuote, this.idCaseInsensitive)),
			(a[e] = new z(this.idQuote, this.idCaseInsensitive)))
		);
	}
	maybeChild(e) {
	...
	addCompletions(e) {
		for (let a of e)
			this.addCompletion(
				typeof a == 'string' ? A(a, 'property', this.idQuote, this.idCaseInsensitive) : a
			);
	}
	addNamespace(e) {
		Array.isArray(e)
...
function Ae(t, e, a, r, n, s) {
	var c;
	let l =
			((c = s == null ? void 0 : s.spec.identifierQuotes) === null || c === void 0
				? void 0
				: c[0]) || '"',
	...
	return (
		...
		(p) => {
			...
			let w = d.list;
			if (
				(d == o && O && (w = w.concat(Object.keys(O).map((f) => ({ label: f, type: 'constant' })))),
				B)
			) {
				let f = B[0],
					k = G(f),
...
function Ge(t) {
	return t.schema
		? Ae(t.schema, t.tables, t.schemas, t.defaultTable, t.defaultSchema, t.dialect || U)
		: () => null;
}
function Ye(t) {
...
const it = m.define({
	...
	ot = m.define({
		...
		types:
			h + 'ascii bigint blob counter frozen inet list map static text timeuuid tuple uuid varint',
		slashComments: !0
	}),
	lt = m.define({
		keywords:
...
//# sourceMappingURL=CgbxCejC.js.map
