import './CWj6FrbW.js';
import './69_IOA4Y.js';
import { t as i, a as l, F as m } from './CD8kWLvT.js';
import { a as n, s as p, c } from './BfA5TzUS.js';
import { p as a } from './DbBXQmQ3.js';
var d = m(
	'<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125"></path></svg>'
);
function k(r, s) {
	let e = a(s, 'className', 8, 'w-4 h-4'),
		o = a(s, 'strokeWidth', 8, '1.5');
	var t = d();
	(i(() => {
		(n(t, 'stroke-width', o()), p(t, 0, c(e())));
	}),
		l(r, t));
}
export { k as P };
//# sourceMappingURL=DJIQFSNZ.js.map
