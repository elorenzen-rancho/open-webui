		...
		B = {
			...
			parse: o(function (t) {
				...
				for (var j in this.yy)
					Object.prototype.hasOwnProperty.call(this.yy, j) && (C.yy[j] = this.yy[j]);
				(b.setInput(t, C.yy),
					(C.yy.lexer = b),
					(C.yy.parser = this),
				...
				function z() {
					...
					return (
						(N = u.pop() || b.lex() || E),
						typeof N != 'number' &&
							(N instanceof Array && ((u = N), (N = u.pop())), (N = s.symbols_[N] || N)),
						N
					);
				}
				o(z, 'lex');
				for (var A, W, P, K, H = {}, X, V, et, Y; ; ) {
					if (
						...
						this.defaultActions[W]
							? (P = this.defaultActions[W])
							: ((A === null || typeof A > 'u') && (A = z()), (P = v[W] && v[W][A])),
						typeof P > 'u' || !P.length || !P[0])
					) {
						var Q = '';
						Y = [];
...
Expecting ` +
					...
					if (P[0] instanceof Array && P.length > 1)
						throw new Error(
							'Parse Error: multiple actions possible at state: ' + W + ', token: ' + A
						);
					switch (P[0]) {
						case 1:
							(i.push(A),
						...
						case 2:
							if (
								...
								(K = this.performAction.apply(H, [m, S, I, C.yy, P[1], p, n].concat(R))),
								typeof K < 'u')
							)
								return K;
							(V &&
								((i = i.slice(0, -1 * V * 2)), (p = p.slice(0, -1 * V)), (n = n.slice(0, -1 * V))),
...
` +
				...
				next: o(function () {
					...
					for (var p = this._currentRules(), n = 0; n < p.length; n++)
						if (
							((i = this._input.match(this.rules[p[n]])), i && (!s || i[0].length > s[0].length))
						) {
...
var xt = tt,
	...
	It = o((e, h, r, a) => {
		...
		if (!y) return;
		const f = ot(),
			{ look: d, handDrawnSeed: w, themeVariables: c } = f,
			k = ut(f.fontSize)[0] ?? St,
			g = d === 'handDrawn',
			_ = y.children ?? [],
			x = ((C = f.ishikawa) == null ? void 0 : C.diagramPadding) ?? 20,
			B = ((j = f.ishikawa) == null ? void 0 : j.useMaxWidth) ?? !1,
			O = dt(h),
		...
		for (let T = 0; T < b; T++) {
			...
			for (const [F, z, A] of [
				...
				F && Nt(D, F, i, u, z, A, k, t);
			i = D.selectAll('text')
				.nodes()
				.reduce((F, z) => Math.min(F, z.getBBox().x), 1 / 0);
		}
	...
	Lt = o((e, h, r, a, l, y) => {
		const f = Math.max(6, Math.floor(110 / (l * 0.6))),
			...
			w = q(d, ct(a, f), 0, 0, 'ishikawa-head-label', 'start', l),
			c = w.node().getBBox(),
	...
	J = o((e, h, r, a, l, y) => {
		...
		const d = a / f,
			...
			_ = h,
			x = r,
			B = `M ${_} ${x} L ${_ - d * c * 2 + k} ${x - w * c * 2 + g} L ${_ - d * c * 2 - k} ${x - w * c * 2 - g} Z`,
			O = y.roughSvg.path(B, {
				roughness: 1,
				seed: y.seed,
				fill: y.lineColor,
	...
	Nt = o((e, h, r, a, l, y, f, d) => {
		...
		if (
			...
			return;
		const { entries: B, yOrder: O } = Tt(w, l),
			$ = B.length,
			M = new Array($);
		for (const [p, n] of O.entries()) M[n] = a + g * ((p + 1) / ($ + 1));
		const t = new Map();
		t.set(-1, { x0: r, y0: a, x1: _, y1: x, childCount: w.length, childrenDrawn: 0 });
		const s = -st,
			i = nt * l,
			u = l < 0 ? 'ishikawa-label up' : 'ishikawa-label down';
		for (const [p, n] of B.entries()) {
			const v = M[p],
				m = t.get(n.parentIndex),
			...
			if (n.depth % 2 === 0) {
				...
				((S = lt(m.x0, m.x1, (m.childCount - R) / (m.childCount + 1))),
					(L = m.y0),
					(E = S + s * ((v - L) / i)),
			...
			n.childCount > 0 &&
				t.set(p, { x0: S, y0: L, x1: E, y1: v, childCount: n.childCount, childrenDrawn: 0 });
		}
	}, 'drawBranch'),
	Pt = o((e) => e.split(/<br\s*\/?>|\n/), 'splitLines'),
	ct = o((e, h) => {
...
//# sourceMappingURL=DQ0fYc1z.js.map
