const e = 'เกี่ยวกับ',
	...
	Fr = {
		'-1 for no limit, or a positive integer for a specific limit':
			'-1 สำหรับไม่จำกัด หรือจำนวนเต็มบวกสำหรับจำกัดค่าเฉพาะ',
		"'s', 'm', 'h', 'd', 'w' or '-1' for no expiration.":
			"'s', 'm', 'h', 'd', 'w' หรือ '-1' เพื่อไม่ให้หมดอายุ",
		'(e.g. `sh webui.sh --api --api-auth username_password`)':
			'(เช่น `sh webui.sh --api --api-auth username_password`)',
		...
		'*Prompt node ID(s) are required for image generation':
			'*ต้องระบุ ID ของ prompt node สำหรับการสร้างภาพ',
		'1 hour before': '',
		...
		'A new version (v{{LATEST_VERSION}}) is now available.':
			'เวอร์ชันใหม่ (v{{LATEST_VERSION}}) พร้อมให้ใช้งานแล้ว',
		'A private conversation between you and selected users': '',
		'A task model is used when performing tasks such as generating titles for chats and web search queries':
			'Task Model จะถูกใช้เมื่อทำภารกิจต่างๆ เช่น การสร้างหัวข้อแชทและการค้นหาเว็บ',
		'a user': 'ผู้ใช้',
		About: e,
...
ข้ามไปยังตัวแปรพรอมต์`,
		...
		'Activate this command by typing "/{{COMMAND}}" to chat input.':
			'เปิดใช้งานคำสั่งนี้โดยพิมพ์ "/{{COMMAND}}" ในช่องแชท',
		Active: r,
		...
		'Add a short description about what this model does':
			'เพิ่มคำอธิบายสั้นๆ เกี่ยวกับสิ่งที่โมเดลนี้ทำ',
		'Add a tag': 'เพิ่มแท็ก',
		'Add a tag...': '',
		'Add Access': '',
		...
		'Adjusting these settings will apply changes universally to all users.':
			'การปรับการตั้งค่าเหล่านี้จะมีผลกับผู้ใช้ทุกคนทั่วทั้งระบบ',
		admin: c,
		Admin: d,
		'Admin Contact Email': '',
		...
		'Admins have access to all tools at all times; users need tools assigned per model in the workspace.':
			'ผู้ดูแลระบบสามารถเข้าถึงเครื่องมือทั้งหมดได้ตลอดเวลา ส่วนผู้ใช้ต้องได้รับการกำหนดเครื่องมือต่อโมเดลในแต่ละพื้นที่ทำงาน',
		Advanced: u,
		...
		'Advanced parameters for MinerU parsing (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)':
			'พารามิเตอร์ขั้นสูงสำหรับการแยกวิเคราะห์ MinerU (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)',
		...
		'After updating or changing the embedding model, you must reindex the knowledge base for the changes to take effect. You can do this using the "Reindex" button below.':
			'หลังจากอัปเดตหรือเปลี่ยนโมเดล Embedding คุณต้องสร้างดัชนีฐานความรู้ใหม่เพื่อให้การเปลี่ยนแปลงมีผล คุณสามารถทำได้โดยใช้ปุ่ม "สร้างดัชนีใหม่" ด้านล่าง',
		AI: m,
		All: p,
		...
		'API Base URL for Datalab Marker service. Defaults to: https://www.datalab.to/api/v1/marker':
			'URL พื้นฐานของ API สำหรับบริการ Datalab Marker ค่าเริ่มต้น: https://www.datalab.to/api/v1/marker',
		'API Key': 'คีย์ API',
		...
		'Are you sure you want to clear all memories? This action cannot be undone.':
			'คุณแน่ใจหรือว่าต้องการล้างความจำทั้งหมด? การดำเนินการนี้ไม่สามารถยกเลิกได้',
		...
		'Are you sure you want to unarchive all archived chats?':
			'คุณแน่ใจหรือว่าต้องการยกเลิกการเก็บถาวรแชททั้งหมด?',
		...
		'Base Model List Cache speeds up access by fetching base models only at startup or on settings save—faster, but may not show recent base model changes.':
			'การแคชรายการ Base Model ช่วยเร่งการเข้าถึงโดยดึงข้อมูลโมเดลเฉพาะตอนเริ่มต้นระบบหรือเมื่อบันทึกการตั้งค่า ซึ่งทำให้เร็วขึ้น แต่อาจไม่แสดงการเปลี่ยนแปลงโมเดลล่าสุด',
		Bearer: K,
		before: _,
		'Being lazy': 'ขี้เกียจ',
		Beta: B,
		...
		'Boosting or penalizing specific tokens for constrained responses. Bias values will be clamped between -100 and 100 (inclusive). (Default: none)':
			'เพิ่มหรือลดน้ำหนักโทเค็นเฉพาะสำหรับการตอบกลับที่มีข้อจำกัด ค่าไบแอสจะถูกจำกัดระหว่าง -100 ถึง 100 (รวม) (ค่าเริ่มต้น: ไม่มี)',
		Brave: J,
		...
		'Call feature is not supported when using Web STT engine':
			'ไม่รองรับฟีเจอร์การโทรเมื่อใช้เอนจิน Web STT',
		Camera: z,
		Cancel: Q,
		...
		'Character limit for autocomplete generation input':
			'ขีดจำกัดจำนวนอักขระสำหรับอินพุตการสร้างคำอัตโนมัติ',
		...
		'Click here to download user import template file.':
			'คลิกที่นี่เพื่อดาวน์โหลดไฟล์แม่แบบนำเข้าผู้ใช้',
		'Click here to learn more about faster-whisper and see the available models.':
			'คลิกที่นี่เพื่อเรียนรู้เพิ่มเติมเกี่ยวกับ faster-whisper และดูโมเดลที่มีให้ใช้',
		...
		"Click on the user role button to change a user's role.":
			'คลิกที่ปุ่มสิทธิ์ผู้ใช้เพื่อเปลี่ยนสิทธิ์ของผู้ใช้',
		'Click to connect': '',
		'Click to copy ID': '',
		'Client ID': '',
		'Client Secret': '',
		'Clipboard write permission denied. Please check your browser settings to grant the necessary access.':
			'การอนุญาตให้เขียนคลิปบอร์ดถูกปฏิเสธ โปรดตรวจสอบการตั้งค่าเบราว์เซอร์ของคุณเพื่อให้สิทธิ์ที่จำเป็น',
		Clone: le,
		'Clone Chat': 'โคลนแชท',
		...
		'Connect to your own OpenAI compatible API endpoints.':
			'เชื่อมต่อกับ API Endpoint ที่เข้ากันได้กับ OpenAI ของคุณเอง',
		'Connect to your own OpenAPI compatible external tool servers.':
			'เชื่อมต่อกับเซิร์ฟเวอร์เครื่องมือภายนอกของคุณที่รองรับ OpenAPI',
		'Connected ({{type}})': '',
		...
		'Constrains effort on reasoning for reasoning models. Only applicable to reasoning models from specific providers that support reasoning effort.':
			'จำกัดระดับความพยายามในการให้เหตุผลสำหรับโมเดลที่ใช้การให้เหตุผล ใช้ได้เฉพาะกับโมเดลให้เหตุผลจากผู้ให้บริการบางรายที่รองรับการกำหนดระดับความพยายามในการให้เหตุผล',
		'Contact Admin for WebUI Access': 'ติดต่อผู้ดูแลระบบเพื่อขอสิทธิ์เข้าใช้ WebUI',
		Content: Ce,
		...
		"Control how message text is split for TTS requests. 'Punctuation' splits into sentences, 'paragraphs' splits into paragraphs, and 'none' keeps the message as a single string.":
			"ควบคุมวิธีการแบ่งข้อความสำหรับคำขอ TTS โดย 'เครื่องหมายวรรคตอน' จะแบ่งเป็นประโยค 'ย่อหน้า' จะแบ่งเป็นย่อหน้า และ 'ไม่แบ่ง' จะเก็บข้อความไว้เป็นสตริงเดียว",
		'Control the repetition of token sequences in the generated text. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 1.1) will be more lenient. At 1, it is disabled.':
			'ควบคุมการซ้ำของลำดับโทเค็นในข้อความที่สร้างขึ้น ค่ายิ่งสูง (เช่น 1.5) จะลงโทษการซ้ำหนักขึ้น ในขณะที่ค่ายิ่งต่ำ (เช่น 1.1) จะผ่อนปรนมากกว่า ที่ค่า 1 จะปิดการทำงาน',
		Controls: Ee,
		'Controls the balance between coherence and diversity of the output. A lower value will result in more focused and coherent text.':
			'ควบคุมสมดุลระหว่างความสอดคล้องและความหลากหลายของผลลัพธ์ ค่าในระดับต่ำจะทำให้ข้อความมีจุดเน้นชัดเจนและเชื่อมโยงกันมากขึ้น',
		...
		'CORS must be properly configured by the provider to allow requests from Open WebUI.':
			'ผู้ให้บริการต้องกำหนดค่า CORS ให้ถูกต้องเพื่ออนุญาตคำขอจาก Open WebUI',
		'Could not read file.': '',
		CPU: ke,
		Create: De,
		...
		'Create your first note by clicking on the plus button below.':
			'สร้างบันทึกแรกของคุณโดยคลิกที่ปุ่มบวกด้านล่าง',
		...
		"Default mode works with a wider range of models by calling tools once before execution. Native mode leverages the model's built-in tool-calling capabilities, but requires the model to inherently support this feature.":
			'โหมดเริ่มต้นทำงานร่วมกับโมเดลได้หลากหลายกว่า โดยเรียกใช้เครื่องมือหนึ่งครั้งก่อนการรันคำสั่ง ส่วนโหมด Native จะใช้ความสามารถในการเรียกใช้เครื่องมือที่มีอยู่ในตัวโมเดล แต่ต้องอาศัยว่าโมเดลรองรับฟีเจอร์นี้ในตัวอยู่แล้ว',
		...
		'Default to segmented retrieval for focused and relevant content extraction, this is recommended for most cases.':
			'กำหนดค่าเริ่มต้นให้ใช้การดึงข้อมูลแบบแบ่งส่วนเพื่อการดึงเนื้อหาที่มีโฟกัสและเกี่ยวข้อง แนะนำให้ใช้ตัวเลือกนี้ในกรณีส่วนใหญ่',
		...
		'Direct Connections allow users to connect to their own OpenAI compatible API endpoints.':
			'Direct Connections อนุญาตให้ผู้ใช้เชื่อมต่อกับปลายทาง API ที่เข้ากันได้กับ OpenAI ของตนเอง',
		'Direct Message': '',
		...
		'Disable image extraction from the PDF. If Use LLM is enabled, images will be automatically captioned. Defaults to False.':
			'ปิดใช้งานการดึงรูปภาพจากไฟล์ PDF หากเปิดใช้ Use LLM รูปภาพจะถูกสร้างคำบรรยายให้โดยอัตโนมัติ ค่าเริ่มต้นคือ False',
		Disabled: qe,
		'Disconnect OAuth': '',
		...
		'Discover how to use Open WebUI and seek support from the community.':
			'เรียนรู้วิธีใช้ Open WebUI และขอความช่วยเหลือจากชุมชน',
		'Discover wonders': 'ค้นพบสิ่งมหัศจรรย์',
		'Discover, download, and explore custom functions':
			'ค้นหา ดาวน์โหลด และสำรวจฟังก์ชันที่กำหนดเอง',
		...
		'Displays status updates (e.g., web search progress) in the response':
			'แสดงการอัปเดตสถานะ (เช่น ความคืบหน้าการค้นเว็บ) ภายในคำตอบ',
		'Dive into knowledge': 'เจาะลึกสู่ความรู้',
		'Do not install functions from sources you do not fully trust.':
			'อย่าติดตั้งฟังก์ชันจากแหล่งที่คุณไม่ไว้วางใจอย่างเต็มที่',
		'Do not install tools from sources you do not fully trust.':
			'อย่าติดตั้งเครื่องมือจากแหล่งที่คุณไม่ไว้วางใจอย่างเต็มที่',
		...
		"e.g. '30s','10m'. Valid time units are 's', 'm', 'h'.":
			"เช่น '30s', '10m' หน่วยเวลาที่ใช้ได้คือ 's', 'm', 'h'",
		"e.g. 'low', 'medium', 'high'": '',
		...
		'e.g., audio/wav,audio/mpeg,video/* (leave blank for defaults)':
			'เช่น audio/wav,audio/mpeg,video/* (เว้นว่างไว้เพื่อใช้ค่าเริ่มต้น)',
		'e.g., en-US,ja-JP (leave blank for auto-detect)':
			'เช่น en-US,ja-JP (เว้นว่างเพื่อให้ตรวจจับอัตโนมัติ)',
		...
		'Enable autocomplete generation for chat messages':
			'เปิดใช้งานการเติมข้อความอัตโนมัติสำหรับข้อความแชท',
		...
		"Enable Memory Locking (mlock) to prevent model data from being swapped out of RAM. This option locks the model's working set of pages into RAM, ensuring that they will not be swapped out to disk. This can help maintain performance by avoiding page faults and ensuring fast data access.":
			'เปิดใช้ Memory Locking (mlock) เพื่อป้องกันไม่ให้ข้อมูลโมเดลถูก Swap ออกจาก RAM ตัวเลือกนี้จะล็อกชุด Pages ที่โมเดลกำลังใช้งานให้อยู่ใน RAM ทำให้ไม่ถูก Swap ออกไปยังดิสก์ ซึ่งช่วยรักษาประสิทธิภาพด้วยการหลีกเลี่ยง Page Fault และทำให้เข้าถึงข้อมูลได้อย่างรวดเร็ว',
		'Enable Memory Mapping (mmap) to load model data. This option allows the system to use disk storage as an extension of RAM by treating disk files as if they were in RAM. This can improve model performance by allowing for faster data access. However, it may not work correctly with all systems and can consume a significant amount of disk space.':
			'เปิดใช้ Memory Mapping (mmap) เพื่อโหลดข้อมูลโมเดล ตัวเลือกนี้จะทำให้ระบบสามารถใช้พื้นที่ดิสก์เป็นส่วนขยายของ RAM โดยมองไฟล์บนดิสก์เสมือนว่าอยู่ใน RAM ซึ่งอาจช่วยเพิ่มประสิทธิภาพของโมเดลโดยทำให้เข้าถึงข้อมูลได้เร็วขึ้น อย่างไรก็ตาม อาจทำงานไม่ถูกต้องกับทุกระบบและอาจใช้พื้นที่ดิสก์ในปริมาณมาก',
		'Enable Message Queue': '',
		'Enable Message Rating': 'เปิดใช้งานการให้คะแนนข้อความ',
		'Enable Mirostat sampling for controlling perplexity.':
			'เปิดใช้การสุ่มตัวอย่างแบบ Mirostat เพื่อควบคุม Perplexity',
		...
		'Ensure your CSV file includes 4 columns in this order: Name, Email, Password, Role.':
			'ตรวจสอบว่าไฟล์ CSV ของคุณมี 4 คอลัมน์ในลำดับนี้: ชื่อ, อีเมล, รหัสผ่าน, บทบาท',
		'Enter {{role}} message here': 'ระบุข้อความของ {{role}} ที่นี่',
		'Enter a detail about yourself for your LLMs to recall':
			'ระบุรายละเอียดเกี่ยวกับตัวคุณเพื่อให้ LLM ของคุณจดจำ',
		'Enter a title for the pending user info overlay. Leave empty for default.':
			'ป้อนชื่อสำหรับหน้าซ้อนข้อมูลผู้ใช้ที่กำลังรออยู่ เว้นว่างไว้เพื่อใช้ค่าเริ่มต้น',
		'Enter a watermark for the response. Leave empty for none.':
			'ป้อนลายน้ำสำหรับคำตอบ เว้นว่างหากไม่ต้องการ',
		...
		'Enter api auth string (e.g. username:password)':
			'ใส่สตริงการยืนยันตัวตน API (เช่น username:password)',
		...
		'Enter comma-separated "token:bias_value" pairs (example: 5432:100, 413:-100)':
			'ป้อนคู่ "token:bias_value" ที่คั่นด้วยจุลภาค (ตัวอย่าง: 5432:100, 413:-100)',
		'Enter content for the pending user info overlay. Leave empty for default.':
			'ป้อนเนื้อหาสำหรับ Overlay ข้อมูลผู้ใช้ที่กำลังรออยู่ เว้นว่างไว้เพื่อใช้ค่าเริ่มต้น',
		...
		'Enter JSON config (e.g., {"disable_links": true})':
			'ป้อนการตั้งค่า JSON (เช่น {"disable_links": true})',
		...
		'Enter proxy URL (e.g. https://user:password@host:port)':
			'ป้อน URL พร็อกซี (เช่น https://user:password@host:port)',
		...
		'Enter the public URL of your WebUI. This URL will be used to generate links in the notifications.':
			'ป้อน URL สาธารณะของ WebUI ของคุณ URL นี้จะใช้ในการสร้างลิงก์ในการแจ้งเตือน',
		...
		'Enter Yacy URL (e.g. http://yacy.example.com:8090)':
			'ป้อน URL ของ Yacy (เช่น http://yacy.example.com:8090)',
		...
		"Error: A model with the ID '{{modelId}}' already exists. Please select a different ID to proceed.":
			"ข้อผิดพลาด: มีโมเดลที่ใช้ ID '{{modelId}}' อยู่แล้ว โปรดเลือก ID อื่นเพื่อดำเนินการต่อ",
		'Error: Model ID cannot be empty. Please enter a valid ID to proceed.':
			'ข้อผิดพลาด: ห้ามปล่อย ID โมเดลว่างไว้ กรุณากรอก ID ที่ถูกต้องเพื่อดำเนินการต่อ',
		Evaluations: yt,
		'Event created': '',
		'Event deleted': '',
		'Event title': '',
		...
		'Example: (&(objectClass=inetOrgPerson)(uid=%s))':
			'ตัวอย่าง: (&(objectClass=inetOrgPerson)(uid=%s))',
		...
		'Example: sAMAccountName or uid or userPrincipalName':
			'ตัวอย่าง: sAMAccountName หรือ uid หรือ userPrincipalName',
		'Exceeded the number of seats in your license. Please contact support to increase the number of seats.':
			'จำนวนที่นั่งในไลเซนส์ของคุณเกินขีดจำกัดแล้ว โปรดติดต่อฝ่ายสนับสนุนเพื่อขอเพิ่มจำนวนที่นั่ง',
		Exclude: ft,
		'Execute code': '',
		...
		'Failed to connect to {{URL}} OpenAPI tool server':
			'เชื่อมต่อกับเซิร์ฟเวอร์เครื่องมือ OpenAPI ที่ {{URL}} ไม่สำเร็จ',
		...
		'Failed to extract content from the file: {{error}}':
			'ไม่สามารถดึงเนื้อหาจากไฟล์ได้: {{error}}',
		...
		'Fingerprint spoofing detected: Unable to use initials as avatar. Defaulting to default profile image.':
			'ตรวจพบการปลอมแปลงลายนิ้วมือ: ไม่สามารถใช้ชื่อย่อเป็นอวตาร์ได้ กำลังใช้รูปโปรไฟล์เริ่มต้น',
		...
		'Force OCR on all pages of the PDF. This can lead to worse results if you have good text in your PDFs. Defaults to False.':
			'บังคับใช้ OCR กับทุกหน้าของไฟล์ PDF อาจทำให้ผลลัพธ์แย่ลงได้หากไฟล์ PDF ของคุณมีข้อความที่ดีอยู่แล้ว ค่าเริ่มต้นคือ False',
		...
		'Format the lines in the output. Defaults to False. If set to True, the lines will be formatted to detect inline math and styles.':
			'จัดรูปแบบบรรทัดในผลลัพธ์ ค่าปริยายเป็น False หากตั้งค่าเป็น True บรรทัดจะถูกจัดรูปแบบเพื่อให้ตรวจจับสมการคณิตศาสตร์แบบ Inline และสไตล์ได้',
		...
		'Forwards system user OAuth access token to authenticate':
			'ส่งต่อโทเค็นการเข้าถึง OAuth ของผู้ใช้ระบบเพื่อยืนยันตัวตน',
		'Forwards system user session credentials to authenticate':
			'ส่งต่อข้อมูลรับรอง Session ของผู้ใช้ระบบเพื่อใช้ยืนยันตัวตน',
		Fr_day_of_week: Wt,
		...
		'Get started with {{WEBUI_NAME}}': 'เริ่มต้นใช้งาน {{WEBUI_NAME}}',
		Global: Yt,
		...
		'I acknowledge that I have read and I understand the implications of my action. I am aware of the risks associated with executing arbitrary code and I have verified the trustworthiness of the source.':
			'ฉันรับทราบว่าฉันได้อ่านและเข้าใจผลกระทบของการกระทำของฉัน ฉันทราบถึงความเสี่ยงที่เกี่ยวข้องกับการเรียกใช้โค้ดโดยพลการและฉันได้ตรวจสอบความน่าเชื่อถือของแหล่งที่มาแล้ว',
		ID: mo,
		...
		'Include `--api-auth` flag when running stable-diffusion-webui':
			'ระบุแฟล็ก `--api-auth` เมื่อรัน stable-diffusion-webui',
		'Include `--api` flag when running stable-diffusion-webui':
			'ระบุแฟล็ก `--api` เมื่อเรียกใช้ stable-diffusion-webui',
		'Includes SharePoint': 'รวม SharePoint',
		'Increase UI Scale': '',
		'Influences how quickly the algorithm responds to feedback from the generated text. A lower learning rate will result in slower adjustments, while a higher learning rate will make the algorithm more responsive.':
			'มีผลต่อความเร็วที่อัลกอริทึมตอบสนองต่อข้อเสนอแนะจากข้อความที่สร้างขึ้น ค่า Learning Rate ที่ต่ำจะทำให้การปรับเปลี่ยนช้าลง ในขณะที่ค่า Learning Rate ที่สูงจะทำให้อัลกอริทึมตอบสนองได้รวดเร็วขึ้น',
		Info: Ao,
		Initials: bo,
		'Inject file content into conversation context': '',
		'Inject the entire content as context for comprehensive processing, this is recommended for complex queries.':
			'เพิ่มเนื้อหาทั้งหมดเป็นบริบทสำหรับการประมวลผลอย่างครอบคลุม แนะนำให้ใช้สำหรับคำถามที่ซับซ้อน',
		Input: wo,
		...
		'Invalid JSON format for ComfyUI Edit Workflow.':
			'รูปแบบ JSON ไม่ถูกต้องสำหรับการแก้ไข Workflow ของ ComfyUI',
		...
		'Knowledge reset successfully.': 'รีเซ็ตฐานความรู้สำเร็จแล้ว',
		'Knowledge Sharing': '',
		...
		'Learn more about OpenAPI tool servers.':
			'ดูข้อมูลเพิ่มเติมเกี่ยวกับเซิร์ฟเวอร์เครื่องมือ OpenAPI',
		...
		'Leave empty to include all models from "{{url}}" endpoint':
			'ปล่อยว่างไว้เพื่อรวมโมเดลทั้งหมดจาก Endpoint "{{url}}"',
		'Leave empty to include all models from "{{url}}/api/tags" endpoint':
			'เว้นว่างไว้เพื่อรวมโมเดลทั้งหมดจาก Endpoint "{{url}}/api/tags"',
		'Leave empty to include all models from "{{url}}/models" endpoint':
			'เว้นว่างไว้เพื่อรวมโมเดลทั้งหมดจาก Endpoint "{{url}}/models"',
		'Leave empty to include all models or select specific models':
			'เว้นว่างไว้เพื่อรวมโมเดลทั้งหมด หรือเลือกเฉพาะโมเดลที่ต้องการ',
		...
		'Leave empty to use the default model (voxtral-mini-latest).':
			'เว้นว่างไว้เพื่อใช้โมเดลเริ่มต้น (voxtral-mini-latest)',
		'Leave empty to use the default prompt, or enter a custom prompt':
			'เว้นว่างไว้เพื่อใช้พรอมต์เริ่มต้น หรือป้อนพรอมต์ที่กำหนดเอง',
		'Leave model field empty to use the default model.':
			'เว้นช่องโมเดลว่างไว้เพื่อใช้โมเดลเริ่มต้น',
		Legacy: No,
		lexical: Fo,
		License: Wo,
		...
		'LLMs can make mistakes. Verify important information.':
			'LLM อาจทำผิดพลาดได้ โปรดตรวจสอบข้อมูลสำคัญ',
		Loader: Go,
		...
		Low: qo,
		LTR: jo,
		'Made by Open WebUI Community': 'สร้างโดยชุมชน Open WebUI',
		...
		'Make sure to export a workflow.json file as API format from ComfyUI.':
			'ตรวจสอบให้แน่ใจว่าส่งออกไฟล์ workflow.json เป็นรูปแบบ API จาก ComfyUI',
		Male: zo,
		Manage: Qo,
		'Manage Connections': '',
		...
		'Maximum of 3 models can be downloaded simultaneously. Please try again later.':
			'สามารถดาวน์โหลดโมเดลได้สูงสุด 3 โมเดลในเวลาเดียวกัน โปรดลองอีกครั้งในภายหลัง',
		May: $o,
		MBR: en,
		MCP: tn,
		'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the Open WebUI team, making it the more reliable option for compatibility.':
			'การรองรับ MCP ยังเป็นแบบทดลองและมีการเปลี่ยนแปลงสเปกบ่อยครั้ง ซึ่งอาจทำให้เกิดปัญหาความไม่เข้ากันได้ การรองรับสเปก OpenAPI นั้นดูแลโดยทีม Open WebUI โดยตรง จึงเป็นตัวเลือกที่เชื่อถือได้มากกว่าในด้านความเข้ากันได้',
		Medium: on,
		...
		'Message rating should be enabled to use this feature':
			'ต้องเปิดใช้การให้คะแนนข้อความก่อนจึงจะใช้ฟีเจอร์นี้ได้',
		messages: cn,
		Messages: dn,
		"Messages you send after creating your link won't be shared. Users with the URL will be able to view the shared chat.":
			'ข้อความที่คุณส่งหลังจากสร้างลิงก์แล้วจะไม่ถูกแชร์ ผู้ใช้ที่มี URL จะสามารถดูแชทที่แชร์ได้',
		...
		"Model '{{modelName}}' has been successfully downloaded.":
			"โมเดล '{{modelName}}' ถูกดาวน์โหลดเรียบร้อยแล้ว",
		"Model '{{modelTag}}' is already in queue for downloading.":
			"โมเดล '{{modelTag}}' อยู่ในคิวสำหรับการดาวน์โหลดแล้ว",
		...
		'Model filesystem path detected. Model shortname is required for update, cannot continue.':
			'ตรวจพบเส้นทางระบบไฟล์ของโมเดล ต้องระบุชื่อย่อของโมเดลสำหรับการอัปเดต ไม่สามารถดำเนินการต่อได้',
		...
		'Model name already exists, please choose a different one':
			'มีชื่อโมเดลนี้อยู่แล้ว โปรดเลือกชื่ออื่น',
		...
		'Name and ID are required, please fill them out':
			'จำเป็นต้องกรอกชื่อและ ID โปรดกรอกข้อมูลให้ครบ',
		...
		'No expiration can pose security risks.':
			'ไม่มีวันหมดอายุอาจทำให้เกิดความเสี่ยงด้านความปลอดภัย',
		'No feedback found': '',
		...
		'Note: If you set a minimum score, the search will only return documents with a score greater than or equal to the minimum score.':
			'หมายเหตุ: หากคุณตั้งค่าคะแนนขั้นต่ำ การค้นหาจะคืนเฉพาะเอกสารที่มีคะแนนมากกว่าหรือเท่ากับคะแนนขั้นต่ำ',
		Notes: Un,
		...
		'Only active when "Paste Large Text as File" setting is toggled on.':
			'จะทำงานเฉพาะเมื่อเปิดการตั้งค่า "วางข้อความขนาดใหญ่เป็นไฟล์"',
		'Only active when the chat input is in focus and an LLM is generating a response.':
			'ทำงานเฉพาะเมื่อช่องป้อนข้อความแชทถูกโฟกัสอยู่ และ LLM กำลังสร้างคำตอบ',
		'Only active when the chat input is in focus.':
			'จะใช้งานได้เฉพาะเมื่อช่องป้อนข้อความแชทถูกโฟกัสอยู่เท่านั้น',
		...
		'Only alphanumeric characters and hyphens are allowed in the command string.':
			'อนุญาตให้ใช้เฉพาะตัวอักษร ตัวเลข และขีดกลางในสตริงคำสั่งเท่านั้น',
		'Only can be triggered when the chat input is in focus.':
			'สามารถทริกเกอร์ได้เฉพาะเมื่อช่องป้อนข้อความแชทอยู่ในโฟกัสเท่านั้น',
		'Only collections can be edited, create a new knowledge base to edit/add documents.':
			'สามารถแก้ไขได้เฉพาะคอลเลกชันเท่านั้น โปรดสร้างฐานความรู้ใหม่เพื่อแก้ไข/เพิ่มเอกสาร',
		...
		'Only select users and groups with permission can access':
			'มีเฉพาะผู้ใช้และกลุ่มที่ได้รับสิทธิ์เท่านั้นที่เข้าถึงได้',
		'Only sync new/updated chats': '',
		'Oops! Looks like the URL is invalid. Please double-check and try again.':
			'ขออภัย! ดูเหมือนว่า URL ไม่ถูกต้อง กรุณาตรวจสอบและลองใหม่อีกครั้ง',
		'Oops! There are files still uploading. Please wait for the upload to complete.':
			'โอ๊ะ! ยังมีไฟล์กำลังอัปโหลดอยู่ กรุณารอจนกว่าอัปโหลดจะเสร็จสมบูรณ์',
		'Oops! There was an error in the previous response.': 'ขออภัย! เกิดข้อผิดพลาดในคำตอบก่อนหน้า',
		"Oops! You're using an unsupported method (frontend only). Please serve the WebUI from the backend.":
			'ขออภัย! คุณกำลังใช้วิธีที่ไม่รองรับ (เฉพาะ Frontend) กรุณาให้บริการ WebUI จากฝั่ง Backend',
		'Open file': 'เปิดไฟล์',
		...
		'Open WebUI can use tools provided by any OpenAPI server.':
			'Open WebUI สามารถใช้เครื่องมือที่มีให้จากเซิร์ฟเวอร์ OpenAPI ใดก็ได้',
		'Open WebUI uses faster-whisper internally.': 'Open WebUI ใช้ faster-whisper อยู่ภายใน',
		'Open WebUI uses SpeechT5 and CMU Arctic speaker embeddings.':
			'Open WebUI ใช้ SpeechT5 และ Embedding เสียงผู้พูด CMU Arctic',
		'Open WebUI version': '',
		'Open WebUI version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})':
			'เวอร์ชัน Open WebUI (v{{OPEN_WEBUI_VERSION}}) ต่ำกว่าเวอร์ชันที่ต้องการ (v{{REQUIRED_VERSION}})',
		OpenAI: Wn,
		...
		'Permission denied when accessing microphone: {{error}}':
			'ไม่ได้รับอนุญาตให้เข้าถึงไมโครโฟน: {{error}}',
		Permissions: ta,
		...
		'Pipelines are a plugin system with arbitrary code execution —':
			'Pipelines เป็นระบบปลั๊กอินที่สามารถรันโค้ดได้อย่างอิสระ —',
		...
		'Please do not close the settings page while loading the model.':
			'โปรดอย่าปิดหน้าการตั้งค่าขณะกำลังโหลดโมเดล',
		...
		'Please save the connection to persist the OAuth client information and do not change the ID':
			'โปรดบันทึกการเชื่อมต่อเพื่อคงข้อมูล OAuth Client และอย่าเปลี่ยนแปลง ID',
		...
		'Prefix ID is used to avoid conflicts with other connections by adding a prefix to the model IDs - leave empty to disable':
			'Prefix ID ใช้เพื่อหลีกเลี่ยงความขัดแย้งกับการเชื่อมต่ออื่น โดยการเพิ่มคำนำหน้าให้กับ ID ของโมเดล หากต้องการปิดให้เว้นว่างไว้',
		...
		'Reduces the probability of generating nonsense. A higher value (e.g. 100) will give more diverse answers, while a lower value (e.g. 10) will be more conservative.':
			'ลดโอกาสในการสร้างข้อความที่ไม่มีความหมาย ค่าให้สูงขึ้น (เช่น 100) จะทำให้ได้คำตอบที่หลากหลายมากขึ้น ในขณะที่ค่าให้ต่ำลง (เช่น 10) จะทำให้คำตอบระมัดระวังมากขึ้น',
		'Refer to yourself as "User" (e.g., "User is learning Spanish")':
			'เรียกตัวเองว่า "User" (เช่น "User กำลังเรียนภาษาสเปน")',
		...
		'Response notifications cannot be activated as the website permissions have been denied. Please visit your browser settings to grant the necessary access.':
			'ไม่สามารถเปิดใช้งานการแจ้งเตือนการตอบกลับได้ เนื่องจากสิทธิ์ของเว็บไซต์ถูกปฏิเสธ กรุณาไปที่การตั้งค่าเบราว์เซอร์ของคุณเพื่อให้สิทธิ์การเข้าถึงที่จำเป็น',
		...
		"Saving chat logs directly to your browser's storage is no longer supported. Please take a moment to download and delete your chat logs by clicking the button below. Don't worry, you can easily re-import your chat logs to the backend through":
			'การบันทึก Log การแชทโดยตรงไปยังที่จัดเก็บของเบราว์เซอร์ไม่รองรับอีกต่อไป โปรดสละเวลาสักครู่เพื่อดาวน์โหลดและลบบันทึกการแชทของคุณโดยคลิกปุ่มด้านล่าง ไม่ต้องกังวล คุณสามารถนำเข้าบันทึกการแชทของคุณกลับไปยัง Backend ได้อย่างง่ายดายผ่าน',
		Schedule: za,
		...
		'Set the number of layers, which will be off-loaded to GPU. Increasing this value can significantly improve performance for models that are optimized for GPU acceleration but may also consume more power and GPU resources.':
			'ตั้งค่าจำนวนเลเยอร์ที่จะ Offload ไปยัง GPU การเพิ่มค่านี้สามารถช่วยเพิ่มประสิทธิภาพได้มากสำหรับโมเดลที่ปรับแต่งมาสำหรับการเร่งความเร็วด้วย GPU แต่ก็อาจใช้พลังงานและทรัพยากร GPU มากขึ้นด้วย',
		'Set the number of worker threads used for computation. This option controls how many threads are used to process incoming requests concurrently. Increasing this value can improve performance under high concurrency workloads but may also consume more CPU resources.':
			'กำหนดจำนวนเธรดของ Worker ที่ใช้สำหรับการคำนวณ ตัวเลือกนี้ใช้กำหนดว่าใช้เธรดจำนวนเท่าใดในการประมวลผลคำขอที่เข้ามาพร้อมกัน การเพิ่มค่านี้สามารถช่วยเพิ่มประสิทธิภาพภายใต้ปริมาณงานที่มีความพร้อมกันสูง แต่อาจใช้ทรัพยากร CPU มากขึ้น',
		...
		'Sets a flat bias against tokens that have appeared at least once. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'ตั้งค่า Bias แบบ Flat ต่อโทเค็นที่เคยปรากฏอย่างน้อยหนึ่งครั้ง ค่าให้สูงขึ้น (เช่น 1.5) จะลงโทษการซ้ำมากขึ้น ในขณะที่ค่าให้ต่ำลง (เช่น 0.9) จะผ่อนปรนมากขึ้น หากตั้งเป็น 0 จะปิดการทำงาน',
		'Sets a scaling bias against tokens to penalize repetitions, based on how many times they have appeared. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'ตั้งค่า Bias แบบ Scaling ต่อโทเค็นเพื่อลงโทษการซ้ำ ขึ้นอยู่กับจำนวนครั้งที่โทเค็นนั้นปรากฏ ค่าให้สูงขึ้น (เช่น 1.5) จะลงโทษการซ้ำอย่างรุนแรงมากขึ้น ขณะที่ค่าให้ต่ำลง (เช่น 0.9) จะผ่อนปรนมากกว่า ที่ค่า 0 จะปิดการทำงานฟีเจอร์นี้',
		'Sets how far back for the model to look back to prevent repetition.':
			'ตั้งค่าระยะย้อนหลังที่โมเดลจะใช้พิจารณาเพื่อป้องกันการซ้ำคำ',
		'Sets the random number seed to use for generation. Setting this to a specific number will make the model generate the same text for the same prompt.':
			'ตั้งค่า Seed ของตัวเลขสุ่มที่ใช้ในการสร้างข้อความ การตั้งค่าเป็นหมายเลขเฉพาะจะทำให้โมเดลสร้างข้อความเดิมสำหรับพรอมต์เดิม',
		'Sets the size of the context window used to generate the next token.':
			'ตั้งค่าขนาดของ Context Window ที่ใช้ในการสร้างโทเค็นถัดไป',
		'Sets the stop sequences to use. When this pattern is encountered, the LLM will stop generating text and return. Multiple stop patterns may be set by specifying multiple separate stop parameters in a modelfile.':
			'ตั้งค่าลำดับตัวหยุดที่จะใช้ เมื่อตรวจพบรูปแบบนี้ LLM จะหยุดสร้างข้อความและส่งค่ากลับ สามารถตั้งค่ารูปแบบตัวหยุดหลายแบบได้โดยระบุพารามิเตอร์ Stop แยกกันหลายตัวใน Modelfile',
		Setting: as,
		Settings: ss,
		...
		'Share to Open WebUI Community': 'แชร์ไปยังชุมชน Open WebUI',
		...
		'Show Admin Details in Account Pending Overlay':
			'แสดงรายละเอียดผู้ดูแลระบบในหน้าต่างซ้อนรอการอนุมัติบัญชี',
		'Show All': '',
		...
		'Sign Out': 'ลงชื่อออก',
		'Sign up': 'สมัครสมาชิก',
		'Sign up to {{WEBUI_NAME}}': 'สมัครสมาชิกกับ {{WEBUI_NAME}}',
		'Significantly improves accuracy by using an LLM to enhance tables, forms, inline math, and layout detection. Will increase latency. Defaults to False.':
			'ช่วยเพิ่มความแม่นยำอย่างมากด้วยการใช้ LLM เพื่อเพิ่มประสิทธิภาพการตรวจจับตาราง ฟอร์ม สมการในบรรทัด และโครงร่างหน้าเอกสาร แต่จะทำให้หน่วงเวลามากขึ้น ค่าเริ่มต้นคือ False',
		'Signing in to {{WEBUI_NAME}}': 'กำลังลงชื่อเข้าใช้ {{WEBUI_NAME}}',
		Single: ls,
		...
		'Skip the cache and re-run the inference. Defaults to False.':
			'ข้ามแคชและรันการอนุมานใหม่อีกครั้ง ค่าเริ่มต้นคือ False',
		...
		'Strip existing OCR text from the PDF and re-run OCR. Ignored if Force OCR is enabled. Defaults to False.':
			'ลบข้อความ OCR เดิมออกจาก PDF แล้วรัน OCR ใหม่ จะถูกละเว้นหากเปิดใช้ Force OCR ค่าเริ่มต้นคือ False',
		...
		'Tail free sampling is used to reduce the impact of less probable tokens from the output. A higher value (e.g., 2.0) will reduce the impact more, while a value of 1.0 disables this setting.':
			'Tail free sampling ใช้เพื่อลดผลกระทบของโทเค็นที่มีความน่าจะเป็นต่ำจากผลลัพธ์ ค่าเท่าที่สูงกว่า (เช่น 2.0) จะลดผลกระทบได้มากกว่า ในขณะที่ค่า 1.0 จะปิดการตั้งค่านี้',
		...
		'The Application Account DN you bind with for search':
			'DN ของบัญชีแอปพลิเคชันที่คุณใช้ bind เพื่อค้นหา',
		...
		'The batch size determines how many text requests are processed together at once. A higher batch size can increase the performance and speed of the model, but it also requires more memory.':
			'ขนาด Batch กำหนดว่ามีคำขอข้อความกี่รายการที่ถูกประมวลผลพร้อมกันในครั้งเดียว ขนาด Batch ที่สูงขึ้นสามารถเพิ่มประสิทธิภาพและความเร็วของโมเดลได้ แต่ก็ต้องใช้หน่วยความจำมากขึ้น',
		'The developers behind this plugin are passionate volunteers from the community. If you find this plugin helpful, please consider contributing to its development.':
			'นักพัฒนาที่อยู่เบื้องหลังปลั๊กอินนี้เป็นอาสาสมัครจากชุมชนที่มีความมุ่งมั่น หากคุณพบว่าปลั๊กอินนี้มีประโยชน์ โปรดพิจารณามีส่วนร่วมในการพัฒนาปลั๊กอินนี้',
		'The evaluation leaderboard is based on the Elo rating system and is updated in real-time.':
			'กระดานจัดอันดับการประเมินอิงตามระบบเรตติ้ง Elo และอัปเดตแบบเรียลไทม์',
		'The format to return a response in. Format can be json or a JSON schema.':
			'รูปแบบในการส่งคืนคำตอบ โดยรูปแบบสามารถเป็น json หรือ JSON Schema ได้',
		'The height in pixels to compress images to. Leave empty for no compression.':
			'ความสูงของภาพเป็นพิกเซลที่จะใช้ในการบีบอัด ปล่อยว่างไว้หากไม่ต้องการบีบอัด',
		'The language of the input audio. Supplying the input language in ISO-639-1 (e.g. en) format will improve accuracy and latency. Leave blank to automatically detect the language.':
			'ภาษาของเสียงอินพุต การระบุภาษาของอินพุตในรูปแบบ ISO-639-1 (เช่น en) จะช่วยเพิ่มความแม่นยำและลดเวลาแฝง เว้นว่างไว้เพื่อให้ตรวจจับภาษาโดยอัตโนมัติ',
		'The LDAP attribute that maps to the mail that users use to sign in.':
			'แอตทริบิวต์ LDAP ที่แมปกับอีเมลที่ผู้ใช้ใช้เพื่อลงชื่อเข้าใช้',
		'The LDAP attribute that maps to the username that users use to sign in.':
			'แอตทริบิวต์ LDAP ที่แมปกับชื่อผู้ใช้ที่ผู้ใช้ใช้สำหรับลงชื่อเข้าใช้',
		'The leaderboard is currently in beta, and we may adjust the rating calculations as we refine the algorithm.':
			'กระดานจัดอันดับขณะนี้ยังเป็นเวอร์ชันทดสอบ (เบต้า) และเราอาจปรับวิธีคำนวณคะแนนจัดอันดับได้เมื่อปรับปรุงอัลกอริทึม',
		'The maximum file size in MB. If the file size exceeds this limit, the file will not be uploaded.':
			'ขนาดไฟล์สูงสุดเป็นหน่วย MB หากขนาดไฟล์เกินค่าที่กำหนดนี้ ไฟล์จะไม่ถูกอัปโหลด',
		'The maximum number of files that can be used at once in chat. If the number of files exceeds this limit, the files will not be uploaded.':
			'จำนวนไฟล์สูงสุดที่สามารถใช้ในการแชทได้พร้อมกัน หากจำนวนไฟล์เกินขีดจำกัดนี้ ไฟล์จะไม่ถูกอัปโหลด',
		"The output format for the text. Can be 'json', 'markdown', or 'html'. Defaults to 'markdown'.":
			"รูปแบบผลลัพธ์สำหรับข้อความ อาจเป็น 'json', 'markdown' หรือ 'html' ค่าเริ่มต้นคือ 'markdown'",
		"The passwords you entered don't quite match. Please double-check and try again.":
			'รหัสผ่านที่คุณป้อนไม่ตรงกัน โปรดตรวจสอบอีกครั้งแล้วลองใหม่',
		'The score should be a value between 0.0 (0%) and 1.0 (100%).':
			'คะแนนควรเป็นค่าระหว่าง 0.0 (0%) ถึง 1.0 (100%)',
		'The stream delta chunk size for the model. Increasing the chunk size will make the model respond with larger pieces of text at once.':
			'ขนาด Stream Delta Chunk สำหรับโมเดล การเพิ่มขนาด Chunk จะทำให้โมเดลตอบกลับเป็นข้อความชิ้นใหญ่ขึ้นในแต่ละครั้ง',
		'The temperature of the model. Increasing the temperature will make the model answer more creatively.':
			'Temperature ของโมเดล การเพิ่มค่า Temperature จะทำให้โมเดลตอบอย่างสร้างสรรค์มากขึ้น',
		'The Weight of BM25 Hybrid Search. 0 more semantic, 1 more lexical. Default 0.5':
			'ค่าน้ำหนักของการค้นหาแบบผสม BM25 ค่า 0 เน้นความหมาย (Semantic) ค่า 1 เน้นคำตรงตัว (Lexical) ค่าเริ่มต้น 0.5',
		'The width in pixels to compress images to. Leave empty for no compression.':
			'ความกว้างของภาพเป็นพิกเซลที่จะบีบอัด หากเว้นว่างไว้จะไม่บีบอัด',
		Theme: Rs,
		'There was an error syncing your stats. Please try again.': '',
		'Thinking...': 'กำลังคิด...',
		'This action cannot be undone. Do you wish to continue?':
			'การกระทำนี้ไม่สามารถย้อนกลับได้ คุณต้องการดำเนินการต่อหรือไม่?',
		'This channel was created on {{createdAt}}. This is the very beginning of the {{channelName}} channel.':
			'สร้างช่องนี้เมื่อ {{createdAt}} นี่คือจุดเริ่มต้นของช่อง {{channelName}}',
		"This chat won't appear in history and your messages will not be saved.":
			'การแชทนี้จะไม่ปรากฏในประวัติ และข้อความของคุณจะไม่ถูกบันทึก',
		'This ensures that your valuable conversations are securely saved to your backend database. Thank you!':
			'สิ่งนี้ช่วยให้มั่นใจได้ว่าบทสนทนาที่สำคัญของคุณจะถูกบันทึกอย่างปลอดภัยในฐานข้อมูล Backend ของคุณ ขอบคุณ!',
		...
		'This feature is experimental and may be modified or discontinued without notice.':
			'ฟีเจอร์นี้เป็นแบบทดลอง และอาจถูกแก้ไขหรือยกเลิกได้โดยไม่ต้องแจ้งให้ทราบล่วงหน้า',
		'This folder is empty': '',
		'This is a default user permission and will remain enabled.':
			'นี่คือสิทธิ์ของผู้ใช้เริ่มต้นและจะถูกเปิดใช้งานไว้เสมอ',
		'This is an experimental feature, it may not function as expected and is subject to change at any time.':
			'นี่เป็นฟีเจอร์ทดลอง อาจไม่ทำงานตามที่คาดไว้และอาจมีการเปลี่ยนแปลงได้ตลอดเวลา',
		'This model is not publicly available. Please select another model.':
			'โมเดลนี้ไม่เปิดให้ใช้งานสาธารณะ โปรดเลือกโมเดลอื่น',
		'This option controls how long the model will stay loaded into memory following the request (default: 5m)':
			'ตัวเลือกนี้ควบคุมระยะเวลาที่โมเดลจะถูกโหลดค้างอยู่ในหน่วยความจำหลังจากคำขอ (ค่าเริ่มต้น: 5 นาที)',
		'This option controls how many tokens are preserved when refreshing the context. For example, if set to 2, the last 2 tokens of the conversation context will be retained. Preserving context can help maintain the continuity of a conversation, but it may reduce the ability to respond to new topics.':
			'ตัวเลือกนี้ใช้กำหนดจำนวนโทเค็นที่จะถูกเก็บไว้เมื่อมีการรีเฟรชบริบท ยกตัวอย่างเช่น หากตั้งค่าเป็น 2 โทเค็น 2 ตัวสุดท้ายของบริบทการสนทนาจะถูกเก็บไว้ การเก็บรักษาบริบทสามารถช่วยให้การสนทนาต่อเนื่องมากขึ้น แต่อาจทำให้ความสามารถในการตอบสนองต่อหัวข้อใหม่ลดลง',
		'This option enables or disables the use of the reasoning feature in Ollama, which allows the model to think before generating a response. When enabled, the model can take a moment to process the conversation context and generate a more thoughtful response.':
			'ตัวเลือกนี้ใช้เปิดหรือปิดฟีเจอร์การให้เหตุผลใน Ollama ซึ่งช่วยให้โมเดลสามารถคิดก่อนสร้างคำตอบได้ เมื่อเปิดใช้งาน โมเดลจะใช้เวลาสักครู่เพื่อประมวลผลบริบทของการสนทนาและสร้างคำตอบที่รอบคอบมากขึ้น',
		'This option sets the maximum number of tokens the model can generate in its response. Increasing this limit allows the model to provide longer answers, but it may also increase the likelihood of unhelpful or irrelevant content being generated.':
			'ตัวเลือกนี้ใช้กำหนดจำนวนโทเค็นสูงสุดที่โมเดลสามารถสร้างได้ในคำตอบของตน การเพิ่มขีดจำกัดนี้จะช่วยให้โมเดลตอบได้ยาวขึ้น แต่ก็อาจเพิ่มโอกาสในการสร้างเนื้อหาที่ไม่เป็นประโยชน์หรือไม่เกี่ยวข้องด้วย',
		'This option will delete all existing files in the collection and replace them with newly uploaded files.':
			'ตัวเลือกนี้จะลบไฟล์ทั้งหมดที่มีอยู่ในคอลเลกชันและแทนที่ด้วยไฟล์ที่อัปโหลดใหม่',
		...
		'This will delete <strong>{{NAME}}</strong> and <strong>all its contents</strong>.':
			'การดำเนินการนี้จะลบ <strong>{{NAME}}</strong> และ<strong>เนื้อหาทั้งหมด</strong>',
		'This will delete all models including custom models':
			'การดำเนินการนี้จะลบโมเดลทั้งหมด รวมถึงโมเดลแบบกำหนดเอง',
		'This will delete all models including custom models and cannot be undone.':
			'การดำเนินการนี้จะลบโมเดลทั้งหมดรวมถึงโมเดลที่กำหนดเอง และไม่สามารถยกเลิกได้',
		'This will permanently delete the calendar "{{name}}" and all its events. This action cannot be undone.':
			'',
		'This will reset the knowledge base and sync all files. Do you wish to continue?':
			'การดำเนินการนี้จะรีเซ็ตฐานความรู้และซิงค์ไฟล์ทั้งหมด คุณต้องการดำเนินการต่อหรือไม่?',
		...
		'To access the available model names for downloading,':
			'ในการเข้าถึงชื่อโมเดลที่มีให้ดาวน์โหลดนั้น',
		'To access the GGUF models available for downloading,':
			'ในการเข้าถึงโมเดล GGUF ที่มีให้ดาวน์โหลดได้',
		'To access the WebUI, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.':
			'ในการเข้าถึง WebUI โปรดติดต่อผู้ดูแลระบบ ผู้ดูแลระบบสามารถจัดการสถานะผู้ใช้จากแผงควบคุมผู้ดูแลระบบ',
		'To attach knowledge base here, add them to the "Knowledge" workspace first.':
			'หากต้องการแนบฐานความรู้ที่นี่ โปรดเพิ่มไปที่ Workspace "Knowledge" ก่อน',
		'To learn more about available endpoints, visit our documentation.':
			'หากต้องการเรียนรู้เพิ่มเติมเกี่ยวกับ Endpoints ที่มีอยู่ โปรดดูที่เอกสารของเรา',
		'To select skills here, add them to the "Skills" workspace first.': '',
		'To select toolkits here, add them to the "Tools" workspace first.':
			'ในการเลือกชุดเครื่องมือที่นี่ ให้เพิ่มไปยังพื้นที่ทำงาน "Tools" ก่อน',
		...
		'Tools are a function calling system with arbitrary code execution':
			'เครื่องมือคือระบบเรียกใช้ฟังก์ชันที่สามารถรันโค้ดใดๆ ก็ได้',
		'Tools Function Calling Prompt': 'พรอมต์การเรียกใช้ฟังก์ชันของเครื่องมือ',
		'Tools have a function calling system that allows arbitrary code execution.':
			'เครื่องมือมีระบบเรียกใช้ฟังก์ชันที่สามารถรันโค้ดใดๆ ได้',
		...
		'Upgrade to a licensed plan for enhanced capabilities, including custom theming and branding, and dedicated support.':
			'อัปเกรดเป็นแผนที่มีไลเซนส์เพื่อความสามารถที่เพิ่มขึ้น รวมถึงการปรับธีมและแบรนด์ตามต้องการ และการสนับสนุนเฉพาะทาง',
		Upload: ar,
		...
		'Upload Progress: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)':
			'ความคืบหน้าการอัปโหลด: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)',
		...
		"Use '#' in the prompt input to load and include your knowledge.":
			"ใช้ '#' ในการป้อน Prompt เพื่อโหลดและรวมความรู้ของคุณ",
		'Use /v1/chat/completions endpoint instead of /v1/audio/transcriptions for potentially better accuracy.':
			'ใช้ Endpoint /v1/chat/completions แทน /v1/audio/transcriptions เพื่อความแม่นยำที่อาจดีขึ้น',
		...
		'Use proxy designated by http_proxy and https_proxy environment variables to fetch page contents.':
			'ใช้พร็อกซีที่กำหนดโดยตัวแปรสภาพแวดล้อม http_proxy และ https_proxy เพื่อดึงเนื้อหาหน้าเว็บ',
		user: lr,
		User: cr,
		'User Activity': '',
		...
		'Using the default arena model with all models. Click the plus button to add custom models.':
			'ใช้โมเดล Arena ค่าเริ่มต้นกับโมเดลทั้งหมด คลิกปุ่มบวกเพื่อเพิ่มโมเดลที่กำหนดเอง',
		...
		'Warning: Enabling this will allow users to upload arbitrary code on the server.':
			'คำเตือน: การเปิดใช้งานตัวเลือกนี้จะอนุญาตให้ผู้ใช้อัปโหลดโค้ดใดๆ บนเซิร์ฟเวอร์',
		'Warning: Jupyter execution enables arbitrary code execution, posing severe security risks—proceed with extreme caution.':
			'คำเตือน: การรัน Jupyter อนุญาตให้รันโค้ดใดๆ ก็ได้ ซึ่งก่อให้เกิดความเสี่ยงด้านความปลอดภัยอย่างร้ายแรง—โปรดดำเนินการด้วยความระมัดระวังอย่างยิ่ง',
		We_day_of_week: Sr,
		Web: Ir,
		'Web API': 'เว็บ API',
		...
		'WebUI will make requests to "{{url}}/chat/completions"':
			'WebUI จะส่งคำขอไปยัง "{{url}}/chat/completions"',
		Week: vr,
		Weekly: Cr,
		...
		'When enabled, the model will respond to each chat message in real-time, generating a response as soon as the user sends a message. This mode is useful for live chat applications, but may impact performance on slower hardware.':
			'เมื่อเปิดใช้งาน โมเดลจะตอบกลับแต่ละข้อความแชทแบบเรียลไทม์ โดยสร้างคำตอบทันทีที่ผู้ใช้ส่งข้อความ โหมดนี้มีประโยชน์สำหรับแอปแชทแบบสด แต่อาจส่งผลต่อประสิทธิภาพบนฮาร์ดแวร์ที่ทำงานช้า',
		...
		'Whether to paginate the output. Each page will be separated by a horizontal rule and page number. Defaults to False.':
			'ว่าจะแบ่งหน้าผลลัพธ์หรือไม่ แต่ละหน้าจะถูกแยกด้วยเส้นแบ่งแนวนอนและหมายเลขหน้า ค่าเริ่มต้นคือ False',
		...
		'Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text.':
			'ทำงานร่วมกับ Top K ค่าที่สูงขึ้น (เช่น 0.95) จะนำไปสู่ข้อความที่หลากหลายมากขึ้น ในขณะที่ค่าที่ต่ำลง (เช่น 0.5) จะสร้างข้อความที่มุ่งเน้นและระมัดระวังมากขึ้น',
		Workspace: Dr,
		'Workspace Permissions': 'สิทธิ์ของพื้นที่ทำงาน',
		Write: Mr,
		'Write a summary in 50 words that summarizes {{topic}}.':
			'เขียนสรุป {{topic}} เป็นภาษาไทยให้ได้ 50 คำ',
...
เช่น: คุณคือมาริโอจาก Super Mario Bros และทำหน้าที่เป็นผู้ช่วย`,
		...
		'You are currently using a trial license. Please contact support to upgrade your license.':
			'คุณกำลังใช้สิทธิ์ใช้งานแบบทดลองอยู่ กรุณาติดต่อฝ่ายสนับสนุนเพื่ออัปเกรดสิทธิ์ใช้งานของคุณ',
		'You can only chat with a maximum of {{maxCount}} file(s) at a time.':
			'คุณสามารถแชทกับไฟล์ได้สูงสุด {{maxCount}} ไฟล์ในเวลาเดียวกัน',
		"You can personalize your interactions with LLMs by adding memories through the 'Manage' button below, making them more helpful and tailored to you.":
			'คุณสามารถปรับแต่งการโต้ตอบกับ LLM ของคุณได้โดยเพิ่มความจำผ่านปุ่ม ‘จัดการ’ ด้านล่าง เพื่อให้การใช้งานมีประโยชน์และเหมาะกับคุณมากยิ่งขึ้น',
		...
		'You do not have permission to send messages in this channel.':
			'คุณไม่มีสิทธิ์ส่งข้อความในช่องทางนี้',
		'You do not have permission to send messages in this thread.':
			'คุณไม่มีสิทธิ์ส่งข้อความในเธรดนี้',
		...
		'Your entire contribution will go directly to the plugin developer; Open WebUI does not take any percentage. However, the chosen funding platform might have its own fees.':
			'การสนับสนุนทั้งหมดของคุณจะถูกส่งไปยังนักพัฒนาปลั๊กอินโดยตรง Open WebUI จะไม่หักส่วนแบ่งใดๆ อย่างไรก็ตาม แพลตฟอร์มการระดมทุนที่คุณเลือกอาจมีการเก็บค่าธรรมเนียมในส่วนของตนเอง',
...
//# sourceMappingURL=De_5Dq81.js.map
