...
import { a as J, s as ce, r as fe } from './BfA5TzUS.js';
import { p as ie } from './Bfc47y5P.js';
import { i as le } from './BolXBFGq.js';
import { p as x } from './DbBXQmQ3.js';
import { t as B } from './DtbOSK9Q.js';
import { M as oe } from './sSXaIhmU.js';
import { X as ne } from './D48hh3kS.js';
import { i as me } from './CdIW-EUl.js';
import { b as pe } from './QcGoGJ5n.js';
import { S as xe } from './BqNz_fa8.js';
import { n as be, e as _e } from './BbDdJXLs.js';
var ge = K(
					...
					te(
						(k, R, W, Y, u) => {
							(c(d, k),
								...
								c(z, W),
								c(C, `${Y ?? ''} `),
								J(p, 'href', (Q(b()), i(() => b().funding_url))),
								c(O, (Q(b()), i(() => b().funding_url))),
								c(P, u));
						},
						[
							() => (s(), i(() => s().t('Show your support!'))),
							() => (
								s(),
								i(() =>
									s().t(
...
var he = K('<span class="shrink-0"><!></span>'),
	ye = K(
...
function qe(F, o) {
	...
	let v = x(o, 'show', 12, !1),
		b = x(o, 'onImport', 8, (d) => {});
	x(o, 'onClose', 8, () => {});
	let I = x(o, 'loadUrlHandler', 8, () => {}),
		D = x(o, 'successMessage', 12, ''),
		n = V(!1),
		f = V('');
	const M = async () => {
		if ((q(n, !0), !$(f))) {
			(B.error(s().t('Please enter a valid URL')), q(n, !1));
			return;
		}
		const d = await I()($(f)).catch((a) => (B.error(`${a}`), q(n, !1), null));
		if (d) {
			(D() || D(s().t('Function imported successfully')), B.success(D()));
			let a = d;
			a.id = a.id || be(a.name);
			const r = _e(d.content);
			(r != null && r.title && (a.name = r.title),
				(a.meta = {
					...(a.meta ?? {}),
					description: (r == null ? void 0 : r.description) ?? a.name
				}),
				b()(a),
				v(!1));
		}
				...
				(t(k),
					...
					te(
						(u, S, A, de, ve) => {
							(c(j, u),
								...
								ce(
									k,
									1,
									`px-3.5 py-1.5 text-sm font-medium bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full flex items-center gap-2 whitespace-nowrap ${$(n) ? ' cursor-not-allowed' : ''}`
								),
								(k.disabled = $(n)),
								c(R, `${ve ?? ''} `));
						},
						[
							...
							() => (s(), i(() => s().t('Close'))),
							() => (s(), i(() => s().t('URL'))),
							() => (s(), i(() => s().t('Enter the URL to import'))),
							() => (s(), i(() => s().t('Import')))
						]
					),
					E('click', m, () => {
...
export { qe as I, Re as M };
//# sourceMappingURL=Dfbt9QBv.js.map
