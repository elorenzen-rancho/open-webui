import { p as he } from './BCLU6njT.js';
import { I as $e } from './CL4aqG0y.js';
import {
	q as fe,
	p as ge,
	s as ue,
...
import { p as Ce } from './BqlCGYRq.js';
import { s as Be } from './BX5WB7ra.js';
var p = { NORMAL: 0, REVERSE: 1, HIGHLIGHT: 2, MERGE: 3, CHERRY_PICK: 4 },
	Te = ve.gitGraph,
	D = $(() => ke({ ...Te, ...Q().gitGraph }), 'getConfig'),
	i = new $e(() => {
		const e = D(),
			r = e.mainBranchName,
			t = e.mainBranchOrder;
...
var Le = $(function (e) {
	...
	Ge = $(function (e) {
		...
		if (!t || !i.records.commits.has(t)) {
			...
			const g = {
				...
				tags: o
					? o.filter(Boolean)
					: [`cherry-pick:${n.id}${n.type === p.MERGE ? `|parent:${s}` : ''}`]
			};
			((i.records.head = g),
	...
	ne = $(function (e) {
		if (((e = L.sanitizeText(e, D())), i.records.branches.has(e))) {
			...
			r === void 0 || !r
				? (i.records.head = null)
				: (i.records.head = i.records.commits.get(r) ?? null);
		} else {
			const r = new Error(
...
var He = $(function () {
	...
	Ae = $(function () {
		return [...i.records.branchConfig.values()]
			.map((r, t) =>
				r.order !== null && r.order !== void 0 ? r : { ...r, order: parseFloat(`0.${t}`) }
			)
	...
	A = $((e, r, t = !1) => (t && e > 0 ? ((e - 1) % (r - 1)) + 1 : e % r), 'calcColorIndex'),
	B = new Map(),
	T = new Map(),
	j = 30,
	N = new Map(),
	...
	y = 'LR',
	Qe = $(() => {
		(B.clear(), T.clear(), N.clear(), (O = 0), (U = []), (y = 'LR'));
	}, 'clear'),
	ce = $((e) => {
	...
	sr = $((e, r, t, o, s, d) => {
		...
		if (d === p.HIGHLIGHT)
			(e
				...
				.attr('width', a ? 14 : 20)
				.attr('height', a ? 14 : 20)
				.attr('class', `commit ${r.id} commit-highlight${A(s, S, l)} ${o}-outer`),
				e
					.append('rect')
					.attr('x', t.x - 6 + (a ? 2 : 0))
					.attr('y', t.y - 6 + (a ? 2 : 0))
					.attr('width', a ? 8 : 12)
					.attr('height', a ? 8 : 12)
					.attr('class', `commit ${r.id} commit${A(s, S, l)} ${o}-inner`));
			...
			if (
				...
				g.attr('cy', t.y),
				g.attr('r', a ? 7 : 10),
				g.attr('class', `commit ${r.id} commit${A(s, S, l)}`),
				d === p.MERGE)
			) {
				...
				(f.attr('cx', t.x),
					f.attr('cy', t.y),
					f.attr('r', a ? 5 : 6),
					f.attr('class', `commit ${o} ${r.id} commit${A(s, S, l)}`));
			}
			if (d === p.REVERSE) {
				...
				).attr('class', `commit ${o} ${r.id} commit${A(s, S, l)}`);
			}
		}
	}, 'drawCommitBullet'),
	or = $((e, r, t, o, s) => {
		var d;
		if (
			r.type !== p.CHERRY_PICK &&
			((r.customId && r.type === p.MERGE) || r.type !== p.MERGE) &&
			s.showCommitLabel
		) {
			const n = e.append('g'),
			...
			if (
				h &&
				(a
					.attr('x', t.posWithOffset - h.width / 2 - I)
					.attr('y', t.y + 13.5)
					.attr('width', h.width + 2 * I)
					.attr('height', h.height + 2 * I),
				y === 'TB' || y === 'BT'
	...
	$r = $((e, r, t, o) => {
		...
		t.type === p.MERGE &&
			r.id !== t.parents[0] &&
			(x = (E = B.get(r.branch)) == null ? void 0 : E.index);
		let u;
		if (l) {
		...
		e.append('path')
			.attr('d', u)
			.attr('class', 'arrow arrow' + A(x, S, d));
	}, 'drawArrow'),
	fr = $((e, r) => {
			...
			(s === 'neo' && v.attr('data-look', 'neo'),
				...
				y === 'TB'
					? (v.attr('x', m - C.width / 2 - 10).attr('y', 0),
						R.attr('transform', 'translate(' + (m - C.width / 2 - 5) + ', 0)'),
						h &&
							(v.attr('transform', `translate(${-Y / 2 - 3}, ${-P - 10})`),
							R.attr(
								'transform',
								'translate(' + (m - C.width / 2 - 5) + ', ' + (-P * 2 + 7) + ')'
							)))
					: y === 'BT'
						? (v.attr('x', m - C.width / 2 - 10).attr('y', O),
							R.attr('transform', 'translate(' + (m - C.width / 2 - 5) + ', ' + O + ')'),
							h &&
								(v.attr('transform', `translate(${-Y / 2 - 3}, ${P + 10})`),
								R.attr(
									'transform',
									'translate(' + (m - C.width / 2 - 5) + ', ' + (O + P * 2 + 4) + ')'
								)))
	...
	ur = $(function (e, r, t, o, s) {
		return (
			B.set(e, { pos: r, index: t }),
			(r += 50 + (s ? 40 : 0) + (y === 'TB' || y === 'BT' ? o.width / 2 : 0)),
			r
		);
	}, 'setBranchPosition'),
	yr = $(function (e, r, t, o) {
...
`,
		...
		(a.forEach((b, q) => {
			...
			const _ = ce(b.name),
				...
				R = w.insert('g').attr('class', 'label branch-label');
			(W = R.node()) == null || W.appendChild(_);
			const C = _.getBBox();
			((E = ur(b.name, E, q, C, n)), R.remove(), w.remove(), v.remove());
		}),
			te(l, N, !1, d),
			d.showBranches && gr(l, a, d, r),
			fr(l, N),
			te(l, N, !0, d),
			pe.insertTitle(l, 'gitTitleText', d.titleTopMargin ?? 0, s.getDiagramTitle()),
			be(void 0, l, d.diagramPadding, d.useMaxWidth));
	}, 'draw'),
	xr = { draw: yr },
	de = 8,
...
`;
	...
	Br = Cr,
	Or = { parser: Xe, db: oe, renderer: xr, styles: Br };
export { Or as diagram };
//# sourceMappingURL=DhBksfut.js.map
