import { L as W, E as i, C as B } from './COV9u0MG.js';
import {
	s as U,
	t as n,
	L as b,
...
const fO = new i(
		(O, e) => {
			...
			if ((P(a) || a < 0) && e.context.type != d) {
				...
				(r < e.context.depth ||
					(r == e.context.depth && e.context.type == T && (O.next != 45 || !l(O.peek(1))))) &&
					O.next != -1 &&
					!P(O.next) &&
					O.next != 35 &&
					O.acceptToken(c, -r);
	...
	XO = new i(
		(O, e) => {
			...
			if (O.next == 45)
				(O.advance(),
					l(O.next) &&
						O.acceptToken(e.context.type == T && e.context.depth == X(O, O.pos - 1) ? H : y));
			else if (O.next == 63)
				(O.advance(),
					l(O.next) &&
						O.acceptToken(e.context.type == g && e.context.depth == X(O, O.pos - 1) ? F : Z));
			else {
				let a = O.pos;
				for (;;)
					if (x(O.next)) {
...
const iO = new i((O, e) => {
	...
	kO = new i((O, e) => {
		...
		O: for (;;) {
			...
			if (
				(!o && (f(O, 45, o) || f(O, 46, o))) ||
				(!P(t) && (a < 0 && (a = Math.max(e.context.depth + 1, o)), o < a))
			)
				break;
			for (;;) {
				if (O.next < 0) break O;
...
const xO = C.define({
	...
	parser: mO.configure({ props: [U({ DashLine: n.meta })] })
});
function vO(O) {
	let { language: e, support: a } =
		O.content instanceof b ? O.content : { language: O.content, support: [] };
	return new b(
		xO.configure({
			wrap: E((r) =>
				r.name == 'FrontmatterContent'
					? { parser: L.parser }
					: r.name == 'Body'
						? { parser: e.parser }
						: null
...
//# sourceMappingURL=DhT1KESM.js.map
