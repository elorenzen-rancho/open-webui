...
import { p as V } from './BCLU6njT.js';
import { p as H } from './BqlCGYRq.js';
var h = { showLegend: !0, ticks: 5, max: null, min: 0, graticule: 'circle' },
	w = { axes: [], curves: [], options: h },
	x = structuredClone(w),
	j = z.radar,
	q = l(() => f({ ...j, ...C().radar }), 'getConfig'),
	b = l(() => x.axes, 'getAxes'),
	K = l(() => x.curves, 'getCurves'),
	N = l(() => x.options, 'getOptions'),
	U = l((a) => {
		x.axes = a.map((t) => ({ name: t.name, label: t.label ?? t.name }));
	}, 'setAxes'),
	X = l((a) => {
		x.curves = a.map((t) => ({ name: t.name, label: t.label ?? t.name, entries: Y(t.entries) }));
	}, 'setCurves'),
	Y = l((a) => {
...
var nt = { draw: et },
	...
	pt = { parser: tt, db: $, renderer: nt, styles: lt };
