...
import { p as _ } from './BCLU6njT.js';
import { p as N } from './BqlCGYRq.js';
var I = E.packet,
	u,
	v =
		((u = class {
		...
		for (let { start: a, end: i, bits: d, label: c } of e.blocks) {
			...
			for (
				i ?? (i = a + (d ?? 1) - 1),
					d ?? (d = i - a + 1),
					s = i,
					w.debug(`Packet block ${a} - ${s} with label ${c}`);
				r.length <= l + 1 && t.getPacket().length < L;
			) {
				const [p, o] = Y({ start: a, end: i, bits: d, label: c }, n, l);
				if ((r.push(p), p.end + 1 === n * l && (t.pushWord(r), (r = []), n++), !o)) break;
	...
	O = b(
		(
			...
			t,
			s,
			{ rowHeight: r, paddingX: n, paddingY: l, bitWidth: a, bitsPerRow: i, showBits: d }
		) => {
			const c = e.append('g'),
				p = s * (r + l) + l;
			for (const o of t) {
