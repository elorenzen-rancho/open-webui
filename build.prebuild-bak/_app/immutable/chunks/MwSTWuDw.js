import './CWj6FrbW.js';
import './69_IOA4Y.js';
import { t as i, a as m, F as c } from './CD8kWLvT.js';
import { a as p, s as l, c as n } from './BfA5TzUS.js';
import { p as s } from './DbBXQmQ3.js';
var v = c(
	'<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 0 1 1.5.124m7.5 10.376h3.375c.621 0 1.125-.504 1.125-1.125V11.25c0-4.46-3.243-8.161-7.5-8.876a9.06 9.06 0 0 0-1.5-.124H9.375c-.621 0-1.125.504-1.125 1.125v3.5m7.5 10.375H9.375a1.125 1.125 0 0 1-1.125-1.125v-9.25m12 6.625v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H9.75"></path></svg>'
);
function g(e, a) {
	let o = s(a, 'className', 8, 'w-4 h-4'),
		r = s(a, 'strokeWidth', 8, '1.5');
	var t = v();
	(i(() => {
		(p(t, 'stroke-width', r()), l(t, 0, n(o())));
	}),
		m(e, t));
}
export { g as D };
//# sourceMappingURL=MwSTWuDw.js.map
