import { L as s, E as R, a as Y, C as x } from './COV9u0MG.js';
import {
	s as w,
	t as O,
	L as d,
...
const _ = 177,
	...
	aO = new R(
		(Q, e) => {
			for (
				let n = 0, a = Q.next;
				((e.context && (a < 0 || a == M || a == H || (a == $ && Q.peek(n + 1) == $))) ||
					a == QO ||
					a == eO) &&
					Q.acceptToken(_),
					!(a != K && a != OO);
	...
	return (
		e.cursor(Z.IncludeAnonymous).iterate((i) => {
			if (t) t = !1;
				...
				for (let P of W(Q, i.node)) a.push(P);
				return !1;
			}
		}),
		S.set(e, a),
...
const p = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
	m = ['String', 'LineComment', 'BlockComment', 'DefName', 'LabelName', 'FieldName', '.', '?.'],
	rO = (Q) => {
		let e = G(Q.state).resolveInner(Q.pos, -1);
		if (m.indexOf(e.name) > -1) return null;
		let n =
			e.name == 'VariableName' || (e.to - e.from < 20 && p.test(Q.state.sliceDoc(e.from, e.to)));
		if (!n && !Q.explicit) return null;
		let a = [];
		for (let t = e; t; t = t.parent) T.has(t.name) && (a = a.concat(W(Q.state.doc, t)));
...
function TO() {
	let Q = oO.concat($O);
	return new d(r, [r.data.of({ autocomplete: k(m, h(Q)) }), r.data.of({ autocomplete: rO })]);
}
export { TO as go, r as goLanguage, rO as localCompletionSource, oO as snippets };
//# sourceMappingURL=TzVw0Tyr.js.map
