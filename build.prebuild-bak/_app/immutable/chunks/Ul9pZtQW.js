import { _ as V, l as $ } from './CZJnjZH0.js';
import { c as J } from './BTOT7Bb_.js';
import { c as Q, g as lt } from './BC4znSyB.js';
import { s as gt } from './BX5WB7ra.js';
var tt = { exports: {} },
	k = { exports: {} },
	Z = { exports: {} },
	q;
								...
								(h.prototype.updateBounds = function (p) {
									...
									(v == e.MAX_VALUE &&
										...
										L[0].getParent().paddingLeft != null
											? (c = L[0].getParent().paddingLeft)
											: (c = this.margin),
										(this.left = C.x - c),
										(this.right = C.x + C.width + c),
										(this.top = C.y - c),
										(this.bottom = C.y + C.height + c));
								}),
							...
							((e.calcSeparationAmount = function (t, i, l, g) {
								...
								(e.getIntersection = function (t, i, l, g) {
									...
									return (
										...
										A === 0
											? null
											: ((u = (s * L - f * c) / A), (E = (O * c - y * L) / A), new o(u, E))
									);
								}),
						...
						function (N, I, T) {
							...
							((n.prototype.initParameters = function () {
								...
								(n.prototype.calcIdealEdgeLengths = function () {
									for (
										var r, h, a, p, v, D, u = this.getGraphManager().getAllEdges(), E = 0;
										E < u.length;
										E++
									)
										((r = u[E]),
							...
							((y.prototype.newGraphManager = function () {
									...
									for (c = 0; c < s.length; c++) {
										...
										if (!f.has(L)) {
											...
											if (A == m)
												...
												var R = [];
												if (
													((R = R.concat(A.getEdgeListToNode(m))),
													(R = R.concat(m.getEdgeListToNode(A))),
													!f.has(R[0]))
												) {
													if (R.length > 1) {
														var C;
								...
								(y.prototype.positionNodesRadially = function (s) {
									for (
										...
										C % c == 0 &&
											((m = 0), (A = L), C != 0 && (A += n.DEFAULT_COMPONENT_SEPERATION), (L = 0));
										var M = s[C],
											S = p.findCenterOfTree(M);
										((f.x = m),
											(f.y = A),
											(R = y.radialLayout(M, S, f)),
											R.y > L && (L = Math.floor(R.y)),
											(m = Math.floor(R.x + n.DEFAULT_COMPONENT_SEPERATION)));
									}
								...
								(y.prototype.tileNodes = function (s, f) {
									...
									s.sort(function (C, M) {
										return C.rect.width * C.rect.height > M.rect.width * M.rect.height
											? -1
											: C.rect.width * C.rect.height < M.rect.width * M.rect.height
												? 1
												: 0;
									});
									for (var m = 0; m < s.length; m++) {
										var R = s[m];
										(A.rows.length == 0
											? this.insertNodeToRow(A, R, 0, f)
											: this.canAddHorizontal(A, R.rect.width, R.rect.height)
												? this.insertNodeToRow(A, R, this.getShortestRowIndex(A), f)
												: this.insertNodeToRow(A, R, A.rows.length, f),
								...
								(y.prototype.insertNodeToRow = function (s, f, c, L) {
									...
									(s.rows[c].length > 0 && (R += s.horizontalPadding),
										(s.rowWidth[c] = R),
										s.width < R && (s.width = R));
									var C = f.rect.height;
									...
									var M = 0;
									(C > s.rowHeight[c] &&
										((M = s.rowHeight[c]), (s.rowHeight[c] = C), (M = s.rowHeight[c] - M)),
										(s.height += M),
										s.rows[c].push(f));
								}),
								...
								(y.prototype.canAddHorizontal = function (s, f, c) {
									...
									(s.width - A >= f + s.horizontalPadding
										? (R = (s.height + m) / (A + f + s.horizontalPadding))
										: (R = (s.height + m) / s.width),
										(m = c + s.verticalPadding));
									var C;
									return (
										s.width < f ? (C = (s.height + m) / f) : (C = (s.height + m) / s.width),
										C < 1 && (C = 1 / C),
										R < 1 && (R = 1 / R),
										R < C
									);
									...
									for (var s = [], f = !0, c; f; ) {
										...
										if (f == !0) {
											for (var R = [], C = 0; C < A.length; C++)
												A[C][0].getEdges().length == 1 &&
													(R.push(A[C]), A[C][0].getOwner().remove(A[C][0]));
											(s.push(R),
...
async function st(G, b) {
	...
	try {
		...
		const N = await it(G),
			...
			T = ot(N);
		return (
			$.debug(`Layout completed: ${I.length} nodes, ${T.length} edges`),
			{ nodes: I, edges: T }
		);
	} catch (N) {
...
var dt = V(
	...
	Tt = dt;
export { Tt as render };
//# sourceMappingURL=Ul9pZtQW.js.map
