const e = 'О программе',
	...
	Fr = {
		'-1 for no limit, or a positive integer for a specific limit':
			'-1 для отсутствия ограничений или положительное целое число для определенного ограничения.',
		"'s', 'm', 'h', 'd', 'w' or '-1' for no expiration.":
			"'s', 'm', 'h', 'd', 'w' или '-1' чтобы был без срока годности.",
		'(e.g. `sh webui.sh --api --api-auth username_password`)':
			'(например, `sh webui.sh --api --api-auth username_password`)',
		'(e.g. `sh webui.sh --api`)': '(например, `sh webui.sh --api`)',
		'(latest)': '(последняя)',
		'(leave blank for to use commercial endpoint)':
			'(оставьте пустым поле для использования коммерческой конечной точки)',
		...
		'*Prompt node ID(s) are required for image generation':
			'ID узлов промптов обязательны для генерации изображения',
		'1 hour before': '',
		'1 Source': '1 Источник',
		'10 minutes before': '',
		'15 minutes before': '',
		...
		'A collaboration channel where people join as members':
			'Канал для совместной работы с присоединением участников',
		'A discussion channel where access is controlled by groups and permissions':
			'Обсуждение канала, где доступ контролируется группами и разрешениями',
		'A new version (v{{LATEST_VERSION}}) is now available.':
			'Новая версия (v{{LATEST_VERSION}}) теперь доступна.',
		'A private conversation between you and selected users':
			'Приватный чат между вами и выбранными пользователями',
		'A task model is used when performing tasks such as generating titles for chats and web search queries':
			'Модель задач используется при выполнении таких задач, как генерация заголовков для чатов и поисковых запросов в Интернете',
		'a user': 'пользователь',
		About: e,
...
Перейти к переменной запроса`,
		...
		'Activate this command by typing "/{{COMMAND}}" to chat input.':
			'Активируйте эту команду, набрав "/{{COMMAND}}" для ввода в чате.',
		Active: r,
		...
		'Add a short description about what this model does':
			'Добавьте краткое описание того, что делает эта модель',
		...
		'Add your Open Terminal URL and API key in Settings → Integrations.':
			'Добавьте URL и API-ключ Open Terminal в Настройки → Интеграции.',
		...
		'Adds filenames, titles, sections, and snippets into the BM25 text to improve lexical recall.':
			'Добавляет имена файлов, заголовки, разделы и фрагменты в текст BM25 для улучшения лексического поиска.',
		'Adjusting these settings will apply changes universally to all users.':
			'Изменения в этих настройках будут применены для всех пользователей.',
		admin: c,
		Admin: d,
		...
		'Admins have access to all tools at all times; users need tools assigned per model in the workspace.':
			'Администраторы всегда имеют доступ ко всем инструментам; пользователям нужны инструменты, назначенные для каждой модели в рабочем пространстве.',
		Advanced: u,
		'Advanced Parameters': 'Расширенные параметры',
		'Advanced parameters for MinerU parsing (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)':
			'Дополнительные параметры для анализа MinerU (enable_ocr, enable_formula, enable_table, language, model_version, page_ranges)',
		...
		'After updating or changing the embedding model, you must reindex the knowledge base for the changes to take effect. You can do this using the "Reindex" button below.':
			'После обновления или изменения модели встраивания необходимо пересоздать базу знаний для применения изменений. Вы можете это сделать, используя кнопку "Пересоздать" ниже.',
		AI: m,
		All: p,
		...
		'Allow Chat Valves': 'Разрешить вентили чата',
		...
		'API Base URL for Datalab Marker service. Defaults to: https://www.datalab.to/api/v1/marker':
			'Базовый URL API для сервиса Datalab Marker. По умолчанию: https://www.datalab.to/api/v1/marker',
		'API Key': 'Ключ API',
		...
		'applies to all users with the "user" role':
			'применяется ко всем пользователям с ролью «пользователь»',
		April: w,
		Archive: S,
		...
		'Are you sure you want to archive all chats? This action cannot be undone.':
			'Вы уверены, что хотите архивировать все чаты? Это действие нельзя отменить.',
		'Are you sure you want to clear all memories? This action cannot be undone.':
			'Вы уверены, что хотите удалить все воспоминания? Это действие нельзя отменить.',
		...
		"Are you sure you want to delete this version? Child versions will be relinked to this version's parent.":
			'Вы уверены, что хотите удалить эту версию? Дочерние версии будут привязаны к родительской.',
		...
		'Are you sure you want to unarchive all archived chats?':
			'Вы уверены, что хотите разархивировать все заархивированные чаты?',
		...
		'Automatically inject system tools in native function calling mode (e.g., timestamps, memory, chat history, notes, etc.)':
			'Автоматически подключать системные инструменты в режиме вызова функций (временные метки, воспоминания, история чата, заметки и т.д.)',
		Automation: L,
		'Automation created': '',
		'Automation Name': '',
		'Automation title': '',
		...
		'Base Model List Cache speeds up access by fetching base models only at startup or on settings save—faster, but may not show recent base model changes.':
			'Кэш списка базовых моделей ускоряет доступ, загружая базовые модели только при запуске или сохранении настроек — быстрее, но может не показывать новые изменения в базовых моделях.',
		Bearer: W,
		before: K,
		'Being lazy': 'Лениво',
		Beta: G,
		...
		'Boosting or penalizing specific tokens for constrained responses. Bias values will be clamped between -100 and 100 (inclusive). (Default: none)':
			'Увеличение или отмена определенных токенов за ограниченные ответы. Значения смещения будут находиться в диапазоне от -100 до 100 (включительно). (По умолчанию: ничего)',
		Brave: J,
		...
		'Call feature is not supported when using Web STT engine':
			'Функция вызова не поддерживается при использовании Web STT (распознавание речи) движка',
		Camera: z,
		Cancel: Q,
		...
		'Character limit for autocomplete generation input':
			'Ограничение количества символов для ввода при генерации автозаполнения',
		...
		'Chunks smaller than this threshold will be merged with neighboring chunks when possible. Set to 0 to disable merging.':
			'Чанки меньшего размера будут объединены с соседними чанками при возможности. Установите 0 для отключения объединения.',
		Ciphers: se,
		Citation: re,
		Citations: ie,
		...
		'Click here to download user import template file.':
			'Нажмите здесь, чтобы загрузить файл шаблона импорта пользователя',
		'Click here to learn more about faster-whisper and see the available models.':
			'Нажмите здесь, чтобы узнать больше о faster-whisper и ознакомиться с доступными моделями.',
		'Click here to see available models.':
			'Нажмите здесь, чтобы ознакомиться с доступными моделями.',
		...
		'Click here to upload a workflow.json file.':
			'Нажмите здесь, чтобы загрузить файл workflow.json.',
		'click here.': 'нажмите здесь.',
		"Click on the user role button to change a user's role.":
			'Нажмите кнопку роли пользователя, чтобы изменить роль пользователя.',
		'Click to connect': 'Нажмите для подключения',
		'Click to copy ID': 'Нажмите, чтобы скопировать ID',
		'Client ID': 'Client ID',
		'Client Secret': 'Client Secret',
		'Clipboard write permission denied. Please check your browser settings to grant the necessary access.':
			'В разрешении на запись в буфер обмена отказано. Пожалуйста, проверьте настройки своего браузера, чтобы предоставить необходимый доступ.',
		Clone: le,
		...
		'Connect to Open Terminal instances to browse files and use them as always-on tools. Only one can be active at a time.':
			'Подключайтесь к экземплярам Open Terminal для просмотра файлов и использования их как постоянно доступных инструментов. Одновременно может быть активен только один.',
		'Connect to Open Terminal instances. All users will have access to file browsing and terminal tools through these servers.':
			'Подключение к экземплярам Open Terminal. Все пользователи получат доступ к файлам и терминалу через эти серверы.',
		'Connect to your own OpenAI compatible API endpoints.':
			'Подключитесь к своим собственным энд-поинтам API, совместимым с OpenAI.',
		'Connect to your own OpenAPI compatible external tool servers.':
			'Подключитесь к вашим собственным внешним инструментальным серверам, совместимым с OpenAPI.',
		...
		'Constrains effort on reasoning for reasoning models. Only applicable to reasoning models from specific providers that support reasoning effort.':
			'Ограничивает усилия по обоснованию для моделей обоснования. Применимо только к моделям обоснования от конкретных поставщиков, которые поддерживают усилия по обоснованию.',
		...
		"Control how message text is split for TTS requests. 'Punctuation' splits into sentences, 'paragraphs' splits into paragraphs, and 'none' keeps the message as a single string.":
			"Управляйте разделением текста сообщения для запросов TTS. 'Пунктуация' разделяет на предложения, 'абзацы' - разделяет на абзацы, а 'нет' сохраняет сообщение в виде одной строки.",
		'Control the repetition of token sequences in the generated text. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 1.1) will be more lenient. At 1, it is disabled.':
			'Контролируйте повторение последовательностей символов в сгенерированном тексте. Более высокое значение (например, 1.5) будет более строгим наказанием за повторения, в то время как более низкое значение (например, 1.1) будет более мягким. При значении 1 оно отключено.',
		Controls: Te,
		'Controls the balance between coherence and diversity of the output. A lower value will result in more focused and coherent text.':
			'Контролирует баланс между согласованностью и разнообразием результатов. Меньшее значение приведет к получению более сфокусированного и связного текста.',
		...
		'CORS must be properly configured by the provider to allow requests from Open WebUI.':
			'CORS должен быть должным образом настроен провайдером, чтобы разрешать запросы из Open WebUI.',
		...
		'Create your first note by clicking on the plus button below.':
			'Создайте свою первую заметку, нажав на кнопку плюс ниже.',
		'Created at': 'Создан(а)',
		'Created At': 'Создано',
		'Created by': 'Создано',
		...
		"Default mode works with a wider range of models by calling tools once before execution. Native mode leverages the model's built-in tool-calling capabilities, but requires the model to inherently support this feature.":
			'Режим по умолчанию работает с более широким спектром моделей, вызывая инструменты один раз перед выполнением. Режим Нативно использует встроенные в модель возможности вызова инструментов, но требует, чтобы модель изначально поддерживала эту функцию.',
		...
		'Default to segmented retrieval for focused and relevant content extraction, this is recommended for most cases.':
			'По умолчанию используется сегментированный поиск для целенаправленного извлечения релевантного контента, что рекомендуется в большинстве случаев.',
		...
		'Deployment names are required for Azure OpenAI':
			'Для Azure OpenAI требуются названия развертываний',
		Desc: Be,
		...
		'Direct Connections allow users to connect to their own OpenAI compatible API endpoints.':
			'Прямые подключения позволяют пользователям подключаться к своим собственным конечным точкам API, совместимым с OpenAI.',
		...
		'Disable image extraction from the PDF. If Use LLM is enabled, images will be automatically captioned. Defaults to False.':
			'Отключить извлечение изображений из PDF. Если включена параметр Использовать LLM, изображения будут подписаны автоматически. По умолчанию установлено значение Выкл.',
		Disabled: He,
		'Disconnect OAuth': '',
		...
		'Discover how to use Open WebUI and seek support from the community.':
			'Узнайте, как использовать Open WebUI, и обратитесь за поддержкой к сообществу.',
		'Discover wonders': 'Откройте для себя чудеса',
		'Discover, download, and explore custom functions':
			'Открывайте для себя, загружайте и исследуйте пользовательские функции',
		'Discover, download, and explore custom prompts':
			'Открывайте для себя, загружайте и исследуйте пользовательские промпты',
		'Discover, download, and explore custom tools':
			'Открывайте для себя, загружайте и исследуйте пользовательские инструменты',
		'Discover, download, and explore model presets':
			'Открывайте для себя, загружайте и исследуйте пользовательские предустановки моделей',
		'Discussion channel where access is based on groups and permissions':
			'Канал обсуждений с доступом по группам и правам',
		Display: je,
		...
		'Displays status updates (e.g., web search progress) in the response':
			'Отображает обновления статуса (например, прогресс поиска в сети) в ответе',
		'Dive into knowledge': 'Погрузитесь в знания',
		'Do not install functions from sources you do not fully trust.':
			'Не устанавливайте функции из источников, которым вы не полностью доверяете.',
		'Do not install tools from sources you do not fully trust.':
			'Не устанавливайте инструменты из источников, которым вы не полностью доверяете.',
		'Do you want to sync your usage stats with Open WebUI Community?':
			'Хотите синхронизировать статистику использования с сообществом Open WebUI?',
		Docling: ze,
		...
		'does not make any external connections, and your data stays securely on your locally hosted server.':
			'не устанавливает никаких внешних соединений, и ваши данные надежно хранятся на вашем локальном сервере.',
		'Domain Filter List': 'Список доменных фильтров',
		"don't fetch random pipelines from sources you don't trust.":
			'Не получайте случайные pipelines из источников, которым вы не доверяете.',
		"Don't have an account?": 'У вас нет аккаунта?',
		"don't install random functions from sources you don't trust.":
			'не устанавливайте случайные функции из источников, которым вы не доверяете.',
		"don't install random tools from sources you don't trust.":
			'не устанавливайте случайные инструменты из источников, которым вы не доверяете.',
		...
		"e.g. '30s','10m'. Valid time units are 's', 'm', 'h'.":
			"например, '30s','10m'. Допустимые единицы времени: 's', 'm', 'h'.",
		...
		'e.g. A filter to remove profanity from text':
			'например, фильтр для удаления ненормативной лексики из текста',
		...
		'e.g. Step-by-step instructions for code reviews':
			'например, Пошаговые инструкции для код-ревью',
		'e.g. Tell me a fun fact': 'например, Расскажи интересный факт',
		'e.g. Tell me a fun fact about the Roman Empire':
			'например, Расскажи интересный факт о Римской империи',
		'e.g. Tools for performing various operations':
			'например, инструменты для выполнения различных операций',
		'e.g., 3, 4, 5 (leave blank for default)':
			'например, 3, 4, 5 (оставьте поле пустым по умолчанию)',
		'e.g., audio/wav,audio/mpeg,video/* (leave blank for defaults)':
			'например, audio/wav,audio/mpeg,video/* (оставьте пустым для значений по умолчанию)',
		'e.g., en-US,ja-JP (leave blank for auto-detect)':
			'например, en-US,ja-JP (оставьте поле пустым для автоматического определения)',
		...
		'Enable autocomplete generation for chat messages':
			'Включить генерацию автозаполнения для сообщений чата',
		...
		'Enable, disable, or customize the reasoning tags used by the model. "Enabled" uses default tags, "Disabled" turns off reasoning tags, and "Custom" lets you specify your own start and end tags.':
			'Включить, отключить или настроить теги рассуждения, используемые моделью. "Включено" использует теги по умолчанию, "Отключено" выключает теги рассуждения, а "Пользовательские" позволяет указать собственные начальные и конечные теги.',
		Enabled: mt,
		...
		'Ensure your CSV file includes 4 columns in this order: Name, Email, Password, Role.':
			'Убедитесь, что ваш CSV-файл включает в себя 4 столбца в следующем порядке: Имя, Электронная почта, Пароль, Роль.',
		'Enter {{role}} message here': 'Введите сообщение {{role}} здесь',
		'Enter a detail about yourself for your LLMs to recall':
			'Введите детали о себе, чтобы LLMs могли запомнить',
		'Enter a title for the pending user info overlay. Leave empty for default.':
			'Введите заголовок информационного оверлея для ожидающего пользователя. Оставьте поле пустым для параметра по умолчанию.',
		'Enter a watermark for the response. Leave empty for none.':
			'Укажите водяной знак для ответа. Оставьте пустым чтобы не было никакого.',
		'Enter additional headers in JSON format': 'Введите дополнительные заголовки в формате JSON',
		'Enter additional headers in JSON format (e.g. {"X-Custom-Header": "value"}':
			'Введите дополнительные заголовки в формате JSON (напр. {"X-Custom-Header": "value"}',
		...
		'Enter api auth string (e.g. username:password)':
			'Введите строку авторизации api (например, username:password)',
		...
		'Enter content for the pending user info overlay. Leave empty for default.':
			'Введите содержимое информационного оверлея для ожидающего пользователя. Оставьте поле пустым для параметра по умолчанию.',
		...
		'Enter domains separated by commas (e.g., example.com,site.org,!excludedsite.com)':
			'Введите домены через запятую (напр. example.com,site.org,!excludedsite.com)',
		...
		'Enter function name filter list (e.g. func1, !func2)':
			'Введите список фильтров по именам функций (напр. func1, !func2)',
		...
		'Enter JSON config (e.g., {"disable_links": true})':
			'Введите JSON конфигурацию (напр., {"disable_links": true})',
		...
		'Enter proxy URL (e.g. https://user:password@host:port)':
			'Введите URL прокси-сервера (например, https://user:password@host:port)',
		...
		'Enter the public URL of your WebUI. This URL will be used to generate links in the notifications.':
			'Введите общедоступный URL вашего WebUI. Этот URL будет использоваться для создания ссылок в уведомлениях.',
		...
		'Enter Yacy URL (e.g. http://yacy.example.com:8090)':
			'Введите URL-адрес Yacy (напр., http://yacy.example.com:8090)',
		...
		"Error: A model with the ID '{{modelId}}' already exists. Please select a different ID to proceed.":
			"Ошибка: Модель с ID '{{modelId}}' уже существует. Пожалуйста, выберите другой ID для продолжения.",
		'Error: Model ID cannot be empty. Please enter a valid ID to proceed.':
			'Ошибка: ID модели не может быть пустым. Пожалуйста, введите корректный ID для продолжения.',
		Evaluations: yt,
		'Event created': '',
		'Event deleted': '',
		'Event title': '',
		...
		'Example: (&(objectClass=inetOrgPerson)(uid=%s))':
			'Например: (&(objectClass=inetOrgPerson)(uid=%s))',
		...
		'Example: sAMAccountName or uid or userPrincipalName':
			'Например: sAMAccountName или uid или userPrincipalName',
		'Exceeded the number of seats in your license. Please contact support to increase the number of seats.':
			'Превышено количество мест в вашей лицензии. Пожалуйста, свяжитесь со службой поддержки, чтобы увеличить количество мест.',
		Exclude: ft,
		...
		'Failed to connect to {{URL}} OpenAPI tool server':
			'Не удалось подключиться к серверу инструмента OpenAI {{URL}}',
		'Failed to connect to {{URL}} terminal server':
			'Не удалось подключиться к серверу терминала {{URL}}',
		...
		'Failed to extract content from the file: {{error}}':
			'Не удалось извлечь содержимое из файла: {{error}}',
		...
		'Failed to load DOCX file. Please try downloading it instead.':
			'Не удалось загрузить файл DOCX. Попробуйте скачать его.',
		'Failed to load Excel/CSV file. Please try downloading it instead.':
			'Не удалось загрузить файл Excel/CSV. Попробуйте скачать его.',
		...
		'Failed to load PPTX file. Please try downloading it instead.':
			'Не удалось загрузить файл PPTX. Попробуйте скачать его.',
		...
		'Fingerprint spoofing detected: Unable to use initials as avatar. Defaulting to default profile image.':
			'Определение подделки отпечатка: Невозможно использовать инициалы в качестве аватара. По умолчанию используется изображение профиля по умолчанию.',
		...
		'Force OCR on all pages of the PDF. This can lead to worse results if you have good text in your PDFs. Defaults to False.':
			'Принудительное OCR на всех страницах PDF-файла. Это может привести к ухудшению результатов, если в ваших PDF-файлах есть хороший текст. По умолчанию установлено значение Выкл.',
		...
		'Format the lines in the output. Defaults to False. If set to True, the lines will be formatted to detect inline math and styles.':
			'Форматировать строки в выводе. По умолчанию False. Если установлено в True, строки будут отформатированы для обнаружения встроенной математики и стилей.',
		'Formatting may be inconsistent from source.':
			'Форматирование может быть несогласованным с источником.',
		Forward: Ft,
		'Forwards system user OAuth access token to authenticate':
			'Передаёт токен доступа OAuth системного пользователя для аутентификации',
		'Forwards system user session credentials to authenticate':
			'Перенаправляет учетные данные сеанса системного пользователя для проверки подлинности',
		Fr_day_of_week: _t,
		...
		'Get current time and perform date/time calculations':
			'Получить текущее время и выполнить вычисления с датой/временем',
		...
		'Hex Color - Leave empty for default color':
			'Цвет Hex - оставьте пустым значение цвета по умолчанию',
		Hidden: no,
		Hide: ao,
		'Hide All': 'Скрыть Все',
		...
		'I acknowledge that I have read and I understand the implications of my action. I am aware of the risks associated with executing arbitrary code and I have verified the trustworthiness of the source.':
			'Я подтверждаю, что прочитал и осознаю последствия своих действий. Я осознаю риски, связанные с выполнением произвольного кода, и я проверил достоверность источника.',
		ID: mo,
		...
		'Include `--api-auth` flag when running stable-diffusion-webui':
			"Добавьте флаг '--api-auth' при запуске stable-diffusion-webui",
		...
		'Influences how quickly the algorithm responds to feedback from the generated text. A lower learning rate will result in slower adjustments, while a higher learning rate will make the algorithm more responsive.':
			'Влияет на то, насколько быстро алгоритм реагирует на обратную связь из сгенерированного текста. Более низкая скорость обучения приведет к более медленной корректировке, в то время как более высокая скорость обучения сделает алгоритм более отзывчивым.',
		Info: Ao,
		Initials: bo,
		'Inject file content into conversation context':
			'Вставлять содержимое файлов в контекст разговора',
		'Inject the entire content as context for comprehensive processing, this is recommended for complex queries.':
			'Использовать весь контент в качестве контекста для комплексной обработки, это рекомендуется для сложных запросов.',
		Input: wo,
		...
		'Instant Auto-Send After Voice Transcription':
			'Мгновенная автоматическая отправка после расшифровки голоса',
		Instructions: Io,
		Integration: Po,
		Integrations: Co,
		Interface: vo,
		...
		'Invalid JSON format for ComfyUI Edit Workflow.':
			'Недопустимый формат JSON для ComfyUI Edit Workflow.',
		...
		'Invalid JSON format in Additional Config':
			'Неверный формат JSON в дополнительной конфигурации',
		...
		'Leave empty to include all models from "{{url}}" endpoint':
			'Оставьте пустым, чтобы включить все модели из конечной точки "{{url}}"',
		'Leave empty to include all models from "{{url}}/api/tags" endpoint':
			'Оставьте пустым, чтобы включить все модели из конечной точки "{{url}}/api/tags"',
		'Leave empty to include all models from "{{url}}/models" endpoint':
			'Оставьте поле пустым, чтобы включить все модели из конечной точки "{{url}}/models"',
		'Leave empty to include all models or select specific models':
			'Оставьте поле пустым, чтобы включить все модели или выбрать конкретные модели',
		'Leave empty to use first admin user':
			'Оставьте пустым, чтобы использовать первого администратора',
		'Leave empty to use the default config, or enter a valid json (see https://yandex.cloud/en/docs/search-api/api-ref/WebSearch/search#yandex.cloud.searchapi.v2.WebSearchRequest)':
			'Оставьте пустым, чтобы использовать конфигурацию по умолчанию или введите валидный JSON (смотри https://yandex.cloud/en/docs/search-api/api-ref/WebSearch/search#yandex.cloud.searchapi.v2.WebSearchRequest)',
		'Leave empty to use the default model (voxtral-mini-latest).':
			'Оставьте пустым, чтобы использовать модель по умолчанию (voxtral-mini-latest).',
		'Leave empty to use the default prompt, or enter a custom prompt':
			'Оставьте пустым, чтобы использовать промпт по умолчанию, или введите пользовательский промпт',
		'Leave model field empty to use the default model.':
			'Оставьте поле model пустым, чтобы использовать модель по умолчанию.',
		Legacy: xo,
		lexical: Fo,
		License: _o,
		...
		'Limit concurrent search queries. 0 = unlimited (default). Set to 1 for sequential execution (recommended for APIs with strict rate limits like Brave free tier).':
			'Лимит параллельных поисковых запросов. 0 = без ограничений (по умолчанию). Установите 1 для последовательного выполнения (рекомендуется для API со строгими лимитами, например, бесплатный тариф Brave).',
		'Limits the number of concurrent embedding requests. Set to 0 for unlimited.':
			'Ограничивает число параллельных запросов эмбеддингов. 0 — без ограничений.',
		List: Ko,
		...
		'LLMs can make mistakes. Verify important information.':
			'LLMs могут допускать ошибки. Проверяйте важную информацию.',
		Loader: Bo,
		...
		'Make sure to export a workflow.json file as API format from ComfyUI.':
			'Убедитесь, что экспортируете файл workflow.json в формате API из ComfyUI.',
		Male: zo,
		Manage: Qo,
		...
		'Maximum characters to return from fetched URLs. Leave empty for no limit.':
			'Максимальное количество символов, возвращаемых из загруженных URL. Оставьте пустым для снятия ограничения.',
		...
		'Maximum number of files per folder is {{max}}.':
			'Максимальное количество файлов в папке — {{max}}.',
		'Maximum of 3 models can be downloaded simultaneously. Please try again later.':
			'Максимальное количество моделей для загрузки одновременно - 3. Пожалуйста, попробуйте позже.',
		May: $o,
		MBR: en,
		MCP: tn,
		'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the Open WebUI team, making it the more reliable option for compatibility.':
			'Поддержка MCP является экспериментальной, а частые изменения спецификации могут приводить к несовместимостям. Поддержка OpenAPI напрямую обеспечивается командой Open WebUI, что делает её более надёжным вариантом.',
		Medium: on,
		...
		'Memories accessible by LLMs will be shown here.':
			'Воспоминания, доступные LLMs, будут отображаться здесь.',
		Memory: rn,
		...
		'Message counts are based on assistant responses.':
			'Количество сообщений основано на ответах ассистента.',
		'Message rating should be enabled to use this feature':
			'Чтобы использовать эту функцию, необходимо включить оценку сообщения.',
		messages: cn,
		Messages: dn,
		"Messages you send after creating your link won't be shared. Users with the URL will be able to view the shared chat.":
			'Сообщения, отправленные вами после создания ссылки, не будут передаваться другим. Пользователи, у которых есть URL, смогут просматривать общий чат.',
		...
		'MinerU API Key required for Cloud API mode.':
			'API-ключ MinerU необходим для режима Cloud API.',
		...
		"Model '{{modelTag}}' is already in queue for downloading.":
			"Модель '{{modelTag}}' уже находится в очереди на загрузку.",
		...
		'Model can execute code and perform calculations':
			'Модель может выполнять код и выполнять вычисления',
		'Model can generate images based on text prompts':
			'Модель может генерировать изображения на основе текстовых промптов',
		...
		'Model filesystem path detected. Model shortname is required for update, cannot continue.':
			'Обнаружен путь к файловой системе модели. Для обновления требуется краткое имя модели, не удается продолжить.',
		...
		'Model name already exists, please choose a different one':
			'Имя модели уже существует, пожалуйста, выберите другое',
		...
		'Name and ID are required, please fill them out':
			'Имя и ID обязательны, пожалуйста, заполните их',
		...
		'No expiration can pose security risks.':
			'Отсутствие срока действия может представлять угрозу безопасности.',
		...
		'No files yet. Upload files or run Python code to create them.':
			'Файлов пока нет. Загрузите файлы или создайте их с помощью Python.',
		...
		'No HTML, CSS, or JavaScript content found.':
			'Содержимое в формате HTML, CSS или JavaScript не найдено.',
		'No inference engine with management support found':
			'Механизм логического вывода с поддержкой управления не найден',
		'No kernel': 'Нет ядра',
		...
		'Note: If you set a minimum score, the search will only return documents with a score greater than or equal to the minimum score.':
			'Обратите внимание: Если вы установите минимальный балл, поиск будет возвращать только документы с баллом больше или равным минимальному баллу.',
		Notes: En,
		'Notes Public Sharing': 'Публичный обмен заметками',
		...
		'Only active when "Paste Large Text as File" setting is toggled on.':
			'Активно только при включённой настройке «Вставлять большой текст как файл».',
		'Only active when the chat input is in focus and an LLM is generating a response.':
			'Активно только при фокусе на поле ввода и генерации ответа.',
		'Only active when the chat input is in focus.': 'Активно только при фокусе на поле ввода.',
		'Only alphanumeric characters and hyphens are allowed':
			'Разрешены только буквенно-цифровые символы и дефисы.',
		'Only alphanumeric characters and hyphens are allowed in the command string.':
			'В строке команды разрешено использовать только буквенно-цифровые символы и дефисы.',
		'Only can be triggered when the chat input is in focus.':
			'Срабатывает только при фокусе на поле ввода.',
		'Only collections can be edited, create a new knowledge base to edit/add documents.':
			'Редактировать можно только коллекции, создайте новую базу знаний для редактирования/добавления документов.',
		...
		'Only select users and groups with permission can access':
			'Доступ имеют только избранные пользователи и группы, имеющие разрешение.',
		...
		'Oops! There are files still uploading. Please wait for the upload to complete.':
			'Упс! Есть файлы, которые все еще загружаются. Пожалуйста, дождитесь завершения загрузки.',
		'Oops! There was an error in the previous response.': 'Упс! В предыдущем ответе была ошибка.',
		"Oops! You're using an unsupported method (frontend only). Please serve the WebUI from the backend.":
			'Упс! Вы используете неподдерживаемый метод (только фронтенд). Пожалуйста, обслуживайте веб-интерфейс из бэкенда.',
		...
		'Open Modal To Manage Floating Quick Actions':
			'Открыть модальное окно для управления плавающими быстрыми действиями',
		...
		'Open WebUI can use tools provided by any OpenAPI server.':
			'Open WebUI может использовать инструменты, предоставляемые любым сервером OpenAPI.',
		'Open WebUI uses faster-whisper internally.':
			'Open WebUI использует более быстрый внутренний интерфейс whisper.',
		'Open WebUI uses SpeechT5 and CMU Arctic speaker embeddings.':
			'В Open WebUI используются встраиваемые движки генерации речи SpeechT5 и CMU Arctic.',
		'Open WebUI version': 'Версия Open WebUI',
		...
		'Page mode creates one document per page. Single mode combines all pages into one document for better chunking across page boundaries.':
			'Постраничный режим создаёт отдельный документ для каждой страницы. Общий режим объединяет все страницы в один документ для более качественного разбиения на фрагменты без привязки к границам страниц.',
		Paginate: zn,
		Parameters: Qn,
		...
		'Participate in community leaderboards and evaluations! Syncing aggregated usage stats helps drive research and improvements to Open WebUI. Your privacy is paramount: no message content is ever shared.':
			'Участвуйте в рейтингах и оценках сообщества! Синхронизация агрегированной статистики помогает развитию Open WebUI. Ваша конфиденциальность приоритетна: содержимое сообщений никогда не передаётся.',
		Password: Zn,
		...
		'Permission denied when accessing media devices':
			'Отказано в разрешении на доступ к мультимедийным устройствам',
		'Permission denied when accessing microphone': 'Отказано в разрешении на доступ к микрофону',
		'Permission denied when accessing microphone: {{error}}':
			'Отказано в разрешении на доступ к микрофону: {{error}}',
		Permissions: ta,
		...
		'Pipelines are a plugin system with arbitrary code execution —':
			'Конвейеры — это система плагинов с выполнением произвольного кода —',
		...
		'Please connect all required integrations before sending a message':
			'Пожалуйста, подключите все необходимые интеграции перед отправкой сообщения',
		'Please do not close the settings page while loading the model.':
			'Пожалуйста, не закрывайте страницу настроек во время загрузки модели.',
		'Please enter a message or attach a file.':
			'Пожалуйста, введите сообщение или прикрепите файл.',
		...
		'Please save the connection to persist the OAuth client information and do not change the ID':
			'Пожалуйста, сохраните подключение для сохранения данных OAuth-клиента и не меняйте ID',
		...
		'Please select at least one user for Direct Message channel.':
			'Пожалуйста, выберите хотя бы одного пользователя для канала личных сообщений.',
		'Please wait until all files are uploaded.':
			'Пожалуйста, подождите, пока все файлы будут загружены.',
		...
		'Prefix ID is used to avoid conflicts with other connections by adding a prefix to the model IDs - leave empty to disable':
			'ID префикса используется для предотвращения конфликтов с другими подключениями путем добавления префикса к идентификаторам моделей - оставьте пустым, чтобы отключить',
		...
		'Reduces the probability of generating nonsense. A higher value (e.g. 100) will give more diverse answers, while a lower value (e.g. 10) will be more conservative.':
			'Снижает вероятность появления бессмыслицы. Большее значение (например, 100) даст более разнообразные ответы, в то время как меньшее значение (например, 10) будет более консервативным.',
		'Refer to yourself as "User" (e.g., "User is learning Spanish")':
			'Называйте себя "User" (например, "User is learning Spanish").',
		...
		'Response notifications cannot be activated as the website permissions have been denied. Please visit your browser settings to grant the necessary access.':
			'Уведомления об ответах не могут быть активированы, поскольку доступ к веб-сайту был заблокирован. Пожалуйста, перейдите к настройкам своего браузера, чтобы предоставить необходимый доступ.',
		...
		'Runs embedding tasks concurrently to speed up processing. Turn off if rate limits become an issue.':
			'Параллельное выполнение задач эмбеддингов для ускорения. Отключите при проблемах с лимитами запросов.',
		Sa_day_of_week: qa,
		Save: Ha,
		...
		"Saving chat logs directly to your browser's storage is no longer supported. Please take a moment to download and delete your chat logs by clicking the button below. Don't worry, you can easily re-import your chat logs to the backend through":
			'Прямое сохранение журналов чата в хранилище вашего браузера больше не поддерживается. Пожалуйста, потратьте минуту, чтобы скачать и удалить ваши журналы чата, нажав на кнопку ниже. Не волнуйтесь, вы легко сможете повторно импортировать свои журналы чата в бэкенд через',
		Schedule: za,
		...
		'Searxng search language (all, en, es, de, fr, etc.)':
			'Язык поиска Searxng (all, en, es, de, fr и т. д.)',
		...
		'Select how to split message text for TTS requests':
			'Выберите, как разделять текст сообщения для TTS запросов',
		...
		'Selected model(s) do not support image inputs':
			'Выбранные модели не поддерживают ввод изображений',
...
Поддерживаемые провайдеры будут возвращать информацию об использовании токена в ответе, когда это будет установлено.`,
		...
		'Set reranking model (e.g. {{model}})':
			'Установить модель реранжирования (например, {{model}})',
		'Set the default models that are automatically selected for all users when a new chat is created.':
			'Задайте модели по умолчанию, которые автоматически выбираются для всех пользователей при создании нового чата.',
		'Set the models that are automatically pinned to the sidebar for all users.':
			'Задайте модели, автоматически закреплённые в боковой панели для всех пользователей.',
		'Set the number of layers, which will be off-loaded to GPU. Increasing this value can significantly improve performance for models that are optimized for GPU acceleration but may also consume more power and GPU resources.':
			'Задайте количество слоев, которые будут загружаться в графический процессор. Увеличение этого значения может значительно повысить производительность моделей, оптимизированных для ускорения работы графического процессора, но также может потреблять больше энергии и ресурсов графического процессора.',
		...
		'Sets a flat bias against tokens that have appeared at least once. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'Устанавливает нулевое значение для символов, которые появились хотя бы один раз. Более высокое значение (например, 1,5) будет более строгим наказанием за повторения, в то время как более низкое значение (например, 0,9) будет более мягким. При значении 0 он отключается.',
		'Sets a scaling bias against tokens to penalize repetitions, based on how many times they have appeared. A higher value (e.g., 1.5) will penalize repetitions more strongly, while a lower value (e.g., 0.9) will be more lenient. At 0, it is disabled.':
			'Устанавливает смещение масштабирования для токенов, чтобы наказывать за повторения, в зависимости от того, сколько раз они появлялись. Более высокое значение (например, 1,5) будет наказывать за повторения более строго, в то время как более низкое значение (например, 0,9) будет более мягким. При значении 0 оно отключается.',
		'Sets how far back for the model to look back to prevent repetition.':
			'Задает, как далеко назад модель должна вернуться, чтобы предотвратить повторение.',
		'Sets the random number seed to use for generation. Setting this to a specific number will make the model generate the same text for the same prompt.':
			'Задает начальное значение случайного числа, которое будет использоваться для генерации. Установка этого значения на определенное число заставит модель генерировать один и тот же текст для одного и того же запроса.',
		'Sets the size of the context window used to generate the next token.':
			'Устанавливает размер контекстного окна, используемого для генерации следующего токена.',
		'Sets the stop sequences to use. When this pattern is encountered, the LLM will stop generating text and return. Multiple stop patterns may be set by specifying multiple separate stop parameters in a modelfile.':
			'Устанавливает используемые последовательности остановок. При обнаружении этого шаблона LLM прекратит генерировать текст и вернет данные. Можно задать несколько шаблонов остановок, указав несколько отдельных параметров остановки в файле модели.',
		Setting: as,
		Settings: ss,
		...
		'Show Admin Details in Account Pending Overlay':
			'Показывать данные администратора в оверлее ожидающей учетной записи',
		...
		'Showing all messages (user + assistant) per user.':
			'Показаны все сообщения (пользователя и ассистента) для каждого пользователя.',
		'Sign in': 'Войти',
		...
		'Significantly improves accuracy by using an LLM to enhance tables, forms, inline math, and layout detection. Will increase latency. Defaults to False.':
			'Значительно повышает точность, используя LLM для улучшения обнаружения таблиц, форм, встроенной математики и макета. Увеличит задержку. По умолчанию False.',
		...
		'Skills Public Sharing': 'Публичный обмен скиллами',
		...
		'Skip the cache and re-run the inference. Defaults to False.':
			'Пропустить кэширование и перезапустить вывод. По умолчанию установлено значение Выкл.',
		...
		'Split documents by markdown headers before applying character/token splitting.':
			'Разделять документы по заголовкам Markdown перед разбиением на символы/токены.',
		...
		'Strip existing OCR text from the PDF and re-run OCR. Ignored if Force OCR is enabled. Defaults to False.':
			'Удалять существующий текст OCR из PDF и перезапустить OCR. Игнорируется если Принудительное OCR включено. По умолчанию установлено значение Выкл.',
		...
		'Successfully imported {{userCount}} users.':
			'Успешно импортировано {{userCount}} пользователей.',
		...
		'Syncs only chats with updates after your last sync timestamp. Disable to re-sync all chats.':
			'Синхронизирует только чаты, обновлённые после последней синхронизации. Отключите для повторной синхронизации всех чатов.',
		System: vs,
		...
		'Tail free sampling is used to reduce the impact of less probable tokens from the output. A higher value (e.g., 2.0) will reduce the impact more, while a value of 1.0 disables this setting.':
			'Выборка без хвостов используется для уменьшения влияния менее вероятных токенов на выходные данные. Более высокое значение (например, 2.0) еще больше уменьшит влияние, в то время как значение 1.0 отключает эту настройку.',
		...
		'The Application Account DN you bind with for search':
			'Логин учетной записи приложения, к которому вы привязываетесь для поиска',
		...
		'The batch size determines how many text requests are processed together at once. A higher batch size can increase the performance and speed of the model, but it also requires more memory.':
			'Размер пакета определяет, сколько текстовых запросов обрабатывается одновременно. Больший размер пакета может повысить производительность и быстродействие модели, но также требует больше памяти.',
		'The developers behind this plugin are passionate volunteers from the community. If you find this plugin helpful, please consider contributing to its development.':
			'Разработчики этого плагина - увлеченные волонтеры из сообщества. Если вы считаете этот плагин полезным, пожалуйста, подумайте о том, чтобы внести свой вклад в его разработку.',
		'The evaluation leaderboard is based on the Elo rating system and is updated in real-time.':
			'Таблица лидеров оценки основана на рейтинговой системе Elo и обновляется в режиме реального времени.',
		'The format to return a response in. Format can be json or a JSON schema.':
			'Формат, в котором будет возвращен ответ. Форматом может быть json или схема JSON.',
		'The height in pixels to compress images to. Leave empty for no compression.':
			'Высота в пикселях для сжатия картинок. Оставьте пустым, чтобы не сжимать.',
		'The language of the input audio. Supplying the input language in ISO-639-1 (e.g. en) format will improve accuracy and latency. Leave blank to automatically detect the language.':
			'Язык входного аудиосигнала.  Укажите язык ввода в формате ISO-639-1 (например, en), что повысит точность и время ожидания. Оставьте поле пустым для автоматического определения языка.',
		'The LDAP attribute that maps to the mail that users use to sign in.':
			'Атрибут LDAP, который сопоставляется с почтой, используемой пользователями для входа в систему.',
		'The LDAP attribute that maps to the username that users use to sign in.':
			'Атрибут LDAP, который сопоставляется с именем пользователя, используемым пользователями для входа в систему.',
		'The leaderboard is currently in beta, and we may adjust the rating calculations as we refine the algorithm.':
			'В настоящее время таблица лидеров находится в стадии бета-тестирования, и мы можем скорректировать расчеты рейтинга по мере доработки алгоритма.',
		'The maximum file size in MB. If the file size exceeds this limit, the file will not be uploaded.':
			'Максимальный размер файла в МБ. Если размер файла превысит это ограничение, файл не будет загружен.',
		'The maximum number of files that can be used at once in chat. If the number of files exceeds this limit, the files will not be uploaded.':
			'Максимальное количество файлов, которые могут быть использованы одновременно в чате. Если количество файлов превысит это ограничение, файлы не будут загружены.',
		"The output format for the text. Can be 'json', 'markdown', or 'html'. Defaults to 'markdown'.":
			"Формат вывода текста. Может быть 'json', 'markdown', или 'html'. По умолчанию 'markdown'.",
		"The passwords you entered don't quite match. Please double-check and try again.":
			'Пароли не сходятся. Проверь еще раз и попробуй снова.',
		'The score should be a value between 0.0 (0%) and 1.0 (100%).':
			'Оценка должна быть значением между 0,0 (0%) и 1,0 (100%).',
		'The stream delta chunk size for the model. Increasing the chunk size will make the model respond with larger pieces of text at once.':
			'Размер чанка дельты потока для модели. Увеличение размера чанка заставит модель отвечать большими кусками текста одновременно.',
		'The temperature of the model. Increasing the temperature will make the model answer more creatively.':
			'Температура модели. При повышении температуры модель будет отвечать более творчески.',
		'The Weight of BM25 Hybrid Search. 0 more semantic, 1 more lexical. Default 0.5':
			'Вес BM25 в гибридном поиске. 0 — больше семантики, 1 — больше лексики. По умолчанию 0.5',
		'The width in pixels to compress images to. Leave empty for no compression.':
			'Ширина в пикселях для сжатия изображений. Оставьте пустым для отключения сжатия.',
		Theme: Ms,
		'There was an error syncing your stats. Please try again.':
			'Произошла ошибка при синхронизации статистики. Попробуйте снова.',
		'Thinking...': 'Думаю...',
		'This action cannot be undone. Do you wish to continue?':
			'Это действие нельзя отменить. Вы хотите продолжить?',
		'This channel was created on {{createdAt}}. This is the very beginning of the {{channelName}} channel.':
			'Этот канал был создан {{createdAt}}. Это самое начало канала {{channelName}}.',
		"This chat won't appear in history and your messages will not be saved.":
			'Этот чат не появится в истории, и ваши сообщения не будут сохранены.',
		'This ensures that your valuable conversations are securely saved to your backend database. Thank you!':
			'Это обеспечивает сохранение ваших ценных разговоров в безопасной базе данных на вашем сервере. Спасибо!',
		'This feature is currently experimental and may not work as expected.':
			'Эта функция экспериментальна и может работать не так, как ожидается.',
		'This feature is experimental and may be modified or discontinued without notice.':
			'Эта функция является экспериментальной и может быть изменена или отключена без предупреждения.',
		'This folder is empty': 'Эта папка пуста',
		'This is a default user permission and will remain enabled.':
			'Это разрешение по умолчанию, оно остаётся включённым.',
		'This is an experimental feature, it may not function as expected and is subject to change at any time.':
			'Это экспериментальная функция, она может работать не так, как ожидалось, и может быть изменена в любое время.',
		'This model is not publicly available. Please select another model.':
			'Эта модель недоступна в открытом доступе. Пожалуйста, выберите другую модель.',
		'This option controls how long the model will stay loaded into memory following the request (default: 5m)':
			'Этот параметр определяет, как долго модель будет оставаться загруженной в память после запроса (по умолчанию: 5 месяцев).',
		'This option controls how many tokens are preserved when refreshing the context. For example, if set to 2, the last 2 tokens of the conversation context will be retained. Preserving context can help maintain the continuity of a conversation, but it may reduce the ability to respond to new topics.':
			'Этот параметр определяет, сколько токенов сохраняется при обновлении контекста. Например, если задано значение 2, будут сохранены последние 2 токена контекста беседы. Сохранение контекста может помочь сохранить непрерывность беседы, но может уменьшить возможность отвечать на новые темы.',
		'This option enables or disables the use of the reasoning feature in Ollama, which allows the model to think before generating a response. When enabled, the model can take a moment to process the conversation context and generate a more thoughtful response.':
			'Эта опция включает или отключает использование функции рассуждения в Ollama, которая позволяет модели подумать, прежде чем генерировать ответ. Если она включена, модель может потратить некоторое время на обработку контекста разговора и выработку более продуманного ответа.',
		'This option sets the maximum number of tokens the model can generate in its response. Increasing this limit allows the model to provide longer answers, but it may also increase the likelihood of unhelpful or irrelevant content being generated.':
			'Этот параметр устанавливает максимальное количество токенов, которые модель может генерировать в своем ответе. Увеличение этого ограничения позволяет модели предоставлять более длинные ответы, но также может увеличить вероятность создания бесполезного или нерелевантного контента.',
		'This option will delete all existing files in the collection and replace them with newly uploaded files.':
			'Эта опция удалит все существующие файлы в коллекции и заменит их вновь загруженными файлами.',
		...
		'This will delete <strong>{{NAME}}</strong> and <strong>all its contents</strong>.':
			'При этом будет удален <strong>{{NAME}}</strong> и <strong>все его содержимое</strong>.',
		'This will delete all models including custom models':
			'Это приведет к удалению всех моделей, включая пользовательские модели.',
		'This will delete all models including custom models and cannot be undone.':
			'При этом будут удалены все модели, включая пользовательские, и это действие нельзя будет отменить.',
		'This will permanently delete the calendar "{{name}}" and all its events. This action cannot be undone.':
			'',
		'This will reset the knowledge base and sync all files. Do you wish to continue?':
			'Это сбросит базу знаний и синхронизирует все файлы. Хотите продолжить?',
		...
		'Thumbs up/down ratings from users on model responses':
			'Оценки «нравится/не нравится» от пользователей на ответы модели',
		Tika: Ns,
		...
		'To access the available model names for downloading,':
			'Чтобы получить доступ к доступным для загрузки названиям моделей,',
		'To access the GGUF models available for downloading,':
			'Чтобы получить доступ к моделям GGUF, доступным для загрузки,',
		'To access the WebUI, please reach out to the administrator. Admins can manage user statuses from the Admin Panel.':
			'Чтобы получить доступ к WebUI, пожалуйста, обратитесь к администратору. Администраторы могут управлять статусами пользователей из панели администратора.',
		'To attach knowledge base here, add them to the "Knowledge" workspace first.':
			'Чтобы прикрепить сюда базу знаний, сначала добавьте её в "Знания" рабочего пространства.',
		'To learn more about available endpoints, visit our documentation.':
			'Чтобы узнать больше о доступных энд-поинтах, ознакомьтесь с нашей документацией.',
		'To select skills here, add them to the "Skills" workspace first.':
			'Чтобы выбрать скиллы здесь, сначала добавьте их в раздел «Скиллы».',
		'To select toolkits here, add them to the "Tools" workspace first.':
			'Чтобы выбрать инструменты, сначала добавьте их в "Инструменты" рабочего пространства.',
		...
		'Token counts are estimates and may not reflect actual API usage':
			'Количество токенов является приблизительным и может не отражать фактическое использование API',
		tokens: Ys,
		Tokens: Vs,
		...
		'Tools are a function calling system with arbitrary code execution':
			'Инструменты - это система вызова функций с выполнением произвольного кода',
		'Tools Function Calling Prompt': 'Промпт на вызов функции Инструменты',
		'Tools have a function calling system that allows arbitrary code execution.':
			'Инструменты имеют систему вызова функций, которая позволяет выполнять произвольный код.',
		'Tools Public Sharing': 'Публичный обмен инструментами',
		'Tools Sharing': 'Общий доступ к инструментам',
		Top: qs,
		'Top K': 'Top K',
		'Top K Reranker': 'Top K переранжировщик',
		Transformers: Hs,
		...
		'Try adjusting your search or filter to find what you are looking for.':
			'Попробуйте изменить поиск или фильтр, чтобы найти нужное.',
		...
		'Update for the latest features and improvements.':
			'Обновитесь для получения последних функций и улучшений.',
		...
		'Upgrade to a licensed plan for enhanced capabilities, including custom theming and branding, and dedicated support.':
			'Перейдите на лицензионный тарифный план, чтобы получить расширенные возможности, включая настраиваемую тематику и фирменный стиль, а также специальную поддержку.',
		Upload: ar,
		...
		'Upload Progress: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)':
			'Прогресс загрузки: {{uploadedFiles}}/{{totalFiles}} ({{percentage}}%)',
		...
		"Use '#' in the prompt input to load and include your knowledge.":
			'Используйте «#» в строке ввода, чтобы загрузить и включить свои знания.',
		'Use /v1/chat/completions endpoint instead of /v1/audio/transcriptions for potentially better accuracy.':
			'Используйте эндпоинт /v1/chat/completions вместо /v1/audio/transcriptions для потенциально лучшей точности.',
		'Use Chat Completions API': 'Использовать Chat Completions API',
		'Use groups to organize your users and assign permissions.':
			'Используйте группы для организации пользователей и назначения прав.',
		'Use LLM': 'Использовать LLM',
		'Use no proxy to fetch page contents.':
			'Не используйте прокси-сервер для получения содержимого страницы.',
		'Use proxy designated by http_proxy and https_proxy environment variables to fetch page contents.':
			'Используйте прокси-сервер, обозначенный переменными окружения http_proxy и https_proxy, для получения содержимого страницы.',
		user: lr,
		User: cr,
		...
		'Uses DefaultAzureCredential to authenticate':
			'Использует DefaultAzureCredential для аутентификации',
		'Uses OAuth 2.1 Dynamic Client Registration':
			'Использует динамическую регистрацию клиентов OAuth 2.1',
		...
		'Using the default arena model with all models. Click the plus button to add custom models.':
			'Использование модели арены по умолчанию со всеми моделями. Нажмите кнопку «плюс», чтобы добавить пользовательские модели.',
		...
		'Version {{selectedVersion}} of {{totalVersions}}':
			'Версия {{selectedVersion}} из {{totalVersions}}',
		...
		'Voice Mode Custom Prompt': 'Свой промпт голосового режима',
		...
		'Warning: Enabling this will allow users to upload arbitrary code on the server.':
			'Предупреждение. Включение этого параметра позволит пользователям загружать произвольный код на сервер.',
		'Warning: Jupyter execution enables arbitrary code execution, posing severe security risks—proceed with extreme caution.':
			'Предупреждение: Выполнение Jupyter позволяет выполнять произвольный код, что создает серьезные угрозы безопасности — действуйте с особой осторожностью.',
		We_day_of_week: Sr,
		Web: Ir,
		'Web API': 'Веб API',
		...
		'WebUI will make requests to "{{url}}/api/chat"':
			'WebUI будет отправлять запросы к "{{url}}/api/chat"',
		'WebUI will make requests to "{{url}}/chat/completions"':
			'WebUI будет отправлять запросы к "{{url}}/chat/completions"',
		Week: Cr,
		Weekly: vr,
		...
		'When enabled, the model will respond to each chat message in real-time, generating a response as soon as the user sends a message. This mode is useful for live chat applications, but may impact performance on slower hardware.':
			'Если эта функция включена, модель будет отвечать на каждое сообщение чата в режиме реального времени, генерируя ответ, как только пользователь отправит сообщение. Этот режим полезен для приложений живого чата, но может повлиять на производительность на более медленном оборудовании.',
		...
		'Whether to paginate the output. Each page will be separated by a horizontal rule and page number. Defaults to False.':
			'Следует ли разбивать выходные данные на страницы. Каждая страница будет разделена горизонтальной линией и номером страницы. По умолчанию установлено значение Выкл.',
		...
		'Works together with top-k. A higher value (e.g., 0.95) will lead to more diverse text, while a lower value (e.g., 0.5) will generate more focused and conservative text.':
			'Работает совместно с top-k. Более высокое значение (например, 0,95) приведет к более разнообразному тексту, в то время как более низкое значение (например, 0,5) приведет к созданию более сфокусированного и консервативного текста.',
		Workspace: Dr,
		'Workspace Permissions': 'Разрешения для Рабочего пространства',
		Write: Rr,
		'Write a summary in 50 words that summarizes {{topic}}.':
			'Напишите резюме в 50 словах, которое кратко описывает [тему или ключевое слово].',
...
например: вы — Марио из Super Mario Bros и выступаете в роли ассистента.`,
		...
		'You are currently using a trial license. Please contact support to upgrade your license.':
			'В настоящее время вы используете пробную лицензию. Пожалуйста, обратитесь в службу поддержки для обновления вашей лицензии.',
		'You can only chat with a maximum of {{maxCount}} file(s) at a time.':
			'Одновременно вы можете общаться только с максимальным количеством файлов {{maxCount}}.',
		"You can personalize your interactions with LLMs by adding memories through the 'Manage' button below, making them more helpful and tailored to you.":
			'Вы можете персонализировать свое взаимодействие с LLMs, добавив воспоминания с помощью кнопки "Управлять" ниже, что сделает их более полезными и адаптированными для вас.',
		...
		'You do not have permission to edit this prompt.':
			'У вас нет прав на редактирование этого промпта.',
		'You do not have permission to edit this skill.':
			'У вас нет прав на редактирование этого скилла.',
		'You do not have permission to edit this tool':
			'У вас нет прав на редактирование этого инструмента',
		...
		'You do not have permission to send messages in this thread.':
			'У вас нет прав на отправку сообщений в этом обсуждении.',
		'You do not have permission to upload files to this knowledge base.':
			'У вас нет прав на загрузку файлов в эту базу знаний.',
		...
		'Your account status is currently pending activation.':
			'В настоящее время ваша учетная запись ожидает активации.',
		...
		'Your entire contribution will go directly to the plugin developer; Open WebUI does not take any percentage. However, the chosen funding platform might have its own fees.':
			'Весь ваш взнос будет направлен непосредственно разработчику плагина; Open WebUI не взимает никаких процентов. Однако выбранная платформа финансирования может иметь свои собственные сборы.',
		'Your message text or inputs': 'Текст ваших сообщений',
		'Your usage stats have been successfully synced.':
			'Ваша статистика использования успешно синхронизирована.',
		YouTube: xr,
...
//# sourceMappingURL=fE-6uWoH.js.map
