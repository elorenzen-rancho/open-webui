...
var w = function (t, e) {
		for (var a = '', r = t.length && t[t.length - 1].loc.start, i = t.length - 1; i >= 0; i--)
			(t[i].loc.start > r && ((a += ' '), (r = t[i].loc.start)),
				(a += t[i].text),
				(r += t[i].text.length));
		var c = p.go(n.go(a, e));
		return c;
	...
	n = {
		go: function (t, e) {
			...
			for (var i, c = 10, s = []; ; ) {
				i !== t ? ((c = 10), (i = t)) : c--;
				var _ = n.stateMachines[e],
					u = _.transitions[a] || _.transitions['*'];
				t: for (var h = 0; h < u.length; h++) {
					...
					if (x) {
						...
						if (((a = l.nextState || a), t.length > 0)) {
							if ((l.revisit || (t = x.remainder), !l.toContinue)) break t;
						} else return s;
					}
				}
				if (c <= 0) throw ['MhchemBugU', 'mhchem bug U. Please report.'];
			}
		},
		...
		patterns: {
			...
			findObserveGroups: function (t, e, a, r, i, c, s, _, u, h) {
				var x = function (v, m) {
						if (typeof m == 'string') return v.indexOf(m) !== 0 ? null : m;
						var y = v.match(m);
						return y ? y[0] : null;
					},
					l = function (v, m, y) {
						for (var $ = 0; m < v.length; ) {
							var z = v.charAt(m),
								O = x(v.substr(m), y);
							if (O !== null && $ === 0) return { endMatchBegin: m, endMatchEnd: m + O.length };
							if (z === '{') $++;
							else if (z === '}') {
								if ($ === 0)
					...
					d = x(t, e);
				if (d === null || ((t = t.substr(d.length)), (d = x(t, a)), d === null)) return null;
				...
				if (c || s) {
					...
					var k = [f, q.match_];
					return { match_: h ? k.join('') : k, remainder: q.remainder };
				} else return { match_: f, remainder: t.substr(g.endMatchEnd) };
			},
			match_: function (t, e) {
				var a = n.patterns.patterns[t];
				if (a === void 0) throw ['MhchemBugP', 'mhchem bug P. Please report. (' + t + ')'];
				if (typeof a == 'function') return n.patterns.patterns[t](e);
				var r = e.match(a);
				if (r) {
					var i;
					return (
						r[2] ? (i = [r[1], r[2]]) : r[1] ? (i = r[1]) : (i = r[0]),
						{ match_: i, remainder: e.substr(r[0].length) }
					);
				}
				return null;
			}
		...
		actions: {
			...
			'1/2': function (t, e) {
				...
				return (
					(r[1] = r[1].replace(/\$/g, '')),
					a.push({ type_: 'frac', p1: r[1], p2: r[2] }),
					r[3] && ((r[3] = r[3].replace(/\$/g, '')), a.push({ type_: 'tex-math', p1: r[3] })),
					a
				);
			},
			'9,9': function (t, e) {
		...
		createTransitions: function (t) {
			...
			for (e in t)
				for (a in t[e])
					for (r = a.split('|'), t[e][a].stateArray = r, i = 0; i < r.length; i++) c[r[i]] = [];
			for (e in t)
				for (a in t[e])
					for (r = t[e][a].stateArray || [], i = 0; i < r.length; i++) {
						...
						if (s.action_) {
							...
							for (var _ = 0; _ < s.action_.length; _++)
								typeof s.action_[_] == 'string' && (s.action_[_] = { type_: s.action_[_] });
						} else s.action_ = [];
						for (var u = e.split('|'), h = 0; h < u.length; h++)
							if (r[i] === '*') for (var x in c) c[x].push({ pattern: u[h], task: s });
							else c[r[i]].push({ pattern: u[h], task: s });
					}
			return c;
		},
		stateMachines: {}
	...
	ce: {
		...
		actions: {
			...
			'- after o/d': function (t, e, a) {
				var r = n.patterns.match_('orbital', t.o || ''),
					i = n.patterns.match_('one lowercase greek letter $', t.o || ''),
					c = n.patterns.match_('one lowercase latin letter $', t.o || ''),
					s = n.patterns.match_('$one lowercase latin letter$ $', t.o || ''),
					_ = e === '-' && ((r && r.remainder === '') || i || c || s);
				_ && !t.a && !t.b && !t.p && !t.d && !t.q && !r && c && (t.o = '$' + t.o + '$');
				var u = [];
				return (
					_
						? (n.concatArray(u, this.output(t)), u.push({ type_: 'hyphen' }))
						: ((r = n.patterns.match_('digits', t.d || '')),
							a && r && r.remainder === ''
								? (n.concatArray(u, n.actions['d='](t, e)), n.concatArray(u, this.output(t)))
								: (n.concatArray(u, this.output(t)), n.concatArray(u, n.actions.bond(t, e, '-')))),
					u
				);
			},
			'a to o': function (t) {
				...
				if (!t.r)
					((r = []),
						(!t.a && !t.b && !t.p && !t.o && !t.q && !t.d && !a) ||
							...
							!t.o && !t.q && !t.d && !t.b && !t.p && a !== 2
								? ((t.o = t.a), (t.a = void 0))
								: !t.o && !t.q && !t.d && (t.b || t.p)
									? ((t.o = t.a), (t.d = t.b), (t.q = t.p), (t.a = t.b = t.p = void 0))
									: t.o && t.dType === 'kv' && n.patterns.match_('d-oxidation$', t.d || '')
										? (t.dType = 'oxidation')
					...
					var i;
					t.rdt === 'M'
						? (i = n.go(t.rd, 'tex-math'))
						: t.rdt === 'T'
							? (i = [{ type_: 'text', p1: t.rd || '' }])
							: (i = n.go(t.rd));
					var c;
					(t.rqt === 'M'
						? (c = n.go(t.rq, 'tex-math'))
						: t.rqt === 'T'
							? (c = [{ type_: 'text', p1: t.rq || '' }])
							: (c = n.go(t.rq)),
						(r = { type_: 'arrow', r: t.r, rd: i, rq: c }));
				}
				for (var s in t) s !== 'parenthesisLevel' && s !== 'beginsWithBond' && delete t[s];
				return r;
			},
		...
		actions: {
			enumber: function (t, e) {
				...
				return (
					...
					e[1] &&
						...
						e[2] && (e[2].match(/[,.]/) ? n.concatArray(a, n.go(e[2], 'pu-9,9')) : a.push(e[2])),
						(e[3] = e[4] || e[3]),
						e[3] &&
							((e[3] = e[3].trim()),
							e[3] === 'e' || e[3].substr(0, 1) === '*'
								? a.push({ type_: 'cdot' })
								: a.push({ type_: 'times' }))),
					e[3] && a.push('10^{' + e[5] + '}'),
					a
			...
			output: function (t) {
				...
				if (
					...
					var i = { d: n.go(t.d, 'pu'), q: n.go(t.q, 'pu') };
					t.o === '//'
						? (e = { type_: 'pu-frac', p1: i.d, p2: i.q })
						: ((e = i.d),
							i.d.length > 1 || i.q.length > 1 ? e.push({ type_: ' / ' }) : e.push({ type_: '/' }),
							n.concatArray(e, i.q));
				} else e = n.go(t.d, 'pu-2');
				for (var c in t) delete t[c];
				return e;
			}
		}
	},
		...
		actions: {
			...
			output: function (t) {
				...
				if (t.rm) {
					...
					a && a.remainder === '' ? (e = n.go(a.match_, 'pu')) : (e = { type_: 'rm', p1: t.rm });
				}
...
var p = {
	go: function (t, e) {
		...
		for (var a = '', r = !1, i = 0; i < t.length; i++) {
			...
			typeof c == 'string'
				? (a += c)
				: ((a += p._go2(c)), c.type_ === '1st-level escape' && (r = !0));
		}
		return (!e && !r && a && (a = '{' + a + '}'), a);
	},
	_goInner: function (t) {
	...
	_go2: function (t) {
		...
		switch (t.type_) {
			case 'chemfive':
				...
				(a.a && (a.a.match(/^[+\-]/) && (a.a = '{' + a.a + '}'), (e += a.a + '\\,')),
					(a.b || a.p) &&
						...
						(e += '{\\vphantom{X}}'),
						(e += '^{\\smash[t]{\\vphantom{2}}\\mathllap{' + (a.b || '') + '}}'),
						(e += '_{\\vphantom{2}\\mathllap{\\smash[t]{' + (a.p || '') + '}}}')),
			...
			case 'color0':
				...
				break;
			case 'arrow':
				var c = { rd: p._goInner(t.rd), rq: p._goInner(t.rq) },
					s = '\\x' + p._getArrow(t.r);
...
//# sourceMappingURL=pz-xsCF-.js.map
