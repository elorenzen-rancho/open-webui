import './CWj6FrbW.js';
import './69_IOA4Y.js';
import { t as i, a as n, F as p } from './CD8kWLvT.js';
import { a as l, s as m, c as d } from './BfA5TzUS.js';
import { p as e } from './DbBXQmQ3.js';
var c = p(
	'<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path d="M20 13V19C20 20.1046 19.1046 21 18 21H6C4.89543 21 4 20.1046 4 19V13" stroke-linecap="round" stroke-linejoin="round"></path><path d="M12 15V3M12 3L8.5 6.5M12 3L15.5 6.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>'
);
function g(o, r) {
	let s = e(r, 'className', 8, 'size-4'),
		a = e(r, 'strokeWidth', 8, '1.5');
	var t = c();
	(i(() => {
		(l(t, 'stroke-width', a()), m(t, 0, d(s())));
	}),
		n(o, t));
}
export { g as S };
//# sourceMappingURL=sxdcRLV-.js.map
