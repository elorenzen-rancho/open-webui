...
import { i as A } from '../chunks/CdIW-EUl.js';
import { e as V, i as Z } from '../chunks/BUYMbiFA.js';
import { a as C } from '../chunks/BfA5TzUS.js';
import { b as Le } from '../chunks/QcGoGJ5n.js';
import { b as ee } from '../chunks/BCA7d6a5.js';
import { p as je } from '../chunks/Bfc47y5P.js';
import { i as De } from '../chunks/BolXBFGq.js';
import { b as Ee } from '../chunks/CGOTt5HQ.js';
import { t as F } from '../chunks/DtbOSK9Q.js';
import { g as Ie } from '../chunks/D8_neGVh.js';
import { u as Ue } from '../chunks/Bumd5cCN.js';
import { i as ze, a as $e } from '../chunks/CUB_1BUI.js';
import { S as Ae } from '../chunks/BqNz_fa8.js';
var Ce = p(
	...
	Fe = p('<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3"></div>'),
	Me = p(
		'<div class="h-full flex items-center justify-center text-gray-400 dark:text-gray-600 text-sm"> </div>'
	),
	Se = p(
	...
	Ge = p('<div class="flex flex-wrap gap-2 mb-2"></div>'),
	He = p(
		'<button class="px-3.5 py-1.5 text-sm font-medium bg-black hover:bg-gray-900 text-white dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"> </button>'
	),
	Te = p(
		'<button class="px-3.5 py-1.5 text-sm font-medium bg-gray-300 text-black transition rounded-lg flex items-center gap-2" disabled=""><!> </button>'
	),
	qe = p(
...
function Be(j, M) {
	...
	const G = () => {
			a(u) &&
				(Q(u, (a(u).style.height = '')),
				Q(u, (a(u).style.height = Math.min(a(u).scrollHeight, 150) + 'px')));
		},
		oe = (e) => {
			const t = e.target;
			t.files &&
				Array.from(t.files).forEach((i) => {
					const s = new FileReader();
					((s.onload = (n) => {
						var l;
						(l = n.target) != null && l.result && v(d, [...a(d), n.target.result]);
					}),
						s.readAsDataURL(i));
				});
		},
			...
			t &&
				Array.from(t).forEach((s) => {
					if (s.type.startsWith('image/')) {
						...
						((n.onload = (l) => {
							var m;
							(m = l.target) != null && m.result && v(d, [...a(d), l.target.result]);
						}),
							n.readAsDataURL(s));
					}
				});
		...
		H = async () => {
			if (!a(x).trim()) {
				F.error(g().t('Please enter a prompt'));
				return;
			}
			v(k, !0);
			try {
				...
				(a(d).length > 0
					? (a(d).length,
						(e = await ze(localStorage.token, a(d).length === 1 ? a(d)[0] : a(d), a(x))))
					: (e = await $e(localStorage.token, a(x))),
					e && v(R, [...e, ...a(R)]));
			} catch (e) {
				F.error(`${e}`);
			} finally {
		...
		le = async (e, t) => {
			try {
				...
				((l.href = n),
					(l.download = `image-${Date.now()}-${t}.png`),
					l.click(),
					URL.revokeObjectURL(n));
			} catch {
				F.error(g().t('Failed to download image'));
			}
		};
	(ye(async () => {
		var e;
		if (((e = te()) == null ? void 0 : e.role) !== 'admin') {
			await Ie('/');
			return;
		}
	}),
		...
		ne = o(K);
	{
		var de = (e) => {
			...
			ce = (e) => {
				var t = Me(),
					i = o(t, !0);
				(r(t), y((s) => L(i, s), [() => g().t('Generated images will appear here')]), f(e, t));
			};
		A(ne, (e) => {
			a(R).length > 0 ? e(de) : e(ce, -1);
		});
	}
	(r(K), r(B), r(I));
	var O = h(I, 2),
		...
		var ve = (e) => {
			...
			(V(
				...
				(i, s, n) => {
					...
					var Y = h(m, 2),
						be = o(Y);
					(r(Y), r(l), y(() => C(pe, 'src', a(s))), c('click', be, () => se(n)), f(i, l));
				}
			),
				r(t),
				f(e, t));
		...
		ue = o(X);
	{
		var me = (e) => {
			...
			fe = (e) => {
				var t = Te(),
					i = o(t);
				Ae(i, { className: 'size-4' });
				var s = h(i);
				(r(t), y((n) => L(s, ` ${n ?? ''}`), [() => g().t('Running...')]), f(e, t));
			};
		A(ue, (e) => {
			a(k) ? e(fe, -1) : e(me);
		});
	...
	(r(X),
		...
		y(
			(e, t) => {
				(C(b, 'placeholder', e), L(ge, t));
			},
			[
				() => (a(d).length > 0 ? g().t('Describe the edit...') : g().t('Describe the image...')),
				() => g().t('Add Image')
			]
		),
		Le(
		...
		c('focus', b, G),
		c('keydown', b, (e) => {
			e.key === 'Enter' && (e.metaKey || e.ctrlKey) && !a(k) && (e.preventDefault(), H());
		}),
		c('change', $, oe),
		c('click', w, () => {
			var e;
			return (e = a(D)) == null ? void 0 : e.click();
		}),
		c(
			'dragover',
			w,
...
//# sourceMappingURL=32.C1II38cJ.js.map
